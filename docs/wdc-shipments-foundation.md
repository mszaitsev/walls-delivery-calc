...
Shipment admin AJAX orchestration is now outside `OrderShipmentsMetabox`. The metabox still registers the same `wp_ajax_*` action names and localizes the same JS action keys, but the callbacks point to endpoint-owning controllers in `src/Shipments/Admin/Ajax/`. `ShipmentAdminAjaxService` is intentionally small and contains only shared AJAX helpers; create, preview, status, lifecycle continuation, cancel/remove, manual attach, address/pickup/product search, CDEK barcode prepare and DPD contact-history logic live in their concrete controllers. Shared carrier UI payload construction lives in `ShipmentAdminCarrierUiPayloadBuilder`. Since 0.120.1 the controller dependencies are required constructor arguments, and `tests/shipments/run-shipment-admin-ajax-smoke.php` stubs `add_action()` to assert the runtime-registered callbacks are callable. This is an ownership-only refactor: HTML, modal fields, JavaScript, nonce names, request payloads, carrier APIs, persistence, documents, polling and status semantics are unchanged.

## Shipment Regression Profile 0.119.1
The canonical regression command is `php tests/shipments/run-shipment-regression-profile.php`. It runs the mandatory Shipment Framework profile from `tests/shipments/regression/shipment-regression-manifest.php`, launching every smoke as a separate PHP process so WordPress/WooCommerce stubs, globals and function declarations remain isolated. The runner collects a full report by default and supports `--fail-fast`, `--group=framework|cdek|dpd|russian-post|yandex|status-core`, `--list`, `--include-baseline` and `--include-optional`. Exit code `0` means the mandatory profile passed, `1` means a required regression or baseline mismatch failed, `2` means invalid CLI/manifest configuration, and `3` means `TIMEOUT` or `INFRASTRUCTURE`. Since 0.119.1 process infrastructure failures are counted separately from test failures and are classified before baseline matching; skipped counts are scoped to the selected group. Known baseline tests are exact-signature checked and excluded from the default profile; optional carrier harness tests run only when explicitly requested.
...
The shipment admin runtime is split by ownership. `shipments-admin.js` remains the enqueue/localization target and final bootstrap, `shipment-events.js` owns only carrier-neutral DOM event wiring, and generic modules own core helpers, preview, status rendering, polling, allocation and pickup/map behavior. Carrier-specific JavaScript lives under `assets/admin/shipments/extensions/` for CDEK, DPD, Russian Post and Yandex. In 0.117.1 carrier DOM selectors, sender/source picker handling, document-click handling and address-normalization post-processing moved behind small carrier hooks registered before `shipment-events.js` loads. In 0.117.2 polling and status rendering were tightened as carrier-neutral modules: Yandex cancellation/self-pickup presentation and CDEK default registration polling live in carrier extensions, while document visibility uses normalized document actions. In 0.117.3 Yandex polling hooks became carrier-scoped by canonical key `yandex_delivery`, and structure smoke validates duplicate functions, top-level `const`/`let` declarations and function/lexical collisions in production script order. In 0.117.4 the scanner was hardened with occurrence-based lexical tracking and self-tests for duplicate const/let declarations, const/let collisions, function/lexical collisions, local declarations, comments and strings. In 0.118.1 DPD lifecycle transport stays out of the DPD JS extension: common JS handles the neutral `lifecycle` contract, posts `continuation_token` to the generic continuation endpoint and reads lifecycle `purpose`, while DPD keeps contact/address/document/places presentation hooks. The split is behavior-neutral: existing selectors, field names, AJAX actions, payload shape, polling timing and carrier flows are unchanged.

## Shipment Lifecycle Contract 0.118.1
`ShipmentLifecycleResult` is the carrier-neutral response envelope for create flows that are not a single synchronous carrier create. It serializes `phase`, `accepted`, `submit_required`, `poll_required`, `continuation_token`, `message`, `poll_interval_ms`, `poll_max_attempts`, `purpose` and `stop_on_error` without carrier names or transport-specific identifiers. DPD begin returns `submission_required`; the shared `wdc_continue_shipment_lifecycle` admin AJAX endpoint resolves the adapter through the registry and calls `CarrierShipmentLifecycleContinuationInterface` only when the carrier implements it. DPD then performs the existing submit/poll/status classification inside `DpdOrderRegistrationService`, preserving persisted DPD registration fields and snapshots.
