...
Version 0.108.8 makes manual factual parcel input the shared shipment modal contract for every carrier. Initial editable place `weight_g`, `length_cm`, `width_cm` and `height_cm` are rendered empty without mutating the draft; all draft places, place numbers and item allocation rows remain visible. Submitted `places[]` remain the only source for preview/create, so empty fields fail existing strict validation instead of falling back to calculated package values. A single initial place shows only the calculated-weight hint `Расчётный вес товаров: %d г`; calculated dimensions are not shown as hints. Yandex manual attach still uses the existing generic manual attach UI, now labelled `Ввести номер Яндекс вручную` with `Request ID Яндекс` and placeholder `***-udp`.
