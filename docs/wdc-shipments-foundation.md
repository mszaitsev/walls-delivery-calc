...
Shipment admin AJAX orchestration is now outside `OrderShipmentsMetabox`. The metabox still registers the same `wp_ajax_*` action names and localizes the same JS action keys, but the callbacks point to endpoint-owning controllers in `src/Shipments/Admin/Ajax/`. `ShipmentAdminAjaxService` is intentionally small and contains only shared AJAX helpers; create, preview, status, lifecycle continuation, cancel/remove, manual attach, address/pickup/product search, CDEK barcode prepare and DPD contact-history logic live in their concrete controllers. Shared carrier UI payload construction lives in `ShipmentAdminCarrierUiPayloadBuilder`. This is an ownership-only refactor: HTML, modal fields, JavaScript, nonce names, request payloads, carrier APIs, persistence, documents, polling and status semantics are unchanged.
