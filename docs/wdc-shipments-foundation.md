...
Carrier-specific shipment modal field markup is now owned by small extensions registered through `ShipmentModalExtensionRegistry`. The common metabox keeps the modal shell and common places/items/preview/create lifecycle while CDEK, DPD, Russian Post and Yandex extensions render carrier-owned delivery, pickup and courier fragments without a generic form builder. Existing input names, data selectors and JavaScript behavior are preserved. Carrier-specific AJAX/pickup backend responsibilities remain as explicit follow-up debt.
