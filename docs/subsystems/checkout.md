...
PEK destination pickup foundation is not checkout runtime yet. Version 0.131.0 provides a generic pickup-provider registry and PEK admin-only terminal diagnostics, but the public pickup REST controllers do not receive that registry, PEK checkout rates are not calculated, and no PEK terminal selection is stored in checkout session. This is deliberate: PEK terminal search is credentialed, rate-limited, and depends on trusted cargo constraints that will be supplied by the future PEK quote/rate stage.
