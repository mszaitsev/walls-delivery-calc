...
Version 0.36.2 adds fallback from absent/empty/`0` attributes to `type:-` mappings and keeps unknown no-attribute operations as `unknown` / `не определён`. Shipment state stores barcode-based tracking data only; Otpravka `result-id` is not saved or shown in the metabox.
