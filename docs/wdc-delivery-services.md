...
As of 0.62.0, DPD also has a `DPD Расчет` tab. It stores sender/default parcel settings and provides a `getServiceCostByParcels3` test calculator that uses the same terminalCode-aware runtime service as checkout. Results are visible once after redirect with normalized service code/name/cost/period fields and optional raw debug output. The calculator does not write delivery rate tables and does not add shipment actions.
...
DPD checkout rates reuse `calculator2/getServiceCostByParcels3`, receiver `dpd_city_id` from `wdc_location_delivery_codes` through `DpdCityResolver`, sender city ID from tariff settings/override, DPD parcel-builder packaging places, declared value, active local `parcel_shop` terminalCode values, and the common delivery-service post-processing for rounding, minimum price and rules. Returned DPD service options are grouped into one checkout method per delivery type with `tariff_variants`. DPD pickup points / self-delivery terminals are imported into the local `wdc_dpd_pickup_points` table from the DPD `DPD ПВЗ` tab, and DPD pickup checkout uses the shared pickup UI to require and save a selected local `terminal_code`. Pickup pricing sends sender and receiver terminalCode; courier pricing sends sender terminalCode only. DPD order creation, statuses, labels, COD/НПП, `unitLoad` and fiscal receipts are not implemented.
