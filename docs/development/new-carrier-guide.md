...
PEK is the first provider on this contract. Its registry wiring is used only by the PEK admin destination diagnostic in version 0.131.1, and the PEK provider currently requires canonical `location_id` even though the generic query contract can represent fallback-only searches for future providers. PEK checkout rates, public pickup REST, terminal selection in session, and Shipment Framework integration are intentionally not implemented yet.
