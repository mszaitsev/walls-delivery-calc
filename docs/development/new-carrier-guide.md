...
For a carrier-owned catalog, keep API parsing, normalized persistence, background jobs and their lock inside the carrier namespace. Publish a staged catalog only after full completion; future checkout code must consume its active local snapshot rather than a carrier API response.
