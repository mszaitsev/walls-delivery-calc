...
PEK is the first provider on this contract. Its registry wiring is used only by the PEK admin destination diagnostic in version 0.131.3, and the PEK provider currently requires canonical `location_id` even though the generic query contract can represent fallback-only searches for future providers. PEK resolves terminal searches from carrier-owned mappings, treats partial/invalid canonical coordinates as address fallback, validates typed zone and `/branches/nearestdepartments/` collections before use, keeps API contract failures out of persisted unsupported mappings, rejects malformed terminal IDs/text/coordinates/limits instead of treating them as absent, validates terminal-cache payloads, and leaves physical city empty unless PEK provides a true physical city field. PEK checkout rates, public pickup REST, terminal selection in session, and Shipment Framework integration are intentionally not implemented yet.
