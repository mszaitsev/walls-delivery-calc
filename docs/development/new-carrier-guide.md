...
PEK is the first provider on this contract. Its registry wiring is used only by the PEK admin destination diagnostic in version 0.132.4, and the PEK provider currently requires canonical `location_id` even though the generic query contract can represent fallback-only searches for future providers. PEK resolves terminal searches from carrier-owned mappings, treats partial/invalid canonical coordinates as address fallback, always sends a non-empty address to `/branches/nearestdepartments/`, adds decimal-string coordinates when available, fingerprints the full outgoing terminal payload, validates typed zone and `/branches/nearestdepartments/` collections before use, keeps API contract failures out of persisted unsupported mappings, rejects malformed terminal IDs/text/coordinates/limits/schedules instead of treating them as absent, validates terminal-cache payloads through format `2` safe projection, clears old destination diagnostic reports before explicit reruns, renders terminal-search failures with safe stage/endpoint/status/response-shape/rejection counters, renders PEK logical/API detail as a separate redacted `api_error_message`, renders normalized field-level `field_errors` from `error.fields`, logs failed explicit admin diagnostics through the project logger without raw responses, raw error objects, rejected field values, request payloads, or credentials, and leaves physical city empty unless PEK provides a true physical city field. PEK mapping fingerprints use an internal contract revision rather than plugin version; persisted rows are revalidated before cache hit or stale fallback, so old address mappings with `warehousePoint` coordinates or missing `mainWarehouseId` are refreshed lazily instead of migrated destructively. PEK schema recovery is handled only by the controlled migration/install lifecycle: migration `0050` checks physical PEK table presence, creates only missing foundation tables through carrier-owned installers, verifies postconditions, and never performs API calls or runtime DDL. The mapping table avoids MySQL reserved identifiers by storing physical `mapping_precision`; repository code keeps the carrier domain key as `precision`, and migration `0051` backfills from any legacy physical `` `precision` `` column without destructive cleanup. PEK checkout rates, public pickup REST, terminal selection in session, and Shipment Framework integration are intentionally not implemented yet.

For a future PEK runtime carrier, reuse the 0.132.4 quote foundation instead of rebuilding calculator payloads in checkout. `PekQuoteService` accepts canonical `QuoteRequest` plus PEK-specific `PekQuoteOptions` and returns `PekQuoteResult`; it does not create `DeliveryRate`. The quote parser owns calculator-specific root errors, carries safe response metadata into successful and failed results, requires transfer `hasError` as a Boolean, requires service text fields as strings, and preserves Boolean `insuranceTerm` in service diagnostics. `PekQuoteCargoBuilder` applies the current documented light-cargo sealing rule using product weight before store packaging: positive `Package::weight_g < 3000` g requests one seal with `sealingPositionsCount=1` while explicitly keeping `isHP=false`; `Package::total_weight_g` remains the transport calculator weight. `isHP` means protective transport packaging, not a small-bag selector, and a documented calculator bag parameter is still unconfirmed. Sensitive carrier-provided messages and arbitrary field-error names are redacted by the quote service boundary with `PekQuoteMessageSanitizer`; canonical field paths remain visible, raw/original field names are not stored, and logs keep only sanitized field names/counts. The future `PekCarrier` stage will be responsible for converting pickup/courier results into checkout rates, quote caching, pickup selection/session validation, rules, and public provider-backed pickup REST.
