...
0.155.0 adds a reference pattern for checkout-only/non-shipment built-in services. Use this pattern only when the method is a real delivery choice but has no carrier API, selectable pickup provider, shipment creation, tracking, documents, cancellation, autosync, or shipment storage. The runtime carrier may quote a canonical `DeliveryRate` and provide server-owned presentation metadata such as `fixed_pickup_point_snapshot`; order/admin presentation may use the generic `non_shipment_state` capability with all action flags false. Do not use `manual` for this case unless the method really wants manual shipment attach/local remove semantics.
...
Version history before 0.155.0:
...
Manual delivery in 0.153.0 is the reference for a local-only Shipment Framework participant: one carrier-level adapter may support manual attach, local remove, and shared manual actual cost while keeping preview/create, carrier cancel, status API, autosync, documents, and returns unsupported. Preserve concrete order `service_key` and historical service title in the shipment payload, and do not add a fake persistence mapper or API-create path when `OrderShipmentRepository` plus a carrier-owned local service is sufficient.

For pickup carriers, always expose `carrier_key`, `service_key`, `delivery_type=pickup`, and `requires_pickup_point=true` in rate metadata. If a carrier exposes multiple pickup services, the generic resolver derives `<carrier_key>:<service_key>:pickup` when no explicit `pickup_family` is present; single-service carriers keep `<carrier_key>:pickup`.

If a registry-backed pickup provider needs checkout repricing after point selection, set server-owned `requires_rate_refresh=true` in the resolved point metadata. Omit the flag only when preserving legacy default refresh behavior is acceptable; set `false` explicitly for fulfillment-only selections like current manual pickup.

For registry-backed pickup providers, use the shared `CheckoutLocationFingerprint` owner for checkout destination fingerprints. A selected pickup point must compare against the same destination identity during quote, save, checkout refresh, validation, and reload; carrier-owned ad hoc fingerprints make valid selections look stale.

For registry-backed pickup providers, resolve provider context from the current WooCommerce shipping rate whenever WooCommerce is already rendering/caching that rate. A duplicate WDC session snapshot can support diagnostics and fallback, but must not be the only source for `service_key`, pickup family, or provider query metadata.

Registry-backed pickup providers should accept the shared `CarrierPickupPointQuery` locator contract: a destination can be identified by positive `location_id`, canonical textual locality, fallback address, or valid coordinates. Do not require a permanent location row ID when the carrier/provider can use durable textual locality, and do not impose a generic single-place cargo limit unless the provider itself owns that constraint.

For pickup-capable carriers that can expose multiple DeliveryService rows under one carrier key, use the generic pickup provider query `service_key` to isolate point ownership. Manual delivery uses this model: one `manual` carrier, one `ManualPickupPointProvider`, per-service pickup points owned by `service_id`, and server-resolved selection by stable point `code`. Do not create provider instances per service, manual-specific REST routes, manual frontend pickup flows, or manual order pickup meta when the existing Pickup Framework can carry the rate metadata, session selection, checkout validation, and order snapshot.

For manual delivery, do not create a PHP carrier per admin service. Manual services are DeliveryService rows under the one technical `manual` carrier. Carrier-specific manual geography belongs in `src/Carriers/Manual`: regions are stored as textual `country_code + region_name`, city restrictions as textual `country_code + location_name + region_name`, and the runtime receives destination authority only from server-built `QuoteRequest` metadata. Carrier-neutral invalidation for product weight/dimension updates belongs to the shared delivery cache manager, not to manual runtime code. Keep tariff zones and shipment lifecycle work out of the carrier guide until those stages exist.

Do not add a new PHP carrier adapter for each administrator-created manual delivery option. Custom flat-price services belong in the Delivery Services admin UI and are stored as `carrier_key=manual`, `service_type=manual`, with a unique `service_key`; the existing generic manual runtime carrier quotes them through the standard checkout pipeline. Add a dedicated carrier adapter only for a real external/runtime carrier integration with its own API, credentials, geography, or shipment behavior.

For a carrier-owned catalog, keep API parsing, normalized persistence, background jobs and their lock inside the carrier namespace. Publish a staged catalog only after full completion; checkout providers must consume its active local snapshot rather than a carrier API response. Provider map DTOs must contain only presentation-safe fields, while `resolve_selection()` rereads the active row by stable carrier point code and does not trust browser title, address or coordinates.
