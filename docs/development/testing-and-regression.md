...
PEK quote foundation is mandatory as `pek.quote-foundation` in group `pek` and runs with `php tests/pek/run-pek-quote-foundation-smoke.php`. It verifies typed `POST /calculator/calculateprice/`, `currencyCode=643`, `types=[3]`, pickup and courier payloads, configured sender warehouse, mandatory insurance from `Package::declared_value`, counterpart/client-card fields, one aggregate cargo place, upward weight/dimension/volume rounding, strict LTL `transfers[].type=3` parsing, cost-to-kopecks normalization, delivery-time validation, safe services, logical field errors, RU-only fail-closed behavior, deprecated-field absence, and fake HTTP only.
