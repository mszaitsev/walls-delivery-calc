...
Build `dist/walls-delivery-calc-1.0.26.zip`, inspect its one-folder layout, lint/check the extracted runtime files, and install that ZIP through the standard WordPress upload UI. With `WP_DEBUG` and `WP_DEBUG_LOG` enabled, exercise activation, WDC admin pages, checkout location/rates, pickup map/selection, and recalculation. Any WDC notice, warning, deprecation, or fatal is a release blocker.

Order-admin acceptance also runs `tests/orders/run-order-editability-settings-smoke.php` and `tests/orders/run-order-delivery-clear-smoke.php`. They protect the opt-in editability filter, server-authoritative shipment guard, explicit calculation-meta allowlist, unchanged WooCommerce shipping item, and canonical empty calculator/unsupported-shipment state before recalculation restores delivery data.
...
Yandex location-mapping and order-recalculation releases also run the corresponding focused PHP smokes. They protect geo-id-scoped manual overrides when the truthful Yandex source region is empty, independent 20-item pagination of the unique unresolved and active-override queues, page clamping after mutations, atomic one-geo live replacement with rollback and pipeline deferral, and ensure selected canonical WDC state/city values win over carrier pickup presentation fields when an administrator saves recalculated delivery.

Ozon pickup releases run all `run-ozon-delivery-pickup-*-smoke.php` entrypoints plus `node tests/ozon-delivery/run-ozon-delivery-pickup-browser-smoke.js` and `node tests/checkout/run-pickup-map-lifecycle-smoke.js`. The worker smoke covers the 18-second/10-unit/80%-memory slice, deterministic budget stops, retry termination, owner/cancellation races, exactly-one continuation, and bootstrap self-heal. The query-count smoke proves 250-row discovery chunks and constant-write 100-ID enrichment with rollback/row-count integrity. The progressive-map smoke exercises synthetic 2,000/5,000/10,000 active-generation datasets, exact eligible totals, spatial-first ordering, deterministic gap-free keyset continuation, cargo/radius parity, and generation invalidation. The shared browser lifecycle smoke protects viewport-only/100-card sidebar projection, coordinate-less exclusion, address-search-only behavior, unique batched ingestion, coalesced provider commits, selection/viewport persistence, partial retry, and the Ozon Moscow/Saint Petersburg zoom matrix. The import browser smoke protects status-only single-flight polling, terminal stop, and transport recovery.
