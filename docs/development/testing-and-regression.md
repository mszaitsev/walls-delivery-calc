...
PEK geography and destination pickup foundation are registered as mandatory `pek.geography` and `pek.pickup-provider` in group `pek`, and the generic pickup provider registry contract is registered as mandatory `framework.pickup-provider-registry` in group `framework`. Run them directly with `php tests/pek/run-pek-geography-smoke.php`, `php tests/pek/run-pek-pickup-provider-smoke.php`, and `php tests/pickup/run-carrier-pickup-provider-registry-smoke.php`. These smokes verify coordinate-first and address-fallback PEK location resolution, mapping fingerprints, fingerprint-matched stale fallback only, `warehousePoint` exclusion from destination coordinates, method-specific zone response policy, strict malformed `GeoData.Address.country_code` rejection, WordPress-timezone mapping freshness, documented destination `/branches/nearestdepartments/` collections with `departmentOperation=3` and `type=3`, canonical-coordinate-only versus address-only payloads, query coordinate-pair validation, partial/invalid canonical coordinate fallback, PEK canonical `location_id` requirement, project-unit to PEK-unit conversion including multi-place volume, free/paid terminal normalization, positive limit enforcement, format-versioned terminal cache validation including successful empty-result hits, fresh selection validation without an opt-out flag, `PickupPoint::city` semantics, SQL failure handling, no runtime PEK `dbDelta`, no public REST wiring, and fake HTTP only.
