...
PEK quote foundation is mandatory as `pek.quote-foundation` in group `pek` and runs with `php tests/pek/run-pek-quote-foundation-smoke.php`. It verifies typed `POST /calculator/calculateprice/`, calculator-owned root `hasError` handling as `pek_quote_root_error`, generic non-calculator `pek_has_error` preservation, safe response metadata propagation, `currencyCode=643`, `types=[3]`, pickup and courier payloads, configured sender warehouse, mandatory insurance from `Package::declared_value`, counterpart/client-card fields, one aggregate cargo place, PEK light-cargo threshold `<3000` g based on `Package::weight_g` without packaging weight, explicit `isHP=false` plus one seal for light cargo, total calculator weight from `Package::total_weight_g`, separated sealing/protective-packaging diagnostics, returned service breakdown without hardcoded prices, upward weight/dimension/volume rounding, strict LTL `transfers[].type=3` and Boolean `transfers[].hasError` parsing, cost-to-kopecks normalization, delivery-time validation, safe services with string-only text fields and Boolean `insuranceTerm`, redaction of actual credentials/counterpart values from API messages plus field-error names/messages, preservation of canonical field paths, duplicate sanitized-field merging, minimized logger context without API/field messages, logical field errors, RU-only fail-closed behavior, deprecated-field absence, and fake HTTP only.
