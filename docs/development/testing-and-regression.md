...
PEK checkout runtime is mandatory as `pek.checkout-runtime` in group `pek` and runs with `php tests/pek/run-pek-checkout-runtime-smoke.php`. The smoke covers the real WooCommerce rate projection shape (`DeliveryRate` to `WooCommerceRateMapper` to stored session rate), where carrier/service/family flags live on the top-level envelope and trusted PEK provider context lives in nested `rate_meta['pickup_provider_query']`. It rejects missing `rate_meta`, wrong carrier/service/family, courier rates, non-Boolean pickup requirements, empty destination fingerprints, browser-forged cargo/location/family values, and verifies request-scoped plannedDateTime memoization keeps PEK quote cache context and calculator payload aligned.
