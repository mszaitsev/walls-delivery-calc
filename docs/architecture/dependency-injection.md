...
Version 0.132.0 registers `PekQuoteCargoBuilder`, `PekQuoteRequestBuilder`, `PekQuoteResponseParser`, `PekQuoteService`, `PekQuoteDiagnosticStore`, and `PekQuoteDiagnosticService` in `Plugin.php`. `PekAdminPage` receives quote diagnostic collaborators as required dependencies and exposes only an explicit admin calculator diagnostic action. No PEK quote service is injected into checkout, public REST, rules, carrier registry, shipment registries, or shipment UI.
