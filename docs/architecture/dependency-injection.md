...
Version 0.135.6 wires PEK shipment dependencies and the generic `ShipmentCreationAttemptService` into `Plugin.php`. `ShipmentCreationService` receives the attempt service through DI and fails closed on mutation create if that safety dependency is unavailable; it uses the service to reserve server-side `creation_attempt_id` values on explicit preview/create, acquire a carrier-level create lock, persist attempts in canonical shipment/pending records, and keep lifecycle state outside carrier payload builders. PEK dependencies remain carrier-owned: adapter registry, persistence mapper list, modal extension registry, document provider registry, private access token service, sender counterpart verification, SMS release availability, declared-value resolver, sender warehouse resolver, request/cargo/recipient builders, manual attach context resolver, structured courier address resolver, shared required RU phone normalizer, status mapping/service, required status response normalizer, cancellation/button policy, document service, and PEK credentials for account-bound counterpart checks. 0.135.6 keeps status normalization in the injected `PekShipmentStatusResponseNormalizer`, while `PekShipmentAdapter` owns the controlled carrier status error boundary so malformed PEK status data does not escape to admin AJAX as WooCommerce Critical. Definite `/preregistration/submit/` rejection diagnostics remain produced by the PEK API/adapter boundary, and 0.135.5 service-object payload changes remain in the request builder with no DI owner changes.
