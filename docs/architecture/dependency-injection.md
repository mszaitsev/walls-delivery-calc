...
Version 0.134.19 wires PEK shipment dependencies into `Plugin.php`: adapter registry, persistence mapper list, modal extension registry, document provider registry, private access token service, sender counterpart verification, SMS release availability, declared-value resolver, sender warehouse resolver, request/cargo/recipient builders, manual attach context resolver, structured courier address resolver, shared required RU phone normalizer, status mapping/service, required status response normalizer, cancellation/button policy, document service, and PEK credentials for account-bound counterpart checks. Private tokens are request-local only and are never stored in options, transients, order meta, previews, or logs. Sender warehouse validation remains carrier-owned: settings defaults and shipment-local overrides both pass through the injected sender warehouse service, which normalizes PEK warehouse IDs to canonical UUIDs, uses `/branches/nearestdepartments/` with current cargo constraints as the shipment-preview authority, validates cached modal selections by a server-side constraints fingerprint, fresh-revalidates saved defaults by stored address, and returns only safe diagnostics. Courier shipment branch validation reuses the injected `PekLocationResolver` through a shipment-specific `resolve_delivery_address_for_shipment()` method that always calls `/branches/findzonebyaddress/` with the actual recipient address and does not persist into canonical PEK location mappings. Historical courier orders recover missing numeric destination identity through `LocationRepository::find_by_fias_id()` using selected-location FIAS first and generic persisted city-context FIAS second. Counterpart snapshots store only safe sender identity and PEK account login hashes, never raw login or API keys; official `legalForm=3` physical rows from the confirmed counterpart list are not dependencies for sender matching and their documents/raw values are not saved, logged, or exposed.
