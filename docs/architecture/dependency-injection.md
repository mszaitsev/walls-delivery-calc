...
Version 0.135.11 wires PEK shipment dependencies and the generic `ShipmentCreationAttemptService` into `Plugin.php`. `ShipmentCreationService` receives the attempt service through DI and fails closed on mutation create if that safety dependency is unavailable; it uses the service to reserve server-side `creation_attempt_id` values on explicit preview/create, acquire a carrier-level create lock, persist attempts in canonical shipment/pending records, and keep lifecycle state outside carrier payload builders. PEK dependencies remain carrier-owned: adapter registry, persistence mapper list, modal extension registry, document provider registry, private access token service, sender counterpart verification, SMS release availability, declared-value resolver, sender warehouse resolver, request/cargo/recipient builders, manual attach context resolver, structured courier address resolver, shared required RU phone normalizer, status mapping/service, required status response normalizer, cancellation/button policy, document service, and PEK credentials for account-bound counterpart checks. 0.135.9 keeps bounded live status sentinel compatibility in the injected `PekShipmentStatusResponseNormalizer`, while `PekShipmentAdapter` owns the controlled carrier status error boundary so malformed PEK status data does not escape to admin AJAX as WooCommerce Critical. `PekShipmentStatusService` now receives `ShipmentCreationAttemptService` as a required non-null dependency so fresh status-update `CANCELLED` evidence must terminalize the existing attempt before removing active PEK shipment storage; invalid DI fails before deletion. 0.135.11 keeps DateTime normalization inside the injected PEK normalizer: unsupported status date values fail through `PekShipmentStatusNormalizationException` with safe field/type/bounded-value diagnostics, while the adapter logs only that allowlisted context. Cancellation reconciliation remains carrier-owned in `PekShipmentService`: mutation response parsing, one-shot read-only status confirmation, and attempt terminalization only after proven cancellation do not change generic DI ownership. Definite `/preregistration/submit/` rejection diagnostics remain produced by the PEK API/adapter boundary, and service-object payload changes remain in the request builder with no DI owner changes.
