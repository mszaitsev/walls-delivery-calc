...
Version 0.135.7 keeps the PEK read-only verification results, the 0.135.3 generic lock cache fix, the 0.135.4 diagnostics, the 0.135.5 preregistration services-object fix, and the 0.135.6 first-submit status follow-up. The first live cancellation showed that `/order/cancellation/` can be applied externally while the local response projection is not recognized, and the PEK cabinet then reports `Аннулировано до приемки груза`. Cancellation now treats an unambiguous official row (`code` string plus strict boolean `success=true`) as direct success; otherwise it never retries the mutation and performs exactly one fresh read-only status reconciliation. A fresh or precheck status mapped to `cancelled` terminalizes the attempt and removes the active PEK shipment from the order inside the cancellation flow. If reconciliation is unavailable, the outcome is reported as uncertain, the shipment remains local, and the attempt stays active. The status normalizer accepts only integer `-1` and exact string `"-1"` as PEK live sentinels meaning "status ID not assigned/not meaningful"; both preserve the valid `cargoStatus` string and omit `pek_cargo_status_id`. Other negative values, padded string sentinels, zero, floats, bools and arrays remain malformed. Normal status update to `cancelled` remains a historical status update and does not silently delete the shipment or terminalize the attempt outside the cancellation workflow. PEK creation remains RU-only physical-recipient SMS release with first-name/last-name-only recipient identity, LTL `type=3`, `orderType=0`, sender self-delivery, sender-paid services, mandatory insurance, pickup/courier destination modes, safe preview only, and fail-closed SMS validation.
