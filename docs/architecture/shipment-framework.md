...
0.155.0 documents the checkout-only/non-shipment delivery pattern. A delivery method may intentionally not participate in Shipment Framework when there is no carrier shipment lifecycle to create, attach, cancel, sync, document, or persist. Such methods must not create fake shipment records. They may expose a carrier-agnostic `non_shipment_state` in saved rate metadata so the order shipments metabox can show a static informational message with all shipment actions disabled. The built-in `self_pickup` service uses this path and displays `Самовывоз покупателем`.
