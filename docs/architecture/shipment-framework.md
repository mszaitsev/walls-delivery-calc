...
Current carrier implementations cover CDEK, DPD, Russian Post, Yandex Delivery, Jet Logistic, PEK, and Ozon Delivery. PEK supports the production-verified RU automatic flow and the documented foreign manual-attach/read-only status flow. Delivery methods without a carrier shipment lifecycle, including store self-pickup, do not create fake shipment records.
