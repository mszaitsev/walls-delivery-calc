...
Version 0.135.5 keeps the PEK read-only verification results, the 0.135.3 generic lock cache fix, and the 0.135.4 diagnostics for definite PEK `/preregistration/submit/` rejection plus staged SMS release availability failures. The first controlled submit on 0.135.3 was executed once after a successful safe preview and PEK returned a deterministic logical rejection; no automatic retry happened and no shipment/pending record was created. The 0.135.4 diagnostic repeat reached `/preregistration/submit/` after sender warehouse, pickup destination, counterpart, and SMS checks succeeded; PEK returned HTTP 200 logical validation errors requiring the service objects `hardPacking`, `strapping`, `documentsReturning`, and `delivery`. The adapter is thin and delegates payload building, status/manual attach/cancellation, and documents to carrier-owned services under `src/Shipments/Pek/`; persistence stays in `PekShipmentPersistenceMapper`, document actions stay in `PekShipmentDocumentProvider`, and actual cost continues to use shared `actual_cost_*` fields. PEK creation is RU-only physical-recipient SMS release with first-name/last-name-only recipient identity, LTL `type=3`, `orderType=0`, sender self-delivery, sender-paid services, mandatory insurance, pickup/courier destination modes, safe preview only, and fail-closed SMS validation. The preregistration `services` object now always sends explicit disabled states for unordered optional services: `hardPacking.enabled=false`, `strapping.enabled=false`, `documentsReturning.enabled=false`, and pickup `delivery.enabled=false`; courier delivery remains `enabled=true` with `payer.type=1`. `hardPacking` is the PEK protective packing service and is not used for the shop light-cargo bag surcharge, no `storing` or bag aliases are added, insurance remains enabled with declared value and payer, and sealing keeps the existing light-cargo business rule. The shipment modal exposes exactly one PEK delivery scenario from the trusted order calculation and browser `delivery_type` cannot switch pickup/courier without a new checkout calculation. Sender warehouse validation uses `/branches/nearestdepartments/` as the shipment-preview authority: saved defaults are fresh-revalidated by exact canonical `warehouseId` against the stored warehouse address and current cargo constraints, while shipment-local overrides must be posted atomically as `shipment_modal_override` plus canonical UUID and must match the current server-side nearestdepartments cache and constraints fingerprint. The service sends `departmentOperation=2`, `type=3`, weight, volume, max dimension, and max place weight to PEK, accepts only exact UUID matches from `freeDepartments`, never silently substitutes another warehouse, and does not reject preview because the same ID is absent from `/branches/all/`. Sender counterpart verification treats official `legalForm=3` physical-person rows as irrelevant for the current sender contract: they are counted and skipped before legal/IP normalization, while `legalForm=1/2` rows remain strict and any failure exposes only allowlisted diagnostic stage/reason/counters. Pickup destination preparation normally fresh-resolves the selected terminal through `/branches/nearestdepartments/` with cache disabled; if that read-only call fails with access/transport/5xx, an existing order may use only the exact persisted server-validated checkout selection, with matching carrier/service/family, warehouse ID, branch ID, RU location, provider destination fingerprint, selected timestamp, source, availability, and local cargo limits. Browser terminal fields, missing branch/fingerprint/location authority, fresh HTTP 200 negative results, and checkout terminal discovery all remain fail-closed. Courier destination authority is carrier-owned: shipping persisted DaData/Woo fields win for filled shipping destinations, billing is allowed only for empty or semantically identical destination, stale DaData is rejected against current Woo fields, RU country is taken from the effective scope, typed house/block/structure/apartment/office/premise unit data survives checkout order meta and raw Woo fallback parsing, and city/settlement/federal-city normalization preserves address components without exposing raw address in preview. Location binding distinguishes city-level and settlement-level canonical rows before PEK read-only geography calls; explicit city, settlement, or selected-location FIAS mismatch is authoritative and cannot fall back to matching names. After that guard succeeds, courier branch/zone/mainWarehouse authority comes from a fresh `/branches/findzonebyaddress/` call with `ShipmentCreateRequest::recipient_address.raw_address`; exact precision succeeds, near precision adds a safe warning, bad/malformed responses fail before SMS/private calls, and canonical city coordinates are not used as final shipment branch authority. SMS release then runs read-only geography/no-calc-services, private-token, and connected-services checks in order; any failure returns a closed `sms_diagnostic` with stage, safe endpoint/method/status, normalized error code/message, field errors, response shape, and stage booleans, while technical/contract failures keep the existing broad public message and business-unavailable cases use a distinct public text. Pre-SMS PEK API failures return `preparation_diagnostic` with a closed stage such as `destination_pickup`; SMS failures return `sms_diagnostic`. The shipment modal renders these diagnostics in additional PEK technical blocks and does not expose token, credentials, raw response, or PII. Checkout pickup keeps the same fail-closed availability semantics but preserves PEK terminal provider root-cause diagnostics when `/branches/nearestdepartments/` access/transport fails; courier pricing remains independent. The shipment modal displays the server-built ShipmentCreateRequest destination, including billing fallback or selected pickup point, without PEK API calls. Status updates pass through a required typed normalizer: optional fields are merged only when present, PEK DateTime values use explicit round-trip formats, one requested cargo must produce exactly one response row, basic fallback is limited to explicit 403/404 and does not erase expanded-only receiver/cost data, and malformed expanded HTTP 200 responses do not persist. Pending uncertain submissions expose manual attach and local remove through the same generic capability payload, with server-side PEK context restoration before reconciliation.
