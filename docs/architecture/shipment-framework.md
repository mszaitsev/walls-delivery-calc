...
Version 0.136.0 keeps the PEK read-only verification results, the 0.135.3 generic lock cache fix, the 0.135.4 diagnostics, the 0.135.5 preregistration services-object fix, the 0.135.7 cancellation reconciliation flow, the 0.135.8 cancelled-status integer `-3` compatibility, and the 0.135.9 CANCELLED terminal cleanup. The cancelled live shipment returned `cargoStatus="Аннулировано до приемки груза"` with integer `cargoStatusId=-3`; official PEK docs document the status text and Number type but do not document negative sentinel IDs. The status normalizer therefore accepts integer `-3` only when the same response carries that exact cancelled-before-acceptance status after bounded text normalization. This is not general negative-ID support: `-2`, `-4`, `"-2"`, `"-3"`, padded sentinels, zero, floats, bools and arrays remain malformed. Accepted live sentinels (`-1`, `"-1"`, and cancelled-status `-3`) preserve the valid `cargoStatus` string and omit/clear `pek_cargo_status_id`. 0.135.9 changes status-update reconciliation for `CANCELLED` only: fresh carrier status that is cancelled by immutable PEK raw-status truth terminalizes the generic creation attempt and removes the active PEK shipment from order storage, matching the already-confirmed cancellation flow. 0.135.10 makes the attempt lifecycle collaborator required, so terminalization failure prevents deletion instead of orphaning an active attempt. This is backend canonical cleanup, not frontend hiding, and it does not add delivered/returned archival policy. Cancellation itself remains one-shot: ambiguous mutation responses are reconciled through exactly one fresh read-only status call and are never retried automatically. PEK creation remains RU-only physical-recipient SMS release with first-name/last-name-only recipient identity, LTL `type=3`, `orderType=0`, sender self-delivery, sender-paid services, mandatory insurance, pickup/courier destination modes, safe preview only, and fail-closed SMS validation. First controlled courier create evidence showed an 8800 g shipment with `sealing=false` passed preparation but PEK rejected missing `cargos[0].services.sealing`; 0.135.10 therefore always serializes `services.sealing`, using `enabled=false` without payer for non-sealed cargo. 0.135.11 adds exact `deliveryPlanDate` millisecond compatibility after production `/cargos/status/` returned `2026-08-19T22:23:16.833`: only `deliveryPlanDate` accepts strict `Y-m-d\TH:i:s.v`, the source string is preserved, and other status DateTime fields are not broadened without evidence. 0.136.0 makes PEK raw-status to universal-status mapping administrator-configurable with separate pickup/courier values; this is presentation/business mapping only and is intentionally separated from immutable PEK facts such as cancelled-before-acceptance and pre-acceptance cancellation eligibility.

Version: 0.155.7

0.155.7 does not change Shipment Framework contracts. The new platform runtime switch gates Shipment Framework order-admin registration around the framework boundary in `Plugin.php`: order shipment metaboxes, shipment AJAX controllers, document downloads, status autosync, and order-related shipment analytics hooks are not registered while WooCommerce runtime is disabled. `ShipmentStatusAutoSyncService` also checks the platform runtime contract at execution time to protect already queued order-related callbacks. Carrier adapters, persistence mappers, modal extensions, document providers, lifecycle contracts, and shipment payload semantics are unchanged.

0.155.6 does not change Shipment Framework architecture. The patch is limited to Rule Engine condition labels and a new full-cart condition backed by existing checkout context.

0.155.5 does not change Shipment Framework architecture. The patch is limited to Rule Engine checkout price bases and typed cart-total context; shipment adapters, mappers, lifecycle, documents, autosync, and metabox contracts are unchanged.

0.155.4 does not change Shipment Framework architecture. The patch is limited to WooCommerce checkout rate-key ownership and selected-method preservation; shipment adapters, mappers, lifecycle, documents, autosync, and metabox contracts are unchanged.

0.155.3 does not change Shipment Framework architecture. The patch is limited to WooCommerce checkout selected-method arbitration and uses current package shipping rates as authority.

0.155.2 does not change Shipment Framework architecture. Checkout selection reconciliation lives entirely in the WooCommerce checkout/session layer and preserves only WDC shipping method identity after fresh rate calculation; shipment adapters, mappers, lifecycle, documents, autosync, and metabox contracts are unchanged.

0.155.1 refines the checkout-only/non-shipment delivery pattern. A delivery method may intentionally not participate in Shipment Framework when there is no carrier shipment lifecycle to create, attach, cancel, sync, document, or persist. Such methods must not create fake shipment records. They may expose a carrier-agnostic `non_shipment_state` in saved rate metadata with suppression flags; the order shipments metabox keeps the service identity and suppresses shipment status/body/actions/documents/actual-cost UI. The built-in `self_pickup` service uses this path.
...
0.153.0 adds manual delivery services as a Shipment Framework participant without an API lifecycle. The manual capability matrix is: Manual attach yes; Local remove yes; Manual actual cost yes; Preview/create no; Carrier cancel no; Status API no; Autosync no; Documents no. The adapter is carrier-level (`manual`) while each saved shipment payload preserves the concrete order `service_key` and historical service title.
...
Jet Logistic participates in the framework without API shipment creation. The adapter supports presentation, manual attach through the shared AJAX endpoint, status payload/update, local remove, tracking identifier, and shared autosync. `preview/create` and carrier cancellation return unsupported results. Jet persistence for manual attach/status/local remove is owned by its shipment service, not by generic creation flow or a fake persistence mapper. Jet status diagnostics read the existing tracking API for a manually entered cargo number, show at most five compact events with the matched status rule, and do not mutate orders or shipment storage.
