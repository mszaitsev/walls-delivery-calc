...
Version 0.135.6 keeps the PEK read-only verification results, the 0.135.3 generic lock cache fix, the 0.135.4 diagnostics, and the 0.135.5 preregistration services-object fix. The first successful production PEK submit confirmed that the shipment exists in the PEK cabinet and document/label downloads work. The follow-up status call returned a live freshly-created sentinel `cargoStatusId=-1`; the normalizer treats only that exact integer as "status ID not assigned yet", preserves the valid `cargoStatus` string and other fields, and omits `pek_cargo_status_id` instead of failing the whole response. Other negative IDs, floats, bools, arrays, and string `"-1"` remain malformed. Adapter status updates catch carrier/domain normalization failures and return controlled PEK status errors so a malformed external status response does not become a WooCommerce Critical. Successful create also supersedes prior deterministic create-error presentation for the same order/carrier/service/attempt, and the admin create flow avoids duplicate create/status success toasts. The adapter is thin and delegates payload building, status/manual attach/cancellation, and documents to carrier-owned services under `src/Shipments/Pek/`; persistence stays in `PekShipmentPersistenceMapper`, document actions stay in `PekShipmentDocumentProvider`, and actual cost continues to use shared `actual_cost_*` fields. PEK creation remains RU-only physical-recipient SMS release with first-name/last-name-only recipient identity, LTL `type=3`, `orderType=0`, sender self-delivery, sender-paid services, mandatory insurance, pickup/courier destination modes, safe preview only, and fail-closed SMS validation. The preregistration `services` object sends explicit disabled states for unordered optional services: `hardPacking.enabled=false`, `strapping.enabled=false`, `documentsReturning.enabled=false`, and pickup `delivery.enabled=false`; courier delivery remains `enabled=true` with `payer.type=1`. `hardPacking` is the PEK protective packing service and is not used for the shop light-cargo bag surcharge, no `storing` or bag aliases are added, insurance remains enabled with declared value and payer, and sealing keeps the product-weight light-cargo rule shared with checkout surcharge eligibility. Safe preview reports `planned_date_sent=false`; adding `common.plannedDate` is deferred until a separate reviewed sender-warehouse timezone/schedule/cutoff policy exists. Receiver phone remains one documented international `+7` `personPhones` entry; the observed duplicate-looking PEK cabinet phone display is treated as residual server-side presentation evidence, not a plugin payload change.
