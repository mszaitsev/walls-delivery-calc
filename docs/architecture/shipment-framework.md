...
PEK creation correlation is stable for an uncertain pending attempt and is preserved through manual reconciliation. The generic framework still does not own a fresh attempt identity after successful cancellation or local removal; read-only safe preview is not blocked by this, but production submit remains unapproved until that carrier-agnostic policy is reviewed or explicitly accepted.
