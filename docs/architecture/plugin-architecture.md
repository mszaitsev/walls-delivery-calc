...
PEK follows the carrier-owned boundary and is now a checkout runtime carrier in version 0.133.7. `Plugin.php` wires `PekSettings`, `PekCredentials`, PEK HTTP/API transport, request budget, sender warehouse cache, diagnostics, warehouse lookup/validation, quote components, checkout context resolver, plannedDateTime resolver, pickup formatter, and `PekCarrier`. `PekCarrier` is registered only in `CarrierRegistry`; PEK is still absent from Shipment Framework registries. `CheckoutOrchestrator` remains carrier-agnostic and consumes only the optional `CarrierQuoteCacheContextProviderInterface` for PEK cache context. Public pickup REST integrates registry-backed providers through a trusted server-stored rate snapshot, not browser cargo/location authority. It uses the shared `WooCommerceSessionBootstrapper` before reading session-backed rate context so a standalone REST request loads the same WooCommerce customer session created by checkout. The resolver reads production WooCommerce session rates as a top-level envelope plus nested `rate_meta`, where `pickup_provider_query` lives; PEK registry-backed rates require `service_key=pek`, stale or empty provider destination fingerprints fail closed, and coordinates are valid only as a complete bounded pair or address-only `null/null`. Saving a PEK terminal fresh-validates the point code through `resolve_selection()` before session persistence, preserves PEK's `provider_destination_fingerprint` separately from the generic checkout `destination_fingerprint`, and checkout POST cannot reconstruct a PEK selection from hidden fields. Provider validation does not guarantee calculator eligibility: if selected-terminal quote fails, `PekCarrier` does not mutate WooCommerce session directly, but returns a recovered preliminary rate with generic `pickup_selection_rejected` metadata for the current render; the generic WooCommerce shipping layer clears only the indicated family, preserves the recovered method choice, renders an inline message inside the affected pickup method, and strips that transient metadata before session/order/calculation persistence. PEK pickup presentation is source-driven: free points use `Собственный пункт выдачи ПЭК`, paid points use `Партнерский пункт выдачи ПЭК` plus a possible surcharge warning, and internal warehouse UUIDs are kept as technical point codes rather than public titles.
