...
Version 0.134.18 registers PEK Shipment Framework classes only through the composition root: `PekShipmentAdapter`, `PekShipmentPersistenceMapper`, `PekShipmentModalExtension`, `PekShipmentDocumentProvider`, request/cargo/recipient builders, private-token and SMS availability services, sender warehouse and declared-value resolvers, manual attach context resolver, structured courier address resolver, shared RU phone normalizer, status mapping/service, button policy, and document service. Generic shipment creation, metabox rendering, document download, status autosync, actual-cost storage, and lifecycle AJAX remain carrier-agnostic; the PEK sender warehouse picker uses the generic carrier hook rather than a PEK branch in generic JS.
