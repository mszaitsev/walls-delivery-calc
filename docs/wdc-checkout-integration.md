...
As of version 0.18.3, rule context carries selected location FIAS ID when available. City conditions compare by FIAS ID first. Package dimensions are aggregated from WooCommerce product dimensions using max length, width, and height across cart items, while volume remains total package volume in `cm3` and is converted to cubic meters by the rule evaluator.
