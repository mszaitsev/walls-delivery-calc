...
## Order Delivery Recalculation 0.41.12
...
- `src/Orders/Application/OrderDeliveryAddressNormalizationService.php` is the admin thin wrapper for delivery-address normalization/geocoding. It reuses `CheckoutAddressRuntime` when available; courier save requires a normalized payload, and pickup map manual address search uses the same wrapper to place a temporary address marker from DaData coordinates.
...
- `src/Orders/Admin/OrderDeliveryRecalculationAdminController.php` registers AJAX actions for preview, settlement search, pickup point search, address normalization, address geocoding and save. Preview/pickup search remain available for calculation, while save blockers are enforced by the replacement service.
