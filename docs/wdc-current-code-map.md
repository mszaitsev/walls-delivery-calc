...
## Order Delivery Recalculation 0.41.9
...
- `src/Orders/Admin/OrderDeliveryMetabox.php` shows `Пересчитать доставку` in `Калькулятор доставок`, renders the hidden modal markup plus current-settlement selector shell, and provides the save button that starts disabled until JS state is valid.
- `assets/admin/order-delivery-recalculation.js` and `.css` provide the order-admin modal interaction. The JS opens/closes the modal, avoids duplicate preview requests, searches settlements, stores selected location/rate/pickup point/normalized address in modal memory, loads preview HTML, opens the map-backed pickup picker, normalizes manager-entered pickup delivery address, enables save only for valid state, posts save payload, and reloads the page after success.
- `tests/orders/run-order-delivery-recalculation-smoke.php` covers modal metabox markup, order-to-quote mapping, location search, location override preview, all-rates preview, Russian Post pickup/courier groups, pickup endpoint payload/security, save blockers, shipping item create/replace, pickup address rules, WDC meta rewrite, totals, private notes, JS save/normalize hooks, and no mutation during preview/pickup search.
