...
## Order Delivery Recalculation 0.41.11
...
- `src/Orders/Application/OrderDeliveryRecalculationService.php` owns preview recalculation. Preview is not blocked by shipping item count or shipment state; it calls `CheckoutOrchestrator`, normalizes available rates for admin rendering, and returns the destination label used for the preview without mutating the order.
- `src/Orders/Application/OrderDeliveryAddressNormalizationService.php` is the admin thin wrapper for delivery-address normalization. It reuses `CheckoutAddressRuntime` when available; pickup save no longer requires this payload.
- `src/Orders/Application/OrderDeliveryReplacementService.php` owns save/replacement. It blocks saves with multiple shipping items or registered shipment markers, creates a missing shipping item or replaces the single existing one, rewrites WDC platform/calculation/pickup meta, updates shipping address, recalculates totals, adds a private order note, and saves the order. Pickup save requires a pickup point but not a normalized manager address.
- `src/Orders/Admin/OrderDeliveryRecalculationAdminController.php` registers AJAX actions for preview, settlement search, pickup point search, address normalization and save. Preview/pickup search remain available for calculation, while save blockers are enforced by the replacement service.
...
- `src/Orders/Admin/OrderDeliveryMetabox.php` shows `Пересчитать доставку` in `Калькулятор доставок`, renders the hidden modal markup plus current-settlement selector shell, current pickup JSON payload, and the save button that starts disabled until JS state is valid.
- `assets/admin/order-delivery-recalculation.js` and `.css` provide the order-admin modal interaction. The JS opens/closes the modal, avoids duplicate preview requests, searches settlements, stores selected location/rate/pickup point/normalized courier address in modal memory, preselects the current order pickup point when location is unchanged, loads preview HTML, opens the map-backed pickup picker, syncs active marker/list state, shows a temporary search marker for manual address/postcode matches, enables save for pickup once rate+pickup are selected, posts save payload, and reloads the page after success.
- `tests/orders/run-order-delivery-recalculation-smoke.php` covers modal metabox/current pickup markup, order-to-quote mapping, location search, location override preview, all-rates preview, Russian Post pickup/courier groups, pickup endpoint payload/security, save blockers, shipping item create/replace, pickup save without normalized address, WDC meta rewrite, totals, private notes, JS save/prefill/map-sync hooks, viewport scroll CSS, and no mutation during preview/pickup search.
