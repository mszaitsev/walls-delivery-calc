...
# Yandex shipment registration foundation (0.106.1)

- `src/Shipments/Allocation/ShipmentAllocation*.php` is the neutral read-model for a prepared shipment allocation: stable place number/dimensions/weight and per-place source item identity/quantity/unit price/assessed unit price.
- `src/Shipments/Cdek/CdekShipmentAllocationAdapter.php` adapts the established CDEK `meta['cdek_item_rows']` (`item_key`, `place_number`, `amount`) plus `ShipmentPlace`; it validates the source and throws `InvalidArgumentException` for broken allocation instead of silently remapping rows. It does not change CDEK creation payloads.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentPayloadBuilder.php` builds a transport-free offers/create payload. Yandex item billing fields live under `items[].billing_details`; ready intervals are formatted as UTC `yyyy-MM-ddTHH:mm:ss.ffffffZ`. It must remain free of HTTP, persistence and UI wiring.
