## Shipment lifecycle and admin JS modules 0.118.0
- Since 0.118.0 DPD two-stage registration is exposed to common UI through `ShipmentLifecycleResult`, not DPD-specific JS wrappers. `CarrierShipmentLifecycleContinuationInterface` is an optional adapter capability; the shared `wdc_continue_shipment_lifecycle` endpoint resolves order/carrier/attempt context and delegates the carrier submit internally.
