...
- `src/Carriers/Cdek` contains the CDEK foundation, tariff calculation, tariff-management and order-registration support: settings, separate encrypted test/production credentials, active-environment API base URL selection, OAuth token service/cache, API response/exception objects, WP HTTP adapter, destination city resolver, API client methods for `tarifflist`, `alltariffs`, locations, delivery points, `POST /v2/orders`, `GET /v2/orders` and `GET /v2/orders/{uuid}`, plus the managed tariff repository/sync service. Print form downloads, cancellation and webhooks are still not implemented.
- `src/Shipments/Cdek` contains the CDEK shipment adapter, request builder and order status service. It builds `POST /v2/orders` from the carrier-aware `Отправления` modal, stores `registration_pending`, and checks async registration status by UUID/CDEK number/IM number.
