...
# Yandex shipment registration foundation (0.107.0)
...
- `src/Carriers/YandexDelivery/Api/YandexDeliveryApiClient.php` uses the existing `YandexDeliveryHttpClientInterface` / `WpYandexDeliveryHttpClient` transport and now exposes shipment endpoints: `offers/create`, `offers/confirm`, `request/info`, `request/history` and `request/cancel`.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentClient.php` is the typed shipment HTTP facade. It returns DTOs instead of raw arrays and performs no retry.
...
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentRegistrationService.php` orchestrates `payload builder -> offers/create -> selector -> offers/confirm -> request/info`. It does not call `request/create` and is not wired to UI, checkout, order save or persistence.
