...
- `src/Shipments/Presentation/ShipmentActualCostComparisonService.php` owns carrier-neutral actual-cost presentation for CDEK, DPD, Russian Post and Yandex. Carrier services still provide their own actual cost source and the shared `ShipmentBaseApiCostResolver` reads saved Base API cost; comparison uses integer kopecks and exact +3% cross multiplication.
