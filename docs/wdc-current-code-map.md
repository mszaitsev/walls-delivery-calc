...
- `src/Shipments/Cdek/CdekShipmentAllocationAdapter.php` adapts the established CDEK `meta['cdek_item_rows']` (`item_key`, `place_number`, `amount`) plus `ShipmentPlace`; it validates the source and throws `InvalidArgumentException` for broken allocation, empty rows or places without rows instead of silently remapping rows. It does not change CDEK creation payloads.
