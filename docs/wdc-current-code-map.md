...
- 0.51.1 CDEK label/status admin note: `src/Shipments/Admin/OrderShipmentsMetabox.php` renders `Скачать этикетку` / `Открыть этикетку` for registered CDEK shipments, while `assets/admin/shipments-admin.js` and `assets/admin/shipments-admin.css` add a temporary busy spinner for download clicks. `src/DeliveryServices/Admin/DeliveryServicesAdminPage.php` now owns the `Статусы СДЭК` service tab and saves `CdekStatusMappingService::MAPPING_KEY`; the general `ShipmentStatusesAdminPage` no longer renders CDEK mapping. `src/Shipments/Cdek/CdekStatusMappingService.php` contains labels/default mappings for all CDEK order status codes from Appendix 1.
...
- `src/DeliveryServices/Admin/DeliveryServicesAdminPage.php` renders the CDEK `Тарифы` tab, sync preview/confirmation, inline tariff editing, active flags, compact restriction display, Russian delivery-type labels `до ПВЗ` / `до двери`, and editable `Режим тарифа`.
- `src/Carriers/Cdek/CdekSettings.php` stores the CDEK `shipment_point` setting with default `NSK69`, optional `cdek_shipment_point_address`, and `cdek_sender_address` for door-origin `from_location`; the admin CDEK calculation tab renders them near sender city settings.
...
- `src/Shipments/Admin/ShipmentStatusesAdminPage.php` renders `WDC -> Статусы` with main autosync settings, universal shipment status to WooCommerce order status mapping, CDEK raw-status mapping, diagnostics, and the manual run action.
