...
## Order Delivery Recalculation 0.41.24
...
- `src/Orders/Application/OrderDeliveryAddressNormalizationService.php` is the admin thin wrapper for delivery-address normalization/geocoding and courier address suggestions. Courier address suggestions now reuse the shared checkout `AddressSuggestionService`, `AddressSuggestionNormalizer` and `AddressLineParser`, so street, house, flat/room/premise lookup and house-level finalization match checkout. Pickup map manual address search still uses the same wrapper to place a temporary address marker from DaData coordinates.
...
- `src/Orders/Admin/OrderDeliveryRecalculationAdminController.php` registers AJAX actions for preview, settlement search, pickup point search, courier address suggestions, address normalization, address geocoding and save. Preview/pickup search remain available for calculation, while save blockers are enforced by the replacement service.
