...
# Yandex shipment registration foundation (0.106.0)

- `src/Shipments/Allocation/ShipmentAllocation*.php` is the neutral read-model for a prepared shipment allocation: stable place number/dimensions/weight and per-place source item identity/quantity.
- `src/Shipments/Cdek/CdekShipmentAllocationAdapter.php` adapts the established CDEK `meta['cdek_item_rows']` (`item_key`, `place_number`, `amount`) plus `ShipmentPlace`; it does not change CDEK creation payloads.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentPayloadBuilder.php` builds a transport-free offers/create payload. It must remain free of HTTP, persistence and UI wiring.
