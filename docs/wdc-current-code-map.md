...
- Since 0.117.2 `shipment-polling.js` and `shipment-status.js` are carrier-neutral too. DPD owns `submitDpdRegistration()` / `startDpdRegistrationPolling()` and its places row, Yandex owns cancellation polling toast/raw-status presentation and self-pickup code rendering, and CDEK owns the default registration polling wrapper. Document visibility is driven by normalized `label_actions` and the generic document action selector.
