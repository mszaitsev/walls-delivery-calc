...
- `OrderShipmentsMetabox` renders the calculated weight as text only: `Расчётный вес товаров: %d г` for one initial place, or as a common total hint above multiple places. The hint is not an input value and is not submitted in `FormData`.
