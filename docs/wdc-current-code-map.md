...
- `src/Carriers/YandexDelivery/Geo/YandexDeliveryGeoMappingService.php` builds search strings from `LocationRepository`/`Location` models, calls `POST /api/b2b/platform/location/detect`, normalizes variants into mappings, stores raw response JSON and assigns deterministic confidence: `100.00` for locality+region match, `70.00` for locality-only match and `40.00` for multiple/weak matches.
- `src/Carriers/YandexDelivery/Api/YandexDeliveryApiClient.php` now exposes `locationDetect()` and `YandexDeliveryEndpoints.php` includes `/api/b2b/platform/location/detect`.
- `src/DeliveryServices/Admin/DeliveryServicesAdminPage.php` adds the Yandex tab `Yandex geo_id` after `ПВЗ / точки сдачи`. It shows mapping statistics, accepts `location_id` or WDC location-name search, runs `location/detect`, displays `geo_id/locality/region/confidence/status` and lets an admin mark a row primary/manual.
- `src/Core/Plugin.php` registers the Yandex geo mapping repository and service. No checkout carrier, pricing calculator, pickup selector, map, shipment flow, full Russia pickup import, cron or autosync was added in 0.75.0.
- `tests/yandex-delivery/run-yandex-delivery-geo-mapping-smoke.php` covers repository CRUD, multiple `geo_id` rows per location, primary switching, confidence, location/detect normalization, status constants, migration schema and admin wiring.
# Карта текущего кода
