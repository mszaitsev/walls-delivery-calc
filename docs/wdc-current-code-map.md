...
## Order Delivery Recalculation 0.41.8
...
- `src/Orders/Admin/OrderDeliveryRecalculationAdminController.php` registers AJAX action `wdc_order_delivery_recalculate_preview`, checks nonce/capability, loads orders through `wc_get_order()`, accepts selected location payload, and returns rendered preview HTML plus rate/request/location payloads. It also exposes a thin admin location-search AJAX wrapper over existing `CheckoutLocationAjax::payload()` and admin pickup search action `wdc_order_delivery_recalculate_pickup_search` over the existing Russian Post pickup repository. Pickup search supports `mode=location` for initial settlement-wide loading and `mode=search` for explicit administrator queries. On order screens it enqueues the existing pickup map provider assets only for the recalculation modal.
...
- `assets/admin/order-delivery-recalculation.js` and `.css` provide the order-admin modal preview interaction. The JS opens/closes the modal, avoids duplicate preview requests, searches settlements, stores selected location/rate/pickup point only in modal memory, loads preview HTML into the modal content area, keeps the initial state unselected, opens the preview-only map-backed pickup picker for pickup rates, loads all pickup points for the selected settlement on initial open, falls back to list selection when the map provider is unavailable, and clears pickup state for courier rates.
- `tests/orders/run-order-delivery-recalculation-smoke.php` covers shipment blocking, modal metabox markup, order-to-quote mapping, location search, location override preview, all-rates preview, Russian Post pickup/courier groups, pickup endpoint security/blocking/payload, and no mutation of shipping item/totals/shipping address/calculation meta.
