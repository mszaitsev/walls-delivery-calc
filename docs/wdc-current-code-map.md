...
- `src/Carriers/YandexDelivery/Pickup/YandexDeliveryPickupPointImportService.php` is manual-only. It calls `POST /api/b2b/platform/pickup-points/list` with `type=pickup_point`, downloads pages, does not reject `available_for_dropoff=false`, saves normalized rows, activates imported rows and reports `fetched`, `normalized`, `saved`, `inactive`, `errors` and `duration`. `YandexDeliveryPickupPointService.php` exposes search, statistics, lookup and active-environment sender-point validation.
...
- `src/DeliveryServices/Admin/DeliveryServicesAdminPage.php` now has `Яндекс Доставка -> Данные для входа` and `ПВЗ / точки сдачи`. The pickup tab renders active/type/dropoff/last-import stats, sender point validation, a manual import button and first search results by `platform_station_id`, city and type.
- `src/Core/Plugin.php` registers settings, replaceable HTTP client, API client, diagnostic service and Yandex pickup repository/normalizer/import/service. No Yandex checkout carrier, shipment adapter, cron, autosync, offers/create, offers/confirm, documents or status mapping are registered in 0.73.0.
- `tests/yandex-delivery/run-yandex-delivery-foundation-smoke.php` covers the 0.72 API/settings foundation. `tests/yandex-delivery/run-yandex-delivery-pickup-smoke.php` covers migration schema, normalization, save/update, unique `platform_station_id`, inactive/reactivation flow, search, counts, sender validation, import statistics and absence of checkout/shipment/cron wiring.
