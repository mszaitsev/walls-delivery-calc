...
- DPD checkout package input is built by `src/Carriers/Dpd/Tariff/DpdParcelBuilder.php`. DPD `parcel[]` means packaging places, not cart items. The current runtime builds one safe parcel from package-level dimensions/weight first, then a deterministic item-fit box within the temporary 50x50x30 cm model, then DPD defaults. Cart item composition, `unitLoad`, COD/НПП and fiscal receipts are not included.
...
- `DpdQuoteCarrier::quote_id()` includes receiver location, sender/receiver city IDs, normalized parcel signature, parcel count, total weight, dimensions, declared value, package builder source, `delivery_type`, fixed `selfPickup=true`, derived `selfDelivery`, courier-rates enablement, enabled service codes, calculation date and environment for diagnostics.
...
- `tests/dpd/run-dpd-checkout-runtime-smoke.php` covers disabled service, missing credentials, missing receiver cityId, grouped MAX/NDY mapping, enabled-code filtering, unchecked-all behavior, fixed terminal-origin pickup payload, courier-disabled no-call behavior, courier-enabled payload, split orchestrator entries, DPD parcel builder package-level/item-fit/default paths, quote_id parcel/delivery type/courier settings, missing-cost skipping, service-level minimum price post-processing through the orchestrator, DPD runtime registry presence, and shipment adapter absence.
- `docs/wdc-dpd-checkout-runtime.md` documents the 0.58.4 boundary. Pickup points, parcel shop selection/map, postamats, shipment creation, cancellation, statuses, labels, COD/НПП, `unitLoad`, fiscal receipts, complex bin packing and new global carrier branching remain intentionally out of scope.
