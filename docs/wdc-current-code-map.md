## Shipment document actions terminology 0.121.0
- Generic Shipment Framework document action code uses `document_actions()` in PHP and `documentActions` in admin JS. The historic status payload key `label_actions` is intentionally preserved as a wire compatibility alias only.
- Carrier label terminology remains only where it names a real carrier label: Yandex label download/policy, CDEK label/barcode download and carrier-specific action keys such as `download_yandex_label`.
...
- Carrier-specific runtime hooks live in `assets/admin/shipments/extensions/`: CDEK barcode/delivery-mode hooks, DPD address/contact/documents/places presentation hooks, Yandex source drop-off/label hooks, and a Russian Post extension placeholder for the currently shared behavior.
...
- Since 0.117.2 `shipment-polling.js` and `shipment-status.js` are carrier-neutral too. Since 0.118.0 DPD owns only DPD presentation hooks in its extension; lifecycle continuation/polling transport is common and reads the normalized `lifecycle` payload. Yandex owns cancellation polling toast/raw-status presentation and self-pickup code rendering, and CDEK owns the default registration polling wrapper. Document visibility is driven by normalized document actions and the generic document action selector.
- Since 0.117.3 Yandex polling hooks first confirm canonical carrier key `yandex_delivery`; a generic `cancellation` polling purpose does not identify the carrier. The structure smoke scans production script order for duplicate functions, duplicate top-level `const`/`let` declarations and function/lexical collisions.
- Since 0.117.4 the structure smoke stores every top-level lexical occurrence separately and self-tests the scanner against duplicate const/let declarations, const/let collisions, function/lexical collisions, local declarations, comments, strings and comma-separated declarations.
...
- `src/Shipments/Documents/ShipmentDocumentAction.php`, `ShipmentBinaryDocument.php`, `CarrierShipmentDocumentProviderInterface.php`, `ShipmentDocumentProviderRegistry.php` and `ShipmentDocumentDownloadService.php` define the common document/action contract and one protected `admin_post_wdc_download_shipment_document` download shell.
- `OrderShipmentsMetabox` now renders document actions from `ShipmentDocumentProviderRegistry` and builds URLs through `ShipmentDocumentDownloadService`; the old CDEK/DPD/Yandex per-carrier admin-post handlers were removed.
- Carrier-specific providers live beside carrier services: `CdekShipmentDocumentProvider`, `DpdShipmentDocumentProvider`, `YandexShipmentDocumentProvider`, and `RussianPostShipmentDocumentProvider`.
- CDEK/DPD/Yandex keep existing service-level PDF/ZIP generation and validation. Russian Post uses `RussianPostShipmentDocumentService` plus `RussianPostOtpravkaApiClient::download_backlog_forms()` for `GET /1.0/forms/backlog/{id}/forms`; `{id}` comes only from persisted `backlog_order_id`.
...
- `YandexShipmentAdapter::status_payload()` reuses the same actual-cost/base-cost comparison shape used by DPD/CDEK (`actual_cost_label`, `actual_cost_compare_status`, `actual_cost_compare_message`) and exposes `document_actions()` key `download_yandex_label` by universal status.
...
- `YandexShipmentDocumentService` is now invoked through `YandexShipmentDocumentProvider` and the common shipment document download service; request id is read from persisted shipment data, not from the client.
...
- `YandexShipmentButtonPolicy` no longer contains a raw Yandex terminal status list. Canonical buttons are resolved from `universal_status_code` (or a safe raw→universal fallback through `YandexStatusMapping` for old branch test records).
- Yandex cancel polling now determines terminal lifecycle by resolving the raw `request/info` status through the current admin/default mapping. Universal `delivered`, `returned_to_sender`, `cancelled` and `rejected` stop cancel polling; admin overrides can therefore make a raw code terminal or non-terminal.
- Local auto-delete remains a separate safety rule and happens only for raw `CANCELLED`, after the universal status and the shared universal→WooCommerce order status mapping are applied. Non-`CANCELLED` terminal universal statuses keep the shipment, clear cancel-pending flags, hide cancel and expose local remove.
- `PARTICULARLY_DELIVERED` defaults to `in_transit`; an existing saved admin override such as `PARTICULARLY_DELIVERED -> delivered` remains an override until reset.
...
- Yandex cancel now persists `cancellation_started` instead of clearing the metabox. The cancel response returns accepted polling metadata (`purpose=cancellation`, 5000 ms × 14), and the shared JS polling helper continues with status requests only.
...
- `src/Shipments/Application/ShipmentModalRequestMapper.php` parses modal `places[]` plus `shipment_items[]` into `ShipmentPreparationData` (`places`, `item_rows`). It no longer accepts `cdek_items[]`; invalid or missing canonical rows fail through the existing validation model instead of being repaired silently.
- `src/Shipments/Application/OrderShipmentDraftFactory.php` uses the common mapper for CDEK and Yandex admin submit paths. Both carriers now write only canonical `shipment_item_rows` for item allocation meta.
- `src/Shipments/Allocation/ShipmentAllocationBuilder.php` is the carrier-neutral builder from canonical item rows plus `ShipmentPlace` values to `ShipmentAllocation`; it replaces the old CDEK-named allocation adapter for CDEK and Yandex.
- `src/Shipments/YandexDelivery/YandexShipmentRegistrationService.php` reads only `meta['shipment_item_rows']` for canonical allocation rows. `yandex_item_rows`, `cdek_item_rows` and `ShipmentPlace::items` are not production compatibility fallbacks; missing rows fail validation before Yandex HTTP.
- `src/Shipments/*/*ShipmentPersistenceMapper.php` owns carrier-specific create persistence for CDEK, DPD, Russian Post and Yandex. `ShipmentCreationService` keeps order/request mismatch validation, adapter dispatch, mandatory mapper preflight, duplicate guard, common envelope and repository save; it returns `shipment_persistence_mapper_missing` before preview/API/repository side effects when a carrier has no mapper.
- `src/Shipments/Presentation/ShipmentActualCostComparisonService.php` owns carrier-neutral actual-cost presentation for CDEK, DPD, Russian Post and Yandex. Carrier services still provide their own actual cost source and pass only positive actual-cost values into the service; zero actual cost remains unknown and hides the price row. The shared `ShipmentBaseApiCostResolver` reads saved Base API cost and returns only positive base values. Comparison uses integer kopecks and exact +3% cross multiplication.
...
- The real multi-place source for shipment registration is the existing shipment modal submission: `places[]` plus `shipment_items[]`. `OrderShipmentDraftFactory::create_cdek_request_from_admin_data()` and `create_yandex_request_from_admin_data()` use this same canonical source.
...
- Yandex modal item rows are parsed without silent repair: `amount=0`, missing `place_number`, missing `item_key` or zero `weight` remain invalid and are rejected by the neutral `ShipmentAllocationBuilder`/`ShipmentAllocation` validation before HTTP.
- Cancel terminal decisions are no longer based on generic raw codes. `YandexShipmentRegistrationService` resolves the raw Yandex status through `YandexStatusMapping` and treats universal `delivered`, `returned_to_sender`, `cancelled` and `rejected` as terminal for cancel polling. Only raw `CANCELLED` performs local auto-delete; other terminal universal statuses close `cancellation_started`, clear `yandex_cancel_requested` and leave the shipment removable.
...
- `src/Shipments/Application/OrderShipmentDraftFactory.php` builds canonical `shipment_item_rows` for Yandex drafts before falling back to a single place. The rows keep `item_key`/order-item identity, place number and per-place quantity, so split quantity and same-SKU different order items survive into Yandex registration.
...
- `src/Shipments/YandexDelivery/YandexShipmentRegistrationService.php` is the carrier-specific bridge from the common shipment request to the already existing Yandex HTTP registration flow. It converts existing `ShipmentCreateRequest::places` plus canonical `meta['shipment_item_rows']` through `ShipmentAllocationBuilder`, builds the Yandex context, calls `YandexDeliveryShipmentRegistrationService`, and persists only canonical request/info fields.
...
- `src/Shipments/YandexDelivery/YandexShipmentButtonPolicy.php` mirrors the modern DPD approach: absent shipment shows create/manual attach, technical pending states keep their special rules, and canonical Yandex shipments use universal status for cancel/remove. `pending_creation_in_carrier` and `created_in_carrier` allow cancel; every other universal status hides cancel and allows local remove.
...
- `src/Shipments/Application/ShipmentCreationService.php` owns adapter dispatch, mandatory mapper preflight, duplicate checks, common envelope and repository persistence. For Yandex the mapper stores request/info snapshot and Yandex canonical fields, while intentionally not storing the offers/create payload body.
- `src/Shipments/YandexDelivery/YandexShipmentAdapter.php` exposes selected-offer actual cost through the common actual-cost presentation service. Manual attach still has no actual-cost source and keeps the price row hidden.
...
- `OrderDeliveryRecalculationService` exposes the resolved preview location as both `location.id` and `location.location_id`, using `selected_location_id`, then `customer_context.location_id`, then destination `location_id`. This payload is read-only and does not mark the original city as an override.
- `assets/admin/order-delivery-recalculation.js` syncs the preview `location` payload into `selectedLocations` after the first preview, preserving the fuller selected/current location fields while filling missing numeric ids. The Yandex pickup picker therefore sends a numeric location for the original order city without requiring a manual city reselect.
- `OrderDeliveryRecalculationAdminController` loads mapped local Yandex destination PVZ rows through `YandexDeliveryPickupPointV2Repository` and the shared checkout formatter; if a Yandex pickup search arrives without `id/location_id`, it resolves the same preview-compatible location through `OrderDeliveryRecalculationService::resolved_location_payload()` instead of duplicating FIAS/GAR lookup in the controller.
- Admin Yandex popup/list cards consume checkout presentation fields (`point_title`, `card_title`, `display_title`, `presentation_comment`) from `YandexDeliveryCheckoutPickupPointFormatter`. Technical station ids remain in `platform_station_id`/`point_code` for identity, selected-station repricing and save, but are not displayed as titles/codes in the admin picker. Side-card `presentation_comment` is rendered in a vertical heading container directly below the title and before the address.
- The admin pickup map keeps all loaded pickup points in the provider marker dataset, while the side list is filtered locally by provider bounds in the shared `west,south,east,north` format. Bounds are tied to `pointsGeneration`/`boundsGeneration`, so initial provider bounds emitted before a new dataset loads are not applied to that dataset. After `renderMarkers()` and camera changes (`setCenter`, `focusPoint`, `fitToMarkers`), admin JS performs a deferred read-only `provider.getBounds()` sync; the Yandex and Leaflet providers expose `getBounds()` through the same converter used by their `boundschange` callback. Initial center prefers selected pickup coordinates, then preview point coordinates, then selected location coordinates, before the fallback center. Pan/zoom does not call REST, does not rerender the full marker dataset and does not clear the selected point; out-of-viewport selected points remain confirmed/active and reappear in the list when the viewport includes them again.
...
## Yandex Delivery Checkout Pricing and PVZ Selection 0.104.13
...
- `src/Carriers/Runtime/YandexDeliveryCarrier.php` prices courier delivery against the checkout address first, then makes at most one `time_interval` fallback request using the locally resolved address of the same selected/representative destination PVZ used by pickup priority. `destination_pickup_point_by_platform_station_id()` provides the row; only `full_address` or complete locality/street/house enters the transient payload. Safe rate meta records source/fallback/station/error codes without address data, and no checkout/session/order address is mutated.
...
- `src/Shipments/Dpd/DpdShipmentPersistenceMapper.php` owns DPD create persistence fields; no CDEK fields are present in the common creation service or DPD mapper output.
...
- `src/Shipments/Dpd/DpdShipmentAdapter.php` validates, calls DPD createOrder2, normalizes DPD order/request/parcel numbers and keeps labels/documents empty; status autosync is handled globally through DPD `getEvents`, not per order. Its actual-cost status presentation delegates to `ShipmentActualCostComparisonService`.
- `src/Shipments/Dpd/DpdShipmentPersistenceMapper.php` saves successful DPD manual creates in `_wdc_shipments[dpd]` with `pending_creation_in_carrier`, DPD identifiers, sanitized request/response and admin marker through the common `ShipmentCreationService` envelope.
...
- `assets/frontend/pickup-map/wdc-pickup-checkout.js` keeps `pickupSelections` as the restore source of truth, restores the active family bucket on boot/reload from localized `activePickupFamily`/`selectedPickupPoint`, reruns `boot()` on WooCommerce `updated_checkout`, opens `[data-wdc-pickup-open]` through delegated document click handling so ajax-recreated buttons remain clickable, maps `yandex_pickup` to `yandex_delivery:pickup` for family matching/pickup-rate detection, enables named POST fields only in the active pickup container while preserving inactive family values, and restores an available `yandex_pickup` after selected-point repricing with at most one guarded recovery `update_checkout`; it also merges REST/localized dictionaries without replacing complete payloads by code-only points and starts background prefetch for the active pickup family (including CDEK city-code requests);
...
- `src/Checkout/WooCommerce/CheckoutSessionManager.php` exposes the explicit narrow `expire_stale_yandex_5post_selection()` operation. It resolves top-level/snapshot `operator_id`, parses UTC `selected_at`, compares the WordPress-timezone calendar date and clears only the `yandex_delivery:pickup` bucket for stale or malformed 5Post selections. `NewShippingMethod`, `PickupMapCheckout`, `CheckoutValidation` and the warning renderer invoke it before using selected state; validation keeps a request-local stale POST guard.
...
- `assets/frontend/pickup-map/wdc-pickup-checkout.js` marks real map/cross-location saves as `selection_intent=explicit` and automatic same-family rate synchronization as `technical`; `wdc-pickup-api.js` forwards intent at the checkout pickup REST body top level. `src/Pickup/Rest/CheckoutPickupPointRestController.php` defaults missing/unknown intent to explicit, assigns fresh server UTC `selected_at` only for explicit saves, and for technical saves preserves existing-family timestamp before incoming values without creating current time. Yandex `operator_id` remains top-level, with existing operator/station identity used as technical fallback while snapshot data is retained. `CheckoutRateRenderer` emits `wdc-yandex-5post-warning` after ordinary comments only for a current, destination-valid selected 5Post; `assets/frontend/checkout-rates.css` provides its warning presentation.
- src/Checkout/WooCommerce/CheckoutValidation.php requires a Yandex PVZ for yandex_pickup, rejects pickup selections from other carriers, and skips PVZ validation for yandex_courier. OrderShippingMetaPersister saves canonical _wdc_pickup_* meta plus Yandex aliases including _wdc_yandex_delivery_pickup_platform_station_id; visible shipping-item pickup_family is removed while hidden _wdc_pickup_family and calculation data remain.
# Yandex shipment registration foundation (0.107.1)

- `src/Shipments/Allocation/ShipmentAllocation*.php` is the neutral read-model for a prepared shipment allocation: stable place number/dimensions/weight and per-place source item identity/quantity/unit price/assessed unit price. Registration allocation validation requires at least one item overall and at least one item in every place.
- `src/Shipments/Allocation/ShipmentAllocationBuilder.php` adapts canonical `meta['shipment_item_rows']` plus `ShipmentPlace`; it validates the source and throws `InvalidArgumentException` for broken allocation, empty rows or places without rows instead of silently remapping rows. Money fields must already be canonical non-negative integer kopecks or digit-only strings, so decimal/scientific values are rejected instead of cast. It is carrier-neutral and does not change CDEK creation payload semantics.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentPayloadBuilder.php` builds a transport-free offers/create payload. It validates strict pickup/courier destination modes, required pickup station, required courier structured address fields, recipient name/phone/email and `ready_to >= ready_from`. Yandex item billing fields live under `items[].billing_details`; ready intervals are formatted as UTC `yyyy-MM-ddTHH:mm:ss.ffffffZ`. It must remain free of HTTP, persistence and UI wiring.
- `src/Carriers/YandexDelivery/Api/YandexDeliveryApiClient.php` uses the existing `YandexDeliveryHttpClientInterface` / `WpYandexDeliveryHttpClient` transport and exposes shipment endpoints: `offers/create?send_unix=false`, `offers/confirm`, `request/info`, `request/history` and `request/cancel`. `request/info` and `request/history` are GET query requests without JSON body.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentClient.php` is the typed shipment HTTP facade. It returns DTOs instead of raw arrays, validates request_id consistency and barcode-map safety, and performs no retry.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryEarliestOfferSelector.php` chooses the earliest matching offer by delivery min, delivery max, pickup max, price and offer id.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryOffer*.php` parses the production `offer_details` response shape. `YandexDeliveryRequestInfo` parses top-level ids/state plus nested `request` data, `YandexDeliveryRequestHistory` reads `state_history`, and `YandexDeliveryShipmentState` preserves async cancel state.
- `src/Carriers/YandexDelivery/Shipment/YandexDeliveryShipmentRegistrationService.php` orchestrates `payload builder -> offers/create -> selector -> offers/confirm -> request/info`. It does not call `request/create` and is not wired to UI, checkout, order save or persistence. If request/info fails after confirm, it preserves `confirmed_request_id` for reconciliation instead of retrying confirm.
- `docs/wdc-yandex-delivery-shipment-registration.md` records production API findings, allocation ownership and the boundary of this patch.
