...
- `src/Shipments/*/*ShipmentPersistenceMapper.php` owns carrier-specific create persistence for CDEK, DPD, Russian Post and Yandex. `ShipmentCreationService` keeps the common envelope, duplicate guard, adapter dispatch, mapper dispatch and repository save; it no longer contains carrier-specific persistence keys.
...
- `src/Shipments/Application/ShipmentCreationService.php` owns duplicate checks, adapter dispatch, common envelope, mapper dispatch and repository persistence. For Yandex the mapper stores request/info snapshot and Yandex canonical fields, while intentionally not storing the offers/create payload body.
