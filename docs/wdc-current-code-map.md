...
## Yandex Delivery Checkout Pricing and PVZ Selection 0.104.12
...
- `src/Pickup/Rest/CheckoutPickupPointRestController.php` saves Yandex `operator_id` at selection top level while retaining snapshot data and assigning UTC `selected_at` only on explicit REST save. `CheckoutRateRenderer` emits `wdc-yandex-5post-warning` after ordinary comments only for a current, destination-valid selected 5Post; `assets/frontend/checkout-rates.css` provides its warning presentation.
