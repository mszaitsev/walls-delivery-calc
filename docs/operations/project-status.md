...
The current Walls Delivery Calc release source is version 1.0.26. It uses one fresh-install schema migration and does not promise upgrade compatibility from arbitrary unpublished 0.x development databases. Order administrators can optionally edit orders in every status, and can explicitly clear only WDC-owned delivery calculation metadata when no shipment record exists; the WooCommerce shipping line and customer-visible planned delivery metadata remain intact. Jet Logistic manual geography mapping accepts `0` as an explicit unmap command and resets the live row to unmatched. Checkout and admin recalculation now use one shared pickup-map runtime. Its sidebar is always the current viewport projection of loaded valid-coordinate markers, capped at 100 cards after filtering; address search remains DaData/geocoding only. Ozon maps progressively read the complete eligible active-generation dataset from local DB with exact loaded/total progress, browser-side dedupe/batched ingestion, coalesced Leaflet rendering, restart-safe signed continuation, and no Ozon API request during map use. Ozon Moscow and Saint Petersburg receive only an initial `+1` zoom hint; other cities/carriers retain their existing zoom and loading strategy. Generic marker semantics remain unchanged: ordinary points are blue, reliable postamats plus Yandex 5Post and paid PEK partners are purple, and the active point is red. Shipment Framework boundaries and schema remain unchanged.
