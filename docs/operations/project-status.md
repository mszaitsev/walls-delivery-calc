...
- PEK foundation and delivery-service settings. The built-in PEK service is disabled by default, starts with only `RU`, stores legal sender data and the default sender warehouse, preserves branch timezone/availability fields, and keeps `AM`, `BY`, `KG`, and `KZ` planned for later international testing. Runtime checkout quotes, shipment creation, statuses, documents, actual-cost integration, and cancellation are not implemented.
