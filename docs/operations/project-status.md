...
- PEK foundation now seeds a predefined disabled `pek` API delivery service with only `RU` on first setup and preserves administrator country choices on later boot, including an explicit empty selection. PEK settings store login, encrypted API key, legal sender data, client card, HTTP limits, SMS-release contractual limit, and the default sender warehouse snapshot. PEK admin POST actions execute through the carrier-owned admin component and PRG redirect back to the PEK tab; notices are user-scoped one-shot transients, not global settings. `/typesOfDelivery/all/` uses GET; branch/legal/warehouse foundation methods use POST. Manual diagnostics verify credentials, API availability, LTL `type=3`, official `shortName`/`codeByClassifier` branch countries with RU classifier `643`, and legal-form directory; nested diagnostic data is rendered safely. Manual warehouse search uses `/branches/nearestdepartments/` with `departmentOperation=2` and `type=3`, clears stale current-user cache before every new search attempt, normalizes official `timeZone` values like `04:00:00` into canonical `branchTimezone` values like `UTC+04:00`, stores only the new short-lived current-user result, and never leaves old selectable results after empty or failed searches. Selected warehouse IDs are validated fail-closed through fresh cache or `/branches/all/` official nested warehouse data before saving; cached selection can save the canonical timezone without `/branches/all/` fallback when the fresh search result is sufficient. Fallback validation uses mbstring-independent Unicode matching for `Прием грузов`, reads branch timezone from `branches[].timezone`, compares PEK closing fields as real Unix instants via `current_datetime()->getTimestamp()`, rejects impossible or unresolved timezone-less dates fail-closed, stores timezone/availability fields through cache and snapshot, and preserves the previous selected warehouse on validation failure. Shipment Framework remains unchanged.
