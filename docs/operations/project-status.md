...
- PEK foundation now seeds a predefined disabled `pek` API delivery service with only `RU` on first setup and preserves administrator country choices on later boot, including an explicit empty selection. PEK settings store login, encrypted API key, legal sender data, client card, HTTP limits, SMS-release contractual limit, and the default sender warehouse snapshot. PEK admin POST actions execute through the carrier-owned admin component and PRG redirect back to the PEK tab. `/typesOfDelivery/all/` uses GET; branch/legal/warehouse foundation methods use POST. Manual diagnostics verify credentials, API availability, LTL `type=3`, official `shortName`/`codeByClassifier` branch countries with RU classifier `643`, and legal-form directory. Manual warehouse search uses `/branches/nearestdepartments/` with `departmentOperation=2` and `type=3`, stores a short-lived current-user transient cache, and selected warehouse IDs are validated fail-closed through fresh cache or `/branches/all/` official nested warehouse data before saving. Shipment Framework remains unchanged.
