...
This creates `dist/walls-delivery-calc-1.0.5.zip` with one top-level `walls-delivery-calc/` directory. Install it through **Plugins → Add Plugin → Upload Plugin**, then activate **Walls Delivery Calc**. No dependency installation or asset compilation is required on the server.
