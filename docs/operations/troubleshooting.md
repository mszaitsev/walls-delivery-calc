...
On activation, verify the plugin reports version 1.0.22 and that the unchanged schema baseline `wdc_db_version` remains `1.0.0`. A fresh install runs only `database/migrations/0001_initial_schema.php`. If activation reports a migration failure, inspect the WordPress database error and table privileges; do not edit migration options manually or replay deleted 0.x migration files.
...
For Ozon pickup, a building generation must retain the same `job_id` and `lock_owner` as its valid 900-second lease and have at most one exact step continuation. While a callback runs, the same option also contains its private execution token; a second callback with the same job owner must do no work. One callback may advance several pages or 100-ID enrichment batches; this is normal. A retry should leave one delayed continuation, while active/failed/cancelled states leave none. Discovery IDs are written in 250-row `INSERT IGNORE` chunks and enrichment is set-based; affected-row mismatch is an atomic persistence failure, not a reason to activate a partial snapshot. The admin browser only polls status, never starts steps on reload, and continues polling after temporary transport failures.
...
While the Yandex full pipeline is `running` or `paused`, do not drive pickup import, geo builder, region enrichment, region mapping, or location mapping through standalone controls or direct AJAX. The server returns HTTP 409 for these lower-stage mutations and the browser disables their loops because the outer pipeline is their sole executor. A manual location override may still be saved or deactivated, but its one-geo live remap is deferred until the current or next mapping run. Before this ownership guard, a browser loop could race the background worker, make counters appear to roll backward, and attempt another pickup batch after successful promotion had correctly deleted the downloaded JSON. A not-readable JSON error with a lower offset behind already-published live counts is characteristic of that old race, not evidence that successful cleanup itself should be disabled.
