...
WDC business clock times are interpreted in `Asia/Novosibirsk`; scheduler APIs continue to receive Unix timestamps. Daily local-clock owners share `TimezoneService`. Relative intervals retain interval semantics, and all next-run values in WDC admin are formatted in Novosibirsk time.
