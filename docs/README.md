...
0.134.19 addresses the live courier preview compatibility gap found after 0.134.18: the actual address-zone lookup was still blocked for an older courier order whose numeric canonical destination `location_id` and selected-location FIAS were both absent. New PEK rates keep persisting `location_id` in generic rate meta; existing orders now recover strictly from persisted selected-location FIAS first, then from server-side `_wdc_platform_city_fias_id`, without trusting browser input or house/full-address FIAS.
