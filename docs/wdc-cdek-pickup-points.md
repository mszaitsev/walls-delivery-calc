...
0.45.20 stale container restore fix: checkout pickup restore no longer rejects a valid active family bucket only because the `[data-wdc-pickup-checkout]` container has an empty or stale `data-shipping-method-id`. A generic/single pickup block can accept the active pickup method, and the method id is rewritten before `applySelection()`.
...
0.45.16 raw session write fix: canonical pickup buckets now use a dedicated raw array writer/reader for `wdc_platform_pickup_selections`. The save path writes the nested dictionary and immediately rereads the raw session key to verify that the active `...:pickup` family is present.

0.45.15 canonical bucket save fix: REST pickup save now writes the selected point through `CheckoutSessionManager::save_pickup_selection_for_family()`. The canonical `wdc_platform_pickup_selections` dictionary is populated before legacy mirrors, so `pickupSelections['cdek:pickup']` and `pickupSelections['russian_post_domestic:pickup']` survive reload and are available to checkout validation even when hidden fields are empty.
