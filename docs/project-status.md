...
0.55.0 note: DPD geography layer is added without tariff calculation, checkout runtime, pickup points, shipment creation, labels, statuses, cron or FTP import. `DpdCityResolver` now resolves DPD `cityId` from the existing WDC/FIAS/GAR `Location`, first reusing `wdc_location_carrier_codes`, then calling the DPD geography API wrapper `getCitiesCashPay(countryCode)` and matching the returned city list. Successful matches are persisted in `wdc_location_carrier_codes` with `carrier_key=dpd` and `external_code=<dpd_city_id>`. `DpdDuplicateCityResolver` isolates duplicate/list matching by FIAS GUID, GAR ID, cityCode/KLADR, regionCode, postal code and city name. `getPossibleExtraService` remains an optional wrapper but is no longer used as the primary city resolver because DPD expects a fuller service/address payload and live checks returned SOAP errors for sparse location payloads. `DpdGeographyDiagnosticService` catches DPD API/transport exceptions and saves a non-fatal diagnostic result. DPD is still not registered in `CarrierRegistry` or `CarrierShipmentAdapterRegistry`.
