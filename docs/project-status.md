...
0.55.0 note: DPD geography layer is added without tariff calculation, checkout runtime, pickup points, shipment creation, labels, statuses, cron or FTP import. `DpdCityResolver` now resolves DPD `cityId` from the existing WDC/FIAS/GAR `Location`, first reusing `wdc_location_carrier_codes`, then calling the DPD geography API wrapper, then falling back to a FIAS-only lookup. Successful matches are persisted in `wdc_location_carrier_codes` with `carrier_key=dpd` and `external_code=<dpd_city_id>`. `DpdDuplicateCityResolver` isolates `pickupDups` / `deliveryDups` matching by FIAS GUID, GAR ID, cityCode/KLADR, regionCode, postal code and city name. `DpdApiClient` adds geography wrappers for `getCitiesCashPay` and `getPossibleExtraService`; `DpdGeographyDiagnosticService` supports admin diagnostics/manual mapping only. DPD is still not registered in `CarrierRegistry` or `CarrierShipmentAdapterRegistry`.
