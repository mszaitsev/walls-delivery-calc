...
- Plugin entrypoint and `WDC_VERSION` are updated to `0.46.6`.
...
- `src/Carriers/Runtime/CdekCarrier.php` sends CDEK tarifflist `packages[]` per `PackageItem` unit, applies aggregate `packaging_weight_g` once to the first item package unless packaging is already represented by `WDC_PACKAGING`, and keeps the old aggregated package only as no-item fallback.
