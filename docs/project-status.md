...
- Версия / baseline проекта: `0.36.1`, определено по `walls-delivery-calc.php`.
- Базовая ветка: `develop`.
- Последнее обновление статуса: 2026-06-06.
- Общий процент готовности: примерно 66%.
- Текущий этап 0.36.1: mapping ручного обновления статуса Почты России уточнен: pickup-операции `8:2` и связанные пары, `12:1..12:31`, `42:1..42:30` дают `ожидает самовывоза из ПВЗ/постамата`, `8:15` и `8:18` дают `передан курьеру`; неизвестные пары остаются `не определён`. Документы/ярлыки/отмена и автоматическая синхронизация остаются отдельными этапами.
