...
Current stage: 0.155.12 adjusts the WooCommerce checkout billing field presentation inside the WDC checkout runtime. Billing fields now render as name/patronymic, surname, country, city, region, postcode, address, phone, and email; billing address line 2 is removed from the checkout. Billing address line 1 stays visible but optional by default and becomes required only when the selected WDC shipping rate is classified as courier by generic delivery metadata. Phone is required, email keeps WooCommerce's existing requirement, and order comments use the approved multiline placeholder for special handling notes.
