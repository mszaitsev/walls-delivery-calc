...
Current plugin version: 0.68.0.

Version 0.68.0 connects DPD to the existing shipment status autosync cron without changing manual create/update flows. The shared autosync service now runs one global DpdEventSyncService::sync() per cron/manual autosync run, so DPD uses a single getEvents client inbox call instead of polling each order. Autosync-only enrichment calls getStatesByDPDOrder only when a new DPD event updated a shipment that still lacks actual cost or planned delivery date. DPD Расчет adds the enabled-by-default checkbox Автоматическое обновление статусов DPD and readonly last run/result diagnostics. DPD documents/labels, create flow, checkout pricing and event matching were not changed. See docs/wdc-dpd-autosync.md.
Version 0.67.5 fixes two DPD admin create/status UI issues: the registration polling indicator now uses a dedicated animated CSS spinner, and the DPD create AJAX response builds its `status` payload from the refreshed shipment returned after `createOrder2 OK -> getEvents/enrichment/touch`, so `1401 / OrderCreate` and the cancel button appear immediately without a manual status update.
Version 0.67.4 refines DPD `getEvents` for pending re-registration after a previous DPD shipment was cancelled/deleted on the same WooCommerce order. Pending shipments without `dpd_order_number` may still use `clientOrderNr`, but only for events newer than `registration_started_at - 5 minutes`; old cancellation/return events with a DPD number are logged as unmatched with `reason=stale_or_cancelled_pending_event`, do not bind the old `dpdOrderNr`, and do not block confirm. Once `dpd_order_number` is saved, `clientOrderNr` no longer affects matching.
Version 0.67.3 hardens DPD event matching when one WooCommerce order has had multiple DPD shipments over time. Events are matched by `dpdOrderNr` first; `clientOrderNr` is only a fallback while the local shipment has no saved DPD number. If an active shipment already has `dpd_order_number`, an event with another `dpdOrderNr` is treated as unmatched and logged with the saved DPD number, so old cancelled/deleted shipment events cannot overwrite the active shipment.
Version 0.67.2 fixes the DPD create/status QA flow. After `createOrder2` returns `OK + orderNum`, WDC immediately runs the same DPD refresh path as the manual update button: global `getEvents`, current-order enrichment and `tracking_checked_at` touch, so `OrderCreate` can appear in the shipment block without an extra click. Manual `Обновить статус` now updates `tracking_checked_at` / `updated_at` even when `getEvents` returns no new matching event. Button policy after reload reads saved DPD/generic operation fields, so cancellable events such as `1401` keep the cancel button visible and `1301` shows local remove. The DPD shipment record now stores the actual `parcel[]` dimensions and cargo summary sent in `createOrder2` as `dpd_sent_places` / cargo fields and renders them in the `Отправления` block; DPD cabinet display is not treated as the source of truth for dimensions.
...
Version 0.66.2 fixes the DPD shipment modal frontend flow after site verification. Opening the DPD modal now immediately requests preview instead of leaving `preview_pending`; successful courier address normalization syncs hidden DPD structured fields and refreshes preview; sender `ФИО курьера` input/blur refreshes preview with debounce/immediate blur behavior; and the DPD create button is enabled only from the latest preview without errors plus local DPD readiness checks. `Комментарии курьеру` is visible only for DPD courier delivery and remains hidden for DPD pickup. No DPD SOAP business payload changes, checkout pricing, recalculation, labels, status sync or cancellation were added.
Version 0.66.1 fixes the manual DPD live-create payload after site verification. `order2/createOrder2` now uses a longer 90s create timeout, and SOAP `Error Fetching http headers` is treated as an uncertain result: WDC does not save a successful shipment without a DPD response and tells the manager to check the DPD cabinet before retrying. `DPD Расчет` adds `Категория груза DPD`, `Название отправителя DPD` and `Телефон отправителя DPD`; live payload sends `header.payer` from the configured DPD clientNumber, sender name/phone from settings and `header.senderAddress.contactFio` from the modal with editable history. DPD courier receiver addresses for Russia now use structured address fields from DaData hidden inputs instead of `addressString`, and `Комментарии курьеру` maps only to `receiverAddress.instructions`. Visible preview no longer shows legacy `dry_run` / `live_api_call`. No auto retry, status sync, labels or cancellation were added. See `docs/wdc-dpd-create-order.md`.
Version 0.66.0 added manual live DPD shipment creation from the WooCommerce order `Отправления` modal. Managers still use `Предпросмотр payload`, and the new `Создать отправление DPD` button calls DPD SOAP `order2/createOrder2` only after valid datePickup, terminalCode/address and cargo-place data. The same `DpdShipmentPayloadBuilder` feeds preview and live create. Successful responses are saved in `_wdc_shipments` with `pending_creation_in_carrier`, DPD order/request/parcel numbers when returned, selected serviceCode, delivery type, terminalCode/date/cargo snapshots and sanitized request/response. Duplicate active DPD shipments are blocked. No DPD auto-create, cron/status sync, labels, cancellation, COD/NPP or unitLoad were added. See `docs/wdc-dpd-create-order.md`.
...
payload shape. DPD tariff debug output now exposes only redacted payload shape metadata, incomplete credentials
stop before any SOAP call, and DPD geography/tariff action result notices are one-shot after render while the
last import report remains persistent.
...
Version 0.46.5 changes CDEK tariff calculation package formation. `POST /v2/calculator/tarifflist` now receives one package per `PackageItem` unit (`quantity` expands into repeated packages) using item weight and item dimensions with per-dimension CDEK defaults only when a product dimension is missing. Virtual packaging items such as `WDC_PACKAGING` are sent as their own package, and the old aggregated package remains only as a fallback when no package items exist.
...
Version 0.45.1 fixes the first CDEK pickup QA pass. Checkout validation now restores selected CDEK pickup points from the CDEK checkout/session snapshot instead of looking them up as Russian Post points, so selected `cdek:pickup` orders can be created without the false `Выберите пункт выдачи` error. CDEK `point_code`/`cdek_code` now uses the CDEK delivery point code such as `KEM7`, not the postcode. CDEK `PVZ` renders as `Пункт выдачи СДЭК`; CDEK `POSTAMAT` renders as `Постамат СДЭК`, has a separate map marker color, and shows red bold `Срок хранения 3 дня` in pickup cards/popups/order/email output. Pickup descriptions from CDEK are saved into calculation data and rendered with the selected point. Permanent FIAS/GAR -> CDEK `city_code` mapping remains technical debt for a later CDEK integration stage.
...
Version 0.37.0 finishes the Russian Post shipment cleanup for the current manual workflow. The order metabox no longer shows a documents/download placeholder because labels, batches, F103 and Russian Post documents are prepared manually in the Russian Post account. Managers can cancel a just-created backlog shipment while Russian Post still reports `Присвоение идентификатора`; cancellation uses `DELETE /1.0/backlog` with `backlog_order_id`. Managers can also attach a manually created shipment by entering ШПИ; WDC searches `GET /1.0/backlog/search` first and later versions fall back to `GET /1.0/shipment/search`, saves barcode plus `backlog_order_id` when Russian Post returns one, then tries the first Tracking API status refresh. Tracking still uses barcode/ШПИ, not `backlog_order_id`.
...
Version 0.27.9 adds reason-coded pickup reset diagnostics and hardens checkout submit against WooCommerce recalculation payloads that omit `shipping_method`. Address runtime now falls back to `chosen_shipping_methods`, refuses automatic pickup clears while the active method family is Russian Post pickup, and frontend `updated_checkout` skips context refresh during place order. The pickup CSS remains scoped away from WooCommerce button and loader pseudo-elements.

Version 0.27.8 hardens Russian Post pickup selection persistence around WooCommerce checkout recalculation. Pickup selections are now matched and refreshed by the Russian Post pickup method family, address recalculation preserves the point during same-family tariff switches, and frontend reset functions are guarded during place order while `updated_checkout` restores hidden fields from the selected point.

Version 0.27.7 preserves the selected Russian Post pickup point during checkout submit and when switching tariff suffixes inside the Russian Post pickup family. Checkout validation now accepts the saved pickup point by method family, can restore the selection from submitted hidden pickup fields, and the frontend only clears pickup state after a real destination or carrier change.
...
Version 0.27.2 improves cross-location pickup selection UX and persistence. The pickup modal now shows a loading overlay and disables select buttons while the point is being checked, saved, or delivery is recalculating. For pickup points in another locality, checkout now runs a controlled flow: update destination fields, wait for WooCommerce `updated_checkout`, select the cheapest available Russian Post pickup rate if the previous rate disappeared, and only then save the pending pickup point. If no pickup rate remains available after recalculation, the modal shows a warning and does not save the point.
...
Version 0.23.0 starts the checkout MVP for Russian Post domestic pickup points. The checkout now renders a "Выбрать пункт выдачи" control for Russian Post pickup, opens a mobile-friendly Leaflet/OpenStreetMap modal, loads points only by map bbox through `/wp-json/wdc/v1/points`, stores the selected point in WooCommerce session through `/wp-json/wdc/v1/checkout/pickup-point`, validates the required pickup selection server-side, and saves HPOS-safe pickup meta/snapshot on the order and shipping item.
