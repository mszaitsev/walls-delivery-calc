...
Current Ozon Delivery scope: pickup checkout pricing for `Ozon до ПВЗ` is implemented through the carrier-owned quote layer and official Ozon Delivery checkout contract, but remains fail-closed until a safe admin live diagnostic succeeds for the configured `shipment_method_id`. Version 0.141.16 preserves insurance-aware pricing: every Ozon posting is priced as `estimated_delivery_cost + estimated_insurance_cost`, all `results[]` are summed, and the maximum posting delivery days is used. Multi-box pricing stays one `/v1/order/checkout` request with multiple postings. Single posting uses the full declared goods value, while multiple postings use the same integer-safe `ceil(total / posting_count)` whole-ruble declared value per posting. The Ozon local pickup provider now evaluates cargo restrictions per place: `max_place_weight_g` is used for point max weight, every posting/place must fit, and dimensions are compared axis-correctly with rotation by sorting parcel and point limits. Checkout uses the standard WooCommerce `billing_phone`, including AJAX `post_data`, and Ozon has an optional carrier-owned fallback phone for missing or locally invalid customer numbers. The Ozon rate carries the trusted `pickup_provider_query.destination_fingerprint` required by the generic pickup REST resolver, and the buyer map receives every active cargo-compatible local Ozon point inside the trusted 60 km destination radius without arbitrary first-100/500 truncation. For this fixed-area dataset the map performs one remote `/points` load per active modal context, then pan/zoom rebuilds clusters and the virtualized sidebar locally; search works against the full loaded point array. Ozon pickup points declare generic `requires_rate_refresh`, so saving a point triggers selected-point authoritative repricing through the shared checkout lifecycle. Ozon Shipment Framework features are intentionally not included in this version.
