...
$rate_23031_stale_range = new DeliveryRate( RussianPostDomesticSettings::rate_id( DeliveryType::PICKUP, '23031' ), RussianPostDomesticSettings::CARRIER_KEY, 'Почта России', RussianPostDomesticSettings::SERVICE_KEY, RussianPostDomesticSettings::TITLE, '23031', 'Посылка онлайн stale', DeliveryType::PICKUP, 'Посылка онлайн stale', Money::from_rubles( 455 ), null, null, DateRange::range( 5, 6 ), '', '4-5 дн.', array(), false, '', false, false, array( 'tariff_selector_group' => true ) );
...
rpd_assert( '5-6 дней' === (string) ( $selected_rate->meta['tariff_variants'][0]['delivery_days_label'] ?? '' ) && '' === (string) ( $selected_rate->meta['tariff_variants'][0]['planned_delivery_comment'] ?? '' ), 'Domestic selector range variant must keep the short days label separate from an absent canonical planned comment.' );
rpd_assert( '3 дня' === (string) ( $selected_rate->meta['tariff_variants'][1]['delivery_days_label'] ?? '' ), 'Domestic selector single-day variant row must expose final delivery days label.' );
rpd_assert( '2-3 дн.' === (string) ( $selected_rate->meta['tariff_variants'][1]['planned_delivery_comment'] ?? '' ), 'Domestic selector variant must expose its canonical planned comment independently of the short days label.' );
...
$stale_range_rate = $selector_method->invoke( $method, $pickup_group_id, array( $rate_23031_stale_range, $rate_47030 ) );
rpd_assert( $stale_range_rate instanceof DeliveryRate && '5-6 дней' === (string) ( $stale_range_rate->meta['tariff_variants'][0]['delivery_days_label'] ?? '' ) && '4-5 дн.' === (string) ( $stale_range_rate->meta['tariff_variants'][0]['planned_delivery_comment'] ?? '' ), 'Domestic selector range variant must not replace its canonical planned comment with the short days label.' );
$rate_23030_rule_adjusted = new DeliveryRate( RussianPostDomesticSettings::rate_id( DeliveryType::PICKUP, '23030' ), RussianPostDomesticSettings::CARRIER_KEY, 'Почта России', RussianPostDomesticSettings::SERVICE_KEY, RussianPostDomesticSettings::TITLE, '23030', 'Посылка онлайн', DeliveryType::PICKUP, 'Посылка онлайн', Money::from_rubles( 450 ), null, null, $built['rate']->delivery_days, '', '5-6 дн.', array(), false, '', false, false, array( 'tariff_selector_group' => true ) );
$rule_adjusted_rate = $selector_method->invoke( $method, $pickup_group_id, array( $rate_23030_rule_adjusted, $rate_47030 ) );
rpd_assert( $rule_adjusted_rate instanceof DeliveryRate && '7-8 дней' === (string) ( $rule_adjusted_rate->meta['tariff_variants'][0]['delivery_days_label'] ?? '' ) && '5-6 дн.' === (string) ( $rule_adjusted_rate->meta['tariff_variants'][0]['planned_delivery_comment'] ?? '' ), 'Domestic selector rule-adjusted variant must keep final adjusted days and its canonical planned comment as separate fields.' );
...
rpd_assert( ! str_contains( $single_rate_html, 'wdc-platform-delivery-comment' ) && ( '' === $single_rate->planned_delivery_comment || ! str_contains( $single_rate_html, $single_rate->planned_delivery_comment ) ), 'Single-tariff domestic grouped planned delivery comment must stay in the label only.' );
