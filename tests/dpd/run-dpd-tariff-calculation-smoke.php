...
dpd_tariff_assert( str_contains( $plugin_source, 'DpdQuoteCarrier' ) && str_contains( $plugin_source, 'DpdShipmentAdapter' ) && str_contains( $dpd_adapter_source, 'createOrder2' ), 'DPD may be registered for checkout quotes and manual shipment create only.' );
...
dpd_tariff_assert( ! str_contains( $admin_source, 'unitLoad' ), 'DPD tariff admin must not add unitLoad.' );
