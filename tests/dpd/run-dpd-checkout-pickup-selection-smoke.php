...
dpd_checkout_pickup_assert( str_contains( $plugin_source, 'DpdShipmentAdapter' ) && str_contains( $shipments_metabox, 'data-wdc-preview-shipment' ) && str_contains( $shipments_metabox, 'AJAX_CONTINUE_LIFECYCLE' ) && str_contains( $dpd_adapter_source, 'createOrder2' ) && str_contains( $dpd_adapter_source, 'continue_lifecycle' ), 'DPD shipment metabox must expose preview/manual create through the neutral lifecycle continuation contract.' );
