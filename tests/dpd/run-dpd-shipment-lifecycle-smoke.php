...
dpd_lifecycle_assert( str_contains( $registration_source, 'registration_started_at' ) && str_contains( $registration_source, 'dpd_registration_state' ) && str_contains( $registration_source, 'ShipmentLifecycleResult::PHASE_SUBMISSION_REQUIRED' ), 'Registration service must save pending attempt and expose neutral submission_required lifecycle before submit createOrder2.' );
dpd_lifecycle_assert( str_contains( $registration_source, 'ShipmentLifecycleResult::PHASE_POLLING_REQUIRED' ) && str_contains( $registration_source, 'stop_on_error: true' ) && str_contains( $registration_source, 'poll_interval_ms: 10000' ), 'DPD continuation/status must express polling through neutral lifecycle settings.' );
...
dpd_lifecycle_assert( str_contains( $create_controller_source, 'is_array( $result' ) && str_contains( $create_controller_source, '? $result' ) && str_contains( $create_controller_source, 'carrier_ui_payload' ), 'DPD create AJAX payload must build UI status from the refreshed shipment returned by registration service.' );
...
dpd_lifecycle_assert( str_contains( $adapter_source, 'CarrierShipmentLifecycleContinuationInterface' ) && str_contains( $adapter_source, 'continue_lifecycle' ) && ! str_contains( $adapter_source, 'submit_registration' ) && str_contains( $adapter_source, 'supports_status_auto_sync(): bool' ), 'DPD adapter must expose neutral lifecycle continuation and no old submit_registration method.' );
$admin_js_dpd_source = (string) file_get_contents( dirname( __DIR__, 2 ) . '/assets/admin/shipments/extensions/dpd.js' );
$admin_js_events_source = (string) file_get_contents( dirname( __DIR__, 2 ) . '/assets/admin/shipments/shipment-events.js' );
dpd_lifecycle_assert( ! str_contains( $admin_js_dpd_source, 'submitDpdRegistration' ) && ! str_contains( $admin_js_dpd_source, 'startDpdRegistrationPolling' ) && ! str_contains( $admin_js_dpd_source, 'handleCreateResponse' ), 'DPD JS extension must not own lifecycle submit transport.' );
dpd_lifecycle_assert( str_contains( $admin_js_events_source, 'handleShipmentLifecycleResult' ) && ! str_contains( $admin_js_events_source, 'registration_attempt_id' ), 'Generic create JS must use lifecycle result instead of DPD attempt aliases.' );
