...
dpd_lifecycle_assert( str_contains( $ajax_service_source, '?array $shipment_override = null' ) && str_contains( $ajax_service_source, 'is_array( $result' ) && str_contains( $ajax_service_source, '? $result' ), 'DPD create AJAX payload must build UI status from the refreshed shipment returned by registration service.' );
