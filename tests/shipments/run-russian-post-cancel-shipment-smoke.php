...
russian_post_cancel_smoke_assert( str_contains( $metabox_source, 'document_actions_for_carrier' ) && str_contains( $metabox_source, 'document_actions' ) && ! str_contains( $metabox_source, $legacy_document_payload_key ) && ! str_contains( $metabox_source, 'RussianPostDomesticSettings::CARRIER_KEY' . " === \$carrier_key &&" ), 'Russian Post documents button must use generic document actions and must not be hardcoded in the metabox; document_actions is the canonical payload key.' );
...
russian_post_cancel_smoke_assert( str_contains( $metabox_source, 'data-wdc-copy-tracking' ) && str_contains( $metabox_source, 'aria-label="' ) && str_contains( $metabox_source, '🗐' ) && ! str_contains( $metabox_source, '<svg' ) && ! str_contains( $metabox_source, 'fa-light fa-copy' ) && str_contains( $js_source, 'copyText' ), 'Metabox must provide accessible symbol copy tracking button without SVG or Font Awesome dependency.' );
russian_post_cancel_smoke_assert( str_contains( $css_source, '.wdc-copy-tracking-icon' ) && str_contains( $css_source, 'background: transparent' ) && str_contains( $css_source, 'font-size: 15px' ), 'Copy tracking button must have compact symbol CSS.' );
...
russian_post_cancel_smoke_assert( str_contains( $metabox_source, "\$show_cancel = ! empty( \$button_policy['show_cancel'] )" ) && str_contains( $metabox_source, "\$show_remove = ! empty( \$button_policy['show_remove'] )" ), 'Metabox must split cancel and remove button visibility.' );
russian_post_cancel_smoke_assert( str_contains( $js_source, 'requestShipmentRemoveFromOrder' ) && str_contains( $js_source, 'resetShipmentUi(box, shipmentStatusFromResponse(payload.data))' ) && str_contains( $js_source, 'removeFromOrderAction' ), 'JS must remove shipment from order and reset UI on success.' );
russian_post_cancel_smoke_assert( str_contains( $metabox_source, '$show_primary_actions' ) && str_contains( $metabox_source, "\$show_update = ! empty( \$button_policy['show_update'] )" ), 'Metabox must compute primary and update button visibility.' );
russian_post_cancel_smoke_assert( str_contains( $metabox_source, 'data-wdc-open-shipment-modal' ) && str_contains( $metabox_source, '$show_primary_actions ?' ) && str_contains( $metabox_source, 'data-wdc-open-manual-tracking' ) && str_contains( $metabox_source, '$show_manual_attach ?' ), 'Initial state must show prepare/manual actions and hide inactive actions.' );
russian_post_cancel_smoke_assert( str_contains( $metabox_source, 'data-wdc-update-shipment-status' ) && str_contains( $metabox_source, '<?php echo $show_update ?' ), 'Created shipment state must show update status action.' );
russian_post_cancel_smoke_assert( str_contains( $js_source, 'setVisible(openButton, canCreate)' ) && str_contains( $js_source, 'setVisible(updateButton, canUpdate)' ) && str_contains( $js_source, 'setVisible(manualButton, canAttachManual)' ), 'JS must hide inactive primary/update/manual buttons.' );
russian_post_cancel_smoke_assert( str_contains( $js_source, 'setVisible(removeButton, canRemove)' ), 'JS must show remove button only when tracking exists and cancel is unavailable.' );
