...
modal_ext_assert( preg_match( '/#poststuff\s+\.wdc-shipment-modal__title\s*\{[^}]*margin:\s*0 44px 18px 0;[^}]*padding:\s*0;[^}]*color:\s*#1d2327;[^}]*font-size:\s*24px;[^}]*line-height:\s*1\.3;[^}]*font-weight:\s*600;[^}]*\}/s', $shipment_css ) === 1, 'Shipment modal title CSS must beat the WordPress #poststuff h2 rule while preserving the production visual contract.' );
modal_ext_assert( preg_match( '/(^|})\s*\.wdc-shipment-modal__title\s*\{/m', $shipment_css ) !== 1 && preg_match( '/#poststuff\s+\.wdc-shipment-modal__title\s*\{[^}]*!important/si', $shipment_css ) !== 1, 'Shipment modal title must not retain the insufficient standalone selector or use !important.' );
modal_ext_assert( preg_match( '/(^|})\s*h2\s*\{/m', $shipment_css ) !== 1 && ! str_contains( $shipment_css, '.wdc-shipment-modal h2' ) && preg_match( '/#poststuff\s+\.wdc-shipment-modal__title\s*\{[^}]*white-space\s*:\s*nowrap/si', $shipment_css ) !== 1, 'Shipment title styling must not add global/modal-wide h2 rules or disable responsive wrapping.' );
...
modal_ext_assert( str_contains( $render_source, '$create_initially_disabled = OzonDeliverySettings::CARRIER_KEY === $carrier_key && DeliveryType::COURIER === $delivery_type && empty( $modal_extension_context[\'normalization_valid\'] );' ) && str_contains( $render_source, 'disabled( $create_initially_disabled )' ), 'Metabox must render Ozon courier Create initially disabled until a normalized address exists.' );
