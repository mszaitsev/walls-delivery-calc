...
carrier_adapter_registry_assert( str_contains( $plugin_source, 'new ShipmentCreationService( $this->container->get( OrderShipmentRepository::class ), array( $this->container->get( RussianPostShipmentAdapter::class ), $this->container->get( CdekShipmentAdapter::class ), $this->container->get( DpdShipmentAdapter::class ), $this->container->get( YandexShipmentAdapter::class ) )' ) && str_contains( $plugin_source, 'array( $this->container->get( RussianPostShipmentPersistenceMapper::class ), $this->container->get( CdekShipmentPersistenceMapper::class ), $this->container->get( DpdShipmentPersistenceMapper::class ), $this->container->get( YandexShipmentPersistenceMapper::class ) )' ) && ! str_contains( $plugin_source, 'new ShipmentCreationService( $this->container->get( OrderShipmentRepository::class ), array( $this->container->get( RussianPostShipmentAdapter::class ), $this->container->get( CdekShipmentAdapter::class ), $this->container->get( DpdShipmentAdapter::class ), $this->container->get( YandexShipmentAdapter::class ) ), $this->container->get( Logger::class ), $this->container->get( CarrierShipmentAdapterRegistry::class ), null' ), 'ShipmentCreationService live-create wiring must include all adapters, all persistence mappers and no mapper placeholder.' );
...
carrier_adapter_registry_assert( str_contains( $ajax_service_source, '$adapter->update_status' ) && str_contains( $ajax_service_source, '$adapter->cancel_in_carrier' ) && str_contains( $ajax_service_source, '$adapter->remove_from_order' ) && str_contains( $ajax_service_source, '$adapter->attach_manual' ), 'Shipment AJAX actions must delegate to carrier adapters.' );
carrier_adapter_registry_assert( ! str_contains( $ajax_service_source, 'CdekSettings::CARRIER_KEY === $shipment_key && $this->cdek_status_updates' ), 'Shipment AJAX actions must not keep the old CDEK direct dispatch branch.' );
carrier_adapter_registry_assert( str_contains( $ajax_service_source, 'private function carrier_ui_payload' ) && 5 <= substr_count( $ajax_service_source, '$this->carrier_ui_payload(' ), 'Shipment AJAX actions must return one fresh carrier UI payload after create/update/cancel/remove/manual attach.' );
carrier_adapter_registry_assert( str_contains( $ajax_service_source, "'has_shipment' => ! empty( \$status['has_shipment'] )" ) && str_contains( $ajax_service_source, "'can_update_status' => ! empty( \$status['can_update_status'] )" ) && str_contains( $ajax_service_source, "'can_cancel' => ! empty( \$status['can_cancel'] )" ) && str_contains( $ajax_service_source, "'can_remove_from_order' => ! empty( \$status['can_remove_from_order'] )" ), 'Carrier UI payload must expose normalized button flags for JS.' );
