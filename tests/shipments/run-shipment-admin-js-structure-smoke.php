...
shipment_admin_js_structure_assert( str_contains( $source['yandex'], 'cancellationPollingProgressMessage' ) && str_contains( $source['yandex'], 'cancellationPollingExhaustedMessage' ) && str_contains( $source['yandex'], 'function isCancellationPollingPending' ) && str_contains( $source['yandex'], 'yandex_self_pickup_node_code' ) && str_contains( $source['yandex'], 'renderStatus' ) && str_contains( $source['yandex'], 'handlePollingStatus' ) && str_contains( $source['yandex'], 'handlePollingExhausted' ), 'Yandex extension must own cancellation polling and self-pickup status presentation.' );
shipment_admin_js_structure_assert( str_contains( $source['yandex'], 'const rawStatus = String(statusPayload.yandex_status || \'\').trim();' ) && ! str_contains( $source['yandex'], 'statusPayload.yandex_status || statusPayload.carrier_status_title' ), 'Yandex cancellation polling must not treat carrier_status_title as terminal failure status.' );
