...
shipment_admin_js_structure_assert( ! preg_match( '/\\b(cdek|dpd|russian|yandex|ozon|jet)\\b/i', $source['polling'] ), 'Polling module must remain carrier-neutral.' );
shipment_admin_js_structure_assert( ! preg_match( '/\\b(cdek|dpd|russian|yandex|ozon|jet)\\b/i', $source['status'] ), 'Status module must remain carrier-neutral.' );
shipment_admin_js_structure_assert( str_contains( $source['status'], 'function resetShipmentUi(box, statusPayload)' ) && str_contains( $source['status'], 'shipmentButtonStateFromStatus(statusPayload)' ), 'Resetting shipment UI must accept authoritative server status payload for button capabilities.' );
shipment_admin_js_structure_assert( str_contains( $source['polling'], 'resetShipmentUi(box, shipmentStatusFromResponse(payload.data))' ), 'Remove/cancel flows must preserve server capability payload when resetting shipment UI.' );
shipment_admin_js_structure_assert( str_contains( $source['status'], 'autoUpdateStatusAfterManualAttach' ) && str_contains( $source['polling'], 'getPresentation(box).autoUpdateStatusAfterManualAttach === \'1\'' ) && str_contains( $source['polling'], 'requestShipmentStatus(updateButton, { auto: true }).catch(function () {})' ), 'Manual attach must trigger optional first status refresh through the generic presentation capability.' );
...
shipment_admin_js_structure_assert( str_contains( $source['ozon'], 'data-wdc-ozon-place-limits' ) && str_contains( $source['ozon'], 'createAvailability' ) && str_contains( $source['ozon'], 'dimensionsFit' ) && str_contains( $source['ozon'], 'hasConfirmedOzonCourierAddress' ) && str_contains( $source['ozon'], 'validateOzonCourierAddress' ) && str_contains( $source['ozon'], 'Адрес для Ozon подтвержден.' ) && ! str_contains( $source['ozon'], 'Адрес Ozon подтвержден.' ), 'Ozon Delivery extension must own Ozon pickup point limit validation, courier address create availability hook, and post-analysis success wording.' );
...
shipment_admin_js_structure_assert( str_contains( $source['allocation'], "form.querySelectorAll('[data-wdc-courier-section]')" ) && str_contains( $source['allocation'], "form.querySelectorAll('[data-wdc-pickup-section]')" ), 'Scenario section toggling must support multiple pickup/courier layout sections.' );
...
$busy_function_pos = strpos( $source['events'], 'function setShipmentCreateBusy' );
$busy_function     = false !== $busy_function_pos ? substr( $source['events'], $busy_function_pos, 1200 ) : '';
shipment_admin_js_structure_assert( str_contains( $busy_function, 'const form = findShipmentForm(button);' ) && str_contains( $busy_function, 'updateCreateAvailability(form);' ) && str_contains( $busy_function, 'button.disabled = false;' ) && strpos( $busy_function, 'updateCreateAvailability(form);' ) < strpos( $busy_function, 'button.disabled = false;' ), 'Create busy release must delegate final enabled/disabled state to canonical availability before using the defensive no-form fallback.' );
...
$address_reset_pos = strpos( $source['events'], "event.target.matches('[data-wdc-courier-original-address]')" );
$address_reset_block = false !== $address_reset_pos ? substr( $source['events'], $address_reset_pos, 900 ) : '';
shipment_admin_js_structure_assert( false !== $address_reset_pos && str_contains( $address_reset_block, "dispatchShipmentCarrierHook('afterAddressReset'" ) && ! str_contains( $address_reset_block, 'ozon_courier_apartment' ) && ! str_contains( $address_reset_block, 'ozon_courier_entrance' ) && ! str_contains( $address_reset_block, 'ozon_courier_floor' ) && ! str_contains( $address_reset_block, 'ozon_courier_intercom' ), 'Courier address normalization reset must be driven by original address edits, not optional Ozon apartment/entrance/floor/intercom edits.' );
...
shipment_admin_js_structure_assert( strpos( $source['yandex'], 'if (isCancellationTerminalWithoutShipment(statusPayload))' ) > strpos( $source['yandex'], 'if (isCancellationConfirmed(context, statusPayload))' ) && strpos( $source['yandex'], 'if (isCancellationTerminalWithoutShipment(statusPayload))' ) < strpos( $source['yandex'], 'if (rawStatus && rawStatus !== \'CANCELLED\')' ) && str_contains( $source['yandex'], 'statusPayload.has_shipment === false' ) && str_contains( $source['yandex'], 'statusPayload.polling_continue !== true' ) && str_contains( $source['yandex'], 'statusPayload.cancellation_pending !== true' ), 'Yandex terminal no-shipment cancellation polling response must finish the toast before the progress fallback can run.' );
shipment_admin_js_structure_assert( str_contains( $source['yandex'], 'function hasCancellationPollingToast' ) && str_contains( $source['yandex'], 'const hasToast = hasCancellationPollingToast(context && context.box);' ) && str_contains( $source['yandex'], 'if (!isYandexPollingContext(context) && !hasToast) return false;' ) && str_contains( $source['yandex'], 'if (!hasToast && !isCancellationPollingPurpose(settings.purpose || settings.mode)) return false;' ), 'Yandex cancelled_and_removed hook must finish an existing cancellation toast even after resetShipmentUi removes status payload identity or terminal purpose metadata.' );
