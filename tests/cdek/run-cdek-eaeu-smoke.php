...
cdek_eaeu_assert( str_contains( $dpd_import, 'process_foreign_row' ) && str_contains( $dpd_import, "array( 'AM', 'BY', 'KZ', 'KG' )" ) && str_contains( $dpd_import, 'save_dpd_city_id' ), 'DPD geography import must create foreign locations and DPD city mappings.' );
...
cdek_eaeu_assert( str_contains( $builder, "\$recipient['tin']" ) && str_contains( $builder, "'KZ' ===" ) && ! str_contains( $builder, 'passport_number' ), 'CDEK builder must map recipient document only to recipient.tin and require it for KZ.' );
...
cdek_eaeu_assert( str_contains( $modal, 'data-wdc-cdek-recipient-document' ) && str_contains( $js, 'updateCdekRecipientDocumentUi' ) && str_contains( $js, "country === 'KZ'" ) && ! str_contains( $js, 'localStorage' ) && ! str_contains( $js, 'sessionStorage' ), 'CDEK modal document field must be CDEK-only, KZ-aware, and page-memory only.' );
