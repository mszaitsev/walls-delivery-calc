...
cdek_eaeu_assert( str_contains( $checkout_js, "centerSource: centerSource || ''" ) && str_contains( $checkout_js, 'centerTrusted: centerTrusted === true' ) && str_contains( $checkout_js, "'point_search', false" ), 'Frontend pickup prefetch must carry center provenance and mark point-search centers as untrusted.' );
cdek_eaeu_assert( str_contains( $checkout_js, '!validCoordinate(points[0].lat, points[0].lng)' ) && str_contains( $checkout_js, 'if (!validCoordinate(lat, lng))' ), 'Frontend pickup prefetch must reject invalid text-search coordinates before building a bbox request.' );
...
cdek_eaeu_assert( str_contains( $pickup_map_js, 'userViewportInteracted' ) && str_contains( $pickup_map_js, 'markUserViewportInteraction' ) && str_contains( $pickup_map_js, 'programmaticBoundsSuppressed' ) && str_contains( $pickup_map_js, 'scheduleProgrammaticBoundsRelease' ) && ! str_contains( $pickup_map_js, 'suppressedBoundsLoads' ), 'Pickup map must suppress programmatic bounds before debounce and unblock the first user pan.' );
cdek_eaeu_assert( str_contains( $pickup_map_js, 'function claimViewportForExplicitAction()' ) && str_contains( $pickup_map_js, 'loadBounds(bboxAround(searchAddress.lat, searchAddress.lng), { force: true })' ) && str_contains( $pickup_map_js, 'Array.isArray(result.points)' ), 'Pickup map explicit address/geolocation actions must claim viewport and load or render points directly.' );
cdek_eaeu_assert( 1 === preg_match( '/function commit\(point, options\) \{[\s\S]*?claimViewportForExplicitAction\(\);[\s\S]*?if \(options\.focus !== false && provider\.focusPoint\) \{[\s\S]*?beginProgrammaticBoundsSuppression\(\);[\s\S]*?provider\.focusPoint\(point\);/m', $pickup_map_js ), 'Pickup map commit must always claim viewport before optional focus suppression.' );
...
cdek_eaeu_assert( str_contains( $pickup_map_js, 'lat < -90 || lat > 90 || lng < -180 || lng > 180' ) && str_contains( $pickup_map_js, 'Math.abs(lat) < 0.000001 && Math.abs(lng) < 0.000001' ), 'Pickup map coordinate validation must reject out-of-range and 0,0 coordinates.' );
cdek_eaeu_assert( str_contains( $leaflet_provider_js, 'lastPoints.filter(validPointCoordinates)' ) && str_contains( $leaflet_provider_js, 'maxZoom = Number(options.maxZoom || 14)' ) && str_contains( $leaflet_provider_js, 'map.fitBounds(bounds' ) && str_contains( $leaflet_provider_js, 'cancelPendingFit: function () {}' ), 'Leaflet provider fitToMarkers must use raw point coordinates with padding/maxZoom and expose cancelPendingFit.' );
cdek_eaeu_assert( str_contains( $yandex_provider_js, 'pendingFitOptions' ) && str_contains( $yandex_provider_js, 'cancelPendingFit' ) && str_contains( $yandex_provider_js, 'applyPendingFit()' ) && str_contains( $yandex_provider_js, 'pendingPoints.filter(validPointCoordinates)' ) && str_contains( $yandex_provider_js, 'map.setBounds' ), 'Yandex provider fitToMarkers must keep cancellable pending fit requests and use raw point coordinates.' );
...
cdek_eaeu_assert( str_contains( $modal, '$recipient_country = strtoupper' ) && str_contains( $modal, 'recipient_document_visible' ) && str_contains( $modal, 'recipient_document_help' ) && ! str_contains( $modal, 'data-wdc-cdek-recipient-document-row hidden' ) && str_contains( $modal, 'disabled( ! $document_visible )' ), 'CDEK modal must render initial recipient document visibility from recipient_country.' );
cdek_eaeu_assert( str_contains( $modal, '<input type="hidden" value="<?php echo esc_attr( $recipient_country ); ?>" data-wdc-cdek-recipient-country>' ) && ! str_contains( $modal, 'name="recipient_location_country" value="<?php echo esc_attr( $recipient_country ); ?>" data-wdc-cdek-recipient-country' ), 'CDEK recipient country marker must not duplicate submitted recipient_location_country.' );
cdek_eaeu_assert( str_contains( $js, "fieldValue(form, '[data-wdc-cdek-recipient-country]') || fieldValue(form, 'input[name=\"recipient_location_country\"]')" ) && str_contains( $js, 'afterFormInitialized' ) && str_contains( $js, 'updateCdekRecipientDocumentUi(form);' ), 'CDEK JS must sync recipient document UI on form initialization before preview responses.' );
cdek_eaeu_assert( str_contains( $shipment_picker_js, 'form.querySelectorAll(\'[name="\' + name + \'"]\')' ) && str_contains( $shipment_picker_js, "form.querySelectorAll('[data-wdc-cdek-recipient-country]')" ), 'Shipment picker must synchronize all duplicate country fields and CDEK country marker.' );
