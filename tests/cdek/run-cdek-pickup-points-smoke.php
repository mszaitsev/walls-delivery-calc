...
cdek_pickup_assert( ! str_contains( $admin_js, "Ищем ПВЗ СДЭК" ) && str_contains( $admin_js, 'status.textContent = \'Ищем адрес через DaData...\'' ) && str_contains( $admin_js, 'geocodeAddress( box, value )' ), 'Admin recalculation JS must use the shared DaData address search path for CDEK too.' );
