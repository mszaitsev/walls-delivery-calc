...
cdek_pickup_assert( str_contains( $map_js, 'function pointMatchKeys(point)' ) && str_contains( $map_js, 'point && point.cdek_code' ) && str_contains( $map_js, 'point && point.delivery_point' ) && str_contains( $map_js, 'hasSharedPointKey(keys, pointMatchKeys(item))' ) && str_contains( $map_js, 'previewPoint = matchingPreviewPoint' ), 'Checkout selected CDEK pickup preview must match by CDEK code/delivery_point even when REST ids differ.' );
