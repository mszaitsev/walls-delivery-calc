...
cdek_tariff_assert( array() === $GLOBALS['wdc_cdek_tariff_logs'], 'Successful CDEK location resolution and filtering must not emit production logs.' );
...
cdek_tariff_assert( array() === $GLOBALS['wdc_cdek_tariff_logs'], 'Missing CDEK credentials are an expected precondition and must not emit a production warning.' );
...
cdek_tariff_assert( array() === $GLOBALS['wdc_cdek_tariff_logs'], 'A valid CDEK response with no matching tariff is a normal business result and must not be logged.' );
...
cdek_tariff_assert( 1 === count( $GLOBALS['wdc_cdek_tariff_logs'] ), 'A technical CDEK API failure must emit one primary warning without a duplicate empty-result warning.' );
