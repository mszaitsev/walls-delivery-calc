...
cdek_order_assert( is_string( $shipments_js ) && str_contains( $shipments_js, 'markCdekBarcodeDownloadBusy' ) && str_contains( $shipments_js, 'Формируем этикетку...' ) && str_contains( $shipments_js, 'aria-disabled' ) && str_contains( $shipments_js, '15000' ), 'CDEK label download click must show a temporary spinner/disabled waiting state.' );
...
cdek_order_assert( str_contains( $metabox_source, 'ACTION_CDEK_BARCODE_PDF' ) && str_contains( $metabox_source, 'admin_post_cdek_barcode_pdf' ) && str_contains( $metabox_source, 'data-wdc-cdek-barcode-download' ) && str_contains( $metabox_source, 'Скачать этикетку' ) && str_contains( $metabox_source, 'Открыть этикетку' ), 'CDEK shipment metabox must expose renamed label download/open admin-post actions.' );
cdek_order_assert( str_contains( $metabox_source, "target=\"_blank\"" ) && str_contains( $metabox_source, "'mode' => 'inline'" ) && ! str_contains( $metabox_source, 'data-wdc-cdek-barcode-inline download' ), 'CDEK label inline link must open in a new tab without a download attribute.' );
