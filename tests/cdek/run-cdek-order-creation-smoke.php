...
cdek_order_assert( is_string( $shipments_js ) && str_contains( $shipments_js, 'label_actions' ) && str_contains( $shipments_js, 'data-wdc-shipment-document-download' ) && str_contains( $shipments_js, 'data-wdc-cdek-barcode-download' ) && ! str_contains( $shipments_js, 'canPrintBarcode' ) && ! str_contains( $shipments_js, 'data-wdc-cdek-barcode-inline' ), 'Shipment JS must toggle document buttons through normalized label_actions while CDEK extension owns the label download click.' );
