...
self_pickup_assert( ! $policy->calculate( true, 349999, $discount_settings )->eligible, 'Discount threshold must not round 3499.99 up to eligibility.' );
...
self_pickup_assert( ! $policy->calculate( false, 450000, $discount_settings )->eligible, 'Discount policy must require selected self-pickup identity.' );
self_pickup_assert( ! $policy->calculate( true, 450000, array_merge( $discount_settings, array( 'enabled' => false ) ) )->eligible, 'Discount policy must respect disabled setting.' );
...
self_pickup_assert( 'Склад' === $settings->address( (int) $service->id ) && "Пн-Пт\n10:00-19:00" === $settings->working_hours( (int) $service->id ), 'Self-pickup settings must sanitize address and preserve multiline working hours.' );
...
self_pickup_assert( true === ( $draft['modal_capabilities']['non_shipment'] ?? false ) && 'Самовывоз покупателем' === ( $draft['request']['meta']['non_shipment_state']['message'] ?? '' ), 'Non-shipment draft payload must carry static informational state and zero modal requirements.' );
