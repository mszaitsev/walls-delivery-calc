...
yandex_source_assert( str_contains( $address_controller_source, 'purpose' ) && str_contains( $address_controller_source, "'source_dropoff'" ) && str_contains( $address_controller_source, 'source_dropoff_map_points_by_geo_ids' ) && str_contains( $address_controller_source, 'search_source_dropoff_points_near' ) && str_contains( $address_controller_source, 'yandex_source_dropoff_ajax_row' ), 'Shipment modal pickup search endpoint must expose scoped initial and nearby Yandex source dropoff points through the shared map search action.' );
yandex_source_assert( str_contains( $address_controller_source, 'geo_ids_for_location' ) && str_contains( $address_controller_source, 'source_location_id' ) && str_contains( $address_controller_source, 'Не удалось определить область поиска ПВЗ.' ) && ! str_contains( $address_controller_source, "search_source_dropoff_points(\n\t\t\t\tarray(" ), 'Shipment modal Yandex source search must use source_location_id/geo ids or nearby coordinates instead of global first-page dropoff search.' );
yandex_source_assert( str_contains( $create_controller_source, 'validate_yandex_source_station' ) && str_contains( $preview_controller_source, 'validate_yandex_source_station' ) && str_contains( $create_controller_source, 'ПВЗ отправления Яндекс не найден.' ) && str_contains( $create_controller_source, 'Выбранный ПВЗ Яндекс не принимает отправления.' ) && str_contains( $create_controller_source, 'Выбранный ПВЗ Яндекс сейчас недоступен.' ), 'Preview/create validation must reject forged Yandex source overrides before shipment API calls.' );
yandex_source_assert( ! str_contains( $address_controller_source, "update_option( 'yandex_source" ) && ! str_contains( $address_controller_source, "update_post_meta( \$order_id, '_wdc_yandex_source" ), 'Temporary source dropoff selector must not persist override into settings or order meta.' );
