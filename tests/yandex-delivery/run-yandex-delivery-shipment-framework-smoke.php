...
yd_framework_assert( str_contains( $js_source, 'const cancellationPollingToasts = new WeakMap()' ) && str_contains( $js_source, 'function initCancellationPollingToast' ) && str_contains( $js_source, 'cancellationPollingProgressMessage(0, maxAttempts)' ) && str_contains( $js_source, 'проверок отмены' ) && str_contains( $js_source, 'updateCancellationPollingToast(' ) && str_contains( $js_source, 'Отмена не выполнена. Получен статус Яндекс:' ) && str_contains( $js_source, 'cancellationPollingExhaustedMessage(context.attempt' ) && str_contains( $js_source, 'clearCancellationPollingToast(context && context.box)' ), 'Runtime Yandex cancellation polling must use one token-bound extension-owned progress toast, update attempt/max text, handle terminal/exhaustion states and clear progress on local remove.' );
