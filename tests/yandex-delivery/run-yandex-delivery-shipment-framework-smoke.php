...
yd_framework_assert( str_contains( $ajax_service_source, 'shipment_preview_validation_failed' ) && str_contains( $ajax_service_source, 'shipment_preview_unexpected_error' ) && str_contains( $ajax_service_source, 'discard_preview_buffer' ), 'Shipment preview AJAX must return controlled JSON errors instead of leaking HTML output.' );
yd_framework_assert( str_contains( $metabox_source, 'data-wdc-requires-successful-preview' ) && str_contains( $ajax_service_source, 'shipment_create_validation_failed' ) && str_contains( $ajax_service_source, 'shipment_create_unexpected_error' ) && str_contains( $ajax_service_source, 'public_shipment_error_message' ), 'Shipment create AJAX must be JSON-safe and expose the preview-required capability to runtime.' );
yd_framework_assert( str_contains( $ajax_service_source, 'AddressSuggestionService' ) && str_contains( $ajax_service_source, 'normalize_yandex_courier_address' ) && str_contains( $ajax_service_source, "'dadata+yandex'" ) && str_contains( $yandex_modal_extension_source, 'Проверить адрес' ) && str_contains( $yandex_modal_extension_source, 'data-wdc-yandex-address-field="street"' ) && str_contains( $yandex_modal_extension_source, 'Проверьте адрес доставки через DaData' ), 'Yandex courier modal extension must use shared DaData suggestions through the existing shipment normalize endpoint and block preview until explicit verification.' );
...
yd_framework_assert( str_contains( $ajax_service_source, 'shipment_attach_validation_failed' ) && str_contains( $ajax_service_source, 'shipment_attach_unexpected_error' ) && str_contains( $ajax_service_source, "'request_id' => \$barcode" ), 'Generic manual attach AJAX must stay JSON-safe and pass request_id alias to carrier adapters.' );
yd_framework_assert( str_contains( $metabox_source, 'wdc_mark_shipment_poll_exhausted' ) && str_contains( $ajax_service_source, 'ajax_mark_poll_exhausted' ) && str_contains( $ajax_service_source, 'mark_polling_exhausted' ), 'Shared metabox must expose a carrier-neutral AJAX hook to persist exhausted registration polling.' );
...
yd_framework_assert( str_contains( $js_source, 'function renderYandexSelfPickupCode' ) && str_contains( $js_source, 'data-wdc-yandex-self-pickup-code' ) && str_contains( $js_source, 'label_actions' ) && str_contains( $js_source, 'data-wdc-shipment-document-download' ) && str_contains( $js_source, 'function requestYandexLabelDownload' ), 'Runtime shipment renderer must update Yandex pickup code and drive label visibility through normalized document actions.' );
...
yd_framework_assert( str_contains( $js_source, 'const cancellationPollingToasts = new WeakMap()' ) && str_contains( $js_source, 'function isYandexPollingContext' ) && str_contains( $js_source, 'button.dataset.shipmentKey' ) && str_contains( $js_source, "trim() === 'yandex_delivery'" ) && str_contains( $js_source, 'function initCancellationPollingToast' ) && str_contains( $js_source, 'cancellationPollingProgressMessage(0, maxAttempts)' ) && str_contains( $js_source, 'проверок отмены' ) && str_contains( $js_source, 'updateCancellationPollingToast(' ) && str_contains( $js_source, 'Отмена не выполнена. Получен статус Яндекс:' ) && str_contains( $js_source, 'cancellationPollingExhaustedMessage(context.attempt' ) && str_contains( $js_source, 'clearCancellationPollingToast(box)' ), 'Runtime Yandex cancellation polling must be carrier-scoped, use one token-bound extension-owned progress toast, update attempt/max text, handle terminal/exhaustion states and clear progress on local remove.' );
...
$ajax_service_reflection = new ReflectionClass( \WallsShop\WDC\Shipments\Admin\Ajax\ShipmentAdminAjaxService::class );
$ajax_service_without_constructor = $ajax_service_reflection->newInstanceWithoutConstructor();
$locality_method = $ajax_service_reflection->getMethod( 'yandex_locality_from_normalized_item' );
$locality_method->setAccessible( true );
yd_framework_assert( 'Москва' === $locality_method->invoke( $ajax_service_without_constructor, array( 'data' => array( 'region_with_type' => 'г Москва', 'street_with_type' => 'Ходынский бульвар', 'house' => '9' ) ) ), 'Yandex DaData locality must resolve federal city Moscow when city fields are absent.' );
yd_framework_assert( 'Новосибирск' === $locality_method->invoke( $ajax_service_without_constructor, array( 'data' => array( 'city_with_type' => 'г Новосибирск', 'region_with_type' => 'Новосибирская обл', 'street_with_type' => 'Красный проспект', 'house' => '10' ) ) ), 'Yandex DaData locality must resolve a normal city from city_with_type.' );
yd_framework_assert( 'Кольцово' === $locality_method->invoke( $ajax_service_without_constructor, array( 'data' => array( 'settlement_with_type' => 'рп Кольцово', 'city' => '', 'region_with_type' => 'Новосибирская обл', 'street_with_type' => 'Технопарковая ул', 'house' => '1' ) ) ), 'Yandex DaData locality must resolve settlements from settlement fields.' );
yd_framework_assert( 'Химки' === $locality_method->invoke( $ajax_service_without_constructor, array( 'locality' => 'Химки', 'data' => array( 'city' => '', 'region_with_type' => 'Московская обл', 'street_with_type' => 'Ленинградская ул', 'house' => '1' ) ) ), 'Yandex DaData locality must use canonical normalized locality even when raw data city is absent.' );
yd_framework_assert( '' === $locality_method->invoke( $ajax_service_without_constructor, array( 'data' => array( 'city' => '', 'settlement' => '', 'region_with_type' => 'Московская обл', 'street_with_type' => 'Ленина ул', 'house' => '1' ) ) ), 'Yandex DaData locality must not substitute ordinary region as locality.' );
