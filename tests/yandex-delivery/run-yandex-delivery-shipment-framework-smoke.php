...
yd_framework_assert( preg_match( "/cancelledAndRemoved:\\s*function \\(context\\) \\{\\s*const hasToast = hasCancellationPollingToast\\(context && context\\.box\\);[\\s\\S]+?if \\(!isYandexPollingContext\\(context\\) && !hasToast\\) return false;[\\s\\S]+?if \\(!hasToast && !isCancellationPollingPurpose\\(settings\\.purpose \\|\\| settings\\.mode\\)\\) return false;[\\s\\S]+?finishCancellationPollingToast\\(context\\.box, 'Отправление Яндекс отменено\\.', 'success'\\);/s", $js_source ) === 1, 'Yandex cancelled_and_removed must finish an existing progress toast even when resetShipmentUi has already cleared status identity or terminal purpose metadata.' );
...
yd_framework_assert( ! empty( $override_nonterminal_result['success'] ) && empty( $override_nonterminal_result['cancelled_and_removed'] ) && 'cancellation_started' === (string) ( $override_nonterminal_shipment['status'] ?? '' ) && DeliveryStatus::IN_TRANSIT === (string) ( $override_nonterminal_shipment['universal_status_code'] ?? '' ) && ! empty( $override_nonterminal_shipment['yandex_cancel_requested'] ) && 1 === count( $override_nonterminal_http->requests ), 'Admin override DELIVERY_DELIVERED -> in_transit must keep cancellation polling pending without cancelled_and_removed and call only request/info.' );
