...
yd_framework_assert( str_contains( $js_source, 'const cancellationPollingToasts = new WeakMap()' ) && str_contains( $js_source, 'function initCancellationPollingToast' ) && str_contains( $js_source, 'cancellationPollingProgressMessage(0, maxAttempts)' ) && str_contains( $js_source, 'проверок отмены' ) && str_contains( $js_source, 'updateCancellationPollingToast(box, settings.pollingToken, cancellationPollingProgressMessage(settings.attempt' ) && str_contains( $js_source, 'Отмена не выполнена. Получен статус Яндекс:' ) && str_contains( $js_source, 'cancellationPollingExhaustedMessage(attempts' ) && str_contains( $js_source, 'clearCancellationPollingToast(box)' ), 'Runtime Yandex cancellation polling must use one token-bound progress toast, update attempt/max text, handle terminal/exhaustion states and clear progress on local remove.' );
