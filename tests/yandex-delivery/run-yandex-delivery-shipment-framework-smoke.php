...
yd_framework_assert( str_contains( $js_source, 'function renderYandexSelfPickupCode' ) && str_contains( $js_source, 'data-wdc-yandex-self-pickup-code' ) && str_contains( $js_source, 'documentActions' ) && str_contains( $js_source, 'label_actions' ) && str_contains( $js_source, 'data-wdc-shipment-document-download' ) && str_contains( $js_source, 'function requestYandexLabelDownload' ), 'Runtime shipment renderer must update Yandex pickup code and drive document visibility through normalized document actions while preserving the label_actions payload key.' );
