...
yd_geo_batch_assert( str_contains( $admin_source, "\$tabs['yandex_delivery_geo'] = 'Маппинг geo_id';" ) && ! str_contains( $admin_source, "\$tabs['yandex_delivery_geo_batch']" ) && str_contains( $admin_source, 'start_yandex_delivery_geo_batch' ) && str_contains( $admin_source, 'run_yandex_delivery_geo_batch_step' ), 'Admin UI must keep Yandex geo batch actions inside consolidated mapping tab.' );
yd_geo_batch_assert( str_contains( $admin_source, 'Массовый маппинг' ) && str_contains( $admin_source, 'Запустить полный маппинг' ) && str_contains( $admin_source, 'Обработать тех.ошибки' ) && str_contains( $admin_source, 'Выполнить шаг' ) && str_contains( $admin_source, 'Пауза' ) && str_contains( $admin_source, 'Сбросить прогресс' ) && ! str_contains( $admin_source, 'yandex_delivery_geo_batch_limit' ) && ! str_contains( $admin_source, 'yandex_delivery_geo_batch_size' ), 'Consolidated mapping tab must render production runner controls without legacy limit/batch_size fields.' );
yd_geo_batch_assert( str_contains( $admin_source, "'start_yandex_delivery_geo_batch', 'run_yandex_delivery_geo_batch_step', 'pause_yandex_delivery_geo_batch', 'reset_yandex_delivery_geo_batch' => 'yandex_delivery_geo'" ), 'Yandex geo batch actions must redirect back to the consolidated mapping tab.' );
