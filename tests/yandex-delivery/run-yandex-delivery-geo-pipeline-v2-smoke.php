...
yd_geo_pipeline_v2_assert( str_contains( $plugin_source, 'YandexDeliveryGeoPipelineV2Runner::CRON_HOOK' ) && str_contains( $plugin_source, 'run_scheduled_step' ), 'Plugin must register pipeline step cron hook.' );
	...
	yd_geo_pipeline_v2_assert( str_contains( $admin_source, 'Р Р°СЃРїРёСЃР°РЅРёРµ РїРѕР»РЅРѕРіРѕ РѕР±РЅРѕРІР»РµРЅРёСЏ' ) && str_contains( $admin_source, 'save_yandex_geo_pipeline_v2_schedule' ) && str_contains( $admin_source, 'Р’СЃРµ РІСЂРµРјСЏ СѓРєР°Р·С‹РІР°РµС‚СЃСЏ РїРѕ РњРѕСЃРєРІРµ (GMT+3)' ), 'Admin must expose pipeline schedule settings with Moscow time explanation.' );
	yd_geo_pipeline_v2_assert( str_contains( $pipeline_source, "DateTimeZone( 'Europe/Moscow' )" ) && str_contains( $pipeline_source, " . ' MSK'" ), 'Pipeline schedule must calculate and display runs in Moscow time.' );
	 yd_geo_pipeline_v2_assert( str_contains( $admin_source, 'РџРѕР»РЅРѕРµ РѕР±РЅРѕРІР»РµРЅРёРµ РЇРЅРґРµРєСЃ РџР’Р—/РіРµРѕРіСЂР°С„РёРё' ) && str_contains( $admin_source, 'wdc_yandex_delivery_geo_pipeline_v2_start' ) && str_contains( $admin_source, 'geoPipelineInitialState' ), 'Admin must expose one-button pipeline block and AJAX actions.' );
...
yd_geo_pipeline_v2_assert( str_contains( $plugin_main, 'Version: 0.128.2' ) && str_contains( $plugin_main, "define( 'WDC_VERSION', '0.128.2' )" ), 'Plugin version must be 0.128.2.' );
