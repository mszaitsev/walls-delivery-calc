...
plugin_architecture_assert( str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentAdapter::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentPersistenceMapper::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentDocumentProvider::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentModalExtension::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentPreflightQuoteService::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentService::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentStatusMapper::class' ) && str_contains( $ozon_plugin_source, 'CarrierShipmentAdapterRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentDocumentProviderRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentModalExtensionRegistry::class' ), 'Ozon shipments must be wired through existing Shipment Framework registries in Plugin.php.' );
...
plugin_architecture_assert( str_contains( $ozon_shipment_status_mapping_source, 'MOVING' ) && str_contains( $ozon_shipment_status_mapping_source, 'AT_THE_PICK_UP_POINT' ) && str_contains( $ozon_shipment_status_mapping_source, 'RECEIVED' ) && str_contains( $ozon_shipment_status_mapping_source, 'UTILIZATION' ) && str_contains( $ozon_shipment_status_mapping_source, 'UTILIZED' ) && str_contains( $ozon_shipment_status_mapping_source, 'WRITTEN_OFF' ) && str_contains( $ozon_shipment_status_mapping_source, 'LOOKING_FOR' ), 'Ozon status catalog must include official return statuses.' );
plugin_architecture_assert( str_contains( $ozon_shipment_status_mapper_source, 'OzonDeliverySettings $settings' ) && str_contains( $ozon_shipment_status_mapper_source, 'shipment_status_mapping' ) && str_contains( $ozon_shipment_status_mapper_source, 'save_from_admin' ) && str_contains( $ozon_shipment_service_source, '$this->status_mapper()->aggregate' ) && ! str_contains( $ozon_shipment_action_policy_source, 'DeliveryStatus::' ), 'Ozon runtime status mapping must use settings-backed mapper while action policy stays raw-status based.' );
...
plugin_architecture_assert( ! str_contains( $ozon_shipment_builder_source, "'wdc-'" ) && ! str_contains( $ozon_shipment_builder_source, "'wdc-order-'" ) && str_contains( $ozon_shipment_builder_source, 'get_order_number' ) && str_contains( $ozon_shipment_builder_source, 'posting_external_id( string $order_number' ), 'Ozon human external IDs must be based on WooCommerce order number and must not include technical wdc prefixes or creation UUIDs.' );
