...
plugin_architecture_assert( str_contains( $pek_cargo_builder_source, '$raw_volume_sum +=' ) && ! str_contains( $pek_cargo_builder_source, 'count( $request->places ) > 50' ), 'PEK cargo aggregate volume must sum raw transmitted place volumes before rounding and must not cap places at 50.' );
plugin_architecture_assert( str_contains( $pek_recipient_builder_source, "'title' => \$title" ) && str_contains( $pek_recipient_builder_source, "'person' => \$title" ) && str_contains( $pek_recipient_builder_source, "'personPhones' => array(" ) && str_contains( $pek_recipient_builder_source, "'individual' => array_filter" ) && str_contains( $pek_recipient_builder_source, "'addressStock'" ) && ! str_contains( $pek_recipient_builder_source, "'identityCard'" ) && ! str_contains( $pek_recipient_builder_source, "'passport'" ) && ! str_contains( $pek_recipient_builder_source, "'phone' => \$phone," ), 'PEK physical receiver must use documented title/person/individual/personPhones/addressStock shape without passport aliases or scalar phone.' );
plugin_architecture_assert( str_contains( $pek_parser_source, "'documentId'" ) && str_contains( $pek_parser_source, "'cargoCode'" ) && ! str_contains( $pek_parser_source, "'cargoBarCode'" ) && ! str_contains( $pek_parser_source, "'positionBarCodes'" ), 'PEK create parser must use preregistration response fixture fields, not cargos/status aliases.' );
...
plugin_architecture_assert( str_contains( $pek_destination_source, 'PekLocationResolver' ) && str_contains( $pek_destination_source, '$this->locations->resolve' ) && str_contains( $pek_destination_source, "'fresh_location_mapping'" ), 'PEK courier destination must fresh-resolve branch through PEK location mapping.' );
...
plugin_architecture_assert( str_contains( $pek_adapter_source, '$submitted' ) && str_contains( $pek_adapter_source, "error_code: 'pek_uncertain_submit'" ) && str_contains( $pek_adapter_source, 'private function safe_summary' ) && ! str_contains( $pek_adapter_source, "'address' =>" ), 'PEK post-submit parser failures must become uncertain and pending summary must exclude address/PII.' );
