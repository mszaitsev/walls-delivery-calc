...
plugin_architecture_assert( str_contains( $pickup_checkout_php_source, "'pickupRateCapabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_php_source, 'private function pickup_rate_capabilities()' ) && ! str_contains( $pickup_checkout_php_source, "'reload_on_viewport_change' => \$provider_capabilities" ) && str_contains( $pickup_checkout_rest_source, "'pickup_rate_capabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_rest_source, 'private function pickup_rate_capabilities()' ) && str_contains( $pickup_checkout_source, 'pickupRateCapabilities = normalizePickupRateCapabilities' ) && str_contains( $pickup_checkout_source, 'function mergePickupRateCapabilitiesFromResponse(response)' ) && str_contains( $pickup_checkout_source, 'function withRateCapabilities(context, method)' ) && str_contains( $pickup_checkout_source, 'withRateCapabilities(withPrefetch(withCarrierContext(resolvedContext, method)), method)' ) && str_contains( $pickup_map_source, 'function listFollowsViewport()' ) && str_contains( $pickup_map_source, 'if (!viewportReloadRequired() && visiblePoints.length)' ) && ! str_contains( $pickup_map_source, 'ozon_delivery' ) && ! str_contains( $pickup_checkout_source, "carrier === 'ozon_delivery'" ), 'Generic pickup map must keep active rate capabilities separate from destination context and preserve fixed-area viewport behavior without Ozon-specific JS branches.' );
