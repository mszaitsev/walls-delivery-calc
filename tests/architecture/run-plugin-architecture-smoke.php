...
$shipment_item_identity_source = plugin_architecture_source( 'src/Shipments/Application/ShipmentOrderItemIdentityResolver.php' );
...
$shipment_allocation_js_source = plugin_architecture_source( 'assets/admin/shipments/shipment-allocation.js' );
...
plugin_architecture_assert( str_contains( $ozon_api_client_source, 'function order_create' ) && str_contains( $ozon_api_client_source, "'Idempotency-Key' => \$idempotency_key" ) && str_contains( $ozon_api_client_source, 'function posting_approve' ) && str_contains( $ozon_api_client_source, 'function posting_info' ) && str_contains( $ozon_api_client_source, 'function posting_cancel' ) && str_contains( $ozon_api_client_source, 'function posting_label' ) && str_contains( $ozon_api_client_source, 'authorized_empty_success_request' ), 'Ozon shipment API client must expose only typed endpoint wrappers and accept empty successful approve/cancel responses.' );
plugin_architecture_assert( str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentAdapter::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentPersistenceMapper::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentDocumentProvider::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentModalExtension::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentService::class' ) && str_contains( $ozon_plugin_source, 'CarrierShipmentAdapterRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentDocumentProviderRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentModalExtensionRegistry::class' ), 'Ozon shipments must be wired through existing Shipment Framework registries in Plugin.php.' );
...
plugin_architecture_assert( str_contains( $shipment_metabox_source, "][order_item_id]" ) && str_contains( $shipment_allocation_js_source, "][order_item_id]" ) && str_contains( $shipment_modal_mapper_source, "'order_item_id'" ) && str_contains( $shipment_modal_mapper_source, "'split_parent'" ), 'Shipment modal item rows must expose canonical WooCommerce order_item_id separately from UI item_key and keep it through split rows.' );
plugin_architecture_assert( str_contains( $ozon_shipment_values_source, 'ShipmentOrderItemIdentityResolver' ) && str_contains( $ozon_shipment_values_source, "\$row['order_item_id']" ) && str_contains( $shipment_item_identity_source, "':split:'" ) && ! str_contains( $ozon_shipment_values_source, 'get_sku' ) && ! str_contains( $ozon_shipment_values_source, 'get_product_id' ), 'Ozon shipment item matching must prefer explicit order_item_id, keep legacy item_key fallback, and must not resolve order lines by product or SKU.' );
...
plugin_architecture_assert( str_contains( $ozon_shipment_values_source, 'get_total' ) && str_contains( $ozon_shipment_values_source, 'get_quantity' ) && str_contains( $ozon_shipment_values_source, 'MoneyParser::numeric_to_kopecks' ) && str_contains( $ozon_shipment_values_source, 'woocommerce_line_total_excluding_tax_discounted_prorated_by_quantity' ) && ! str_contains( $ozon_shipment_values_source, "['cost']" ), 'Ozon shipment declared value must be calculated server-side from WooCommerce order item totals, not browser-supplied prices.' );
