...
plugin_architecture_assert( str_contains( $pek_destination_request_source, 'coordinate_string' ) && str_contains( $pek_destination_request_source, 'sprintf( \'%.7F\'' ), 'PEK destination nearestdepartments request must stringify coordinates at the API boundary.' );
...
plugin_architecture_assert( str_contains( $pek_destination_store_source, "'failure_stage'" ) && str_contains( $pek_destination_store_source, "'response_shape'" ) && str_contains( $pek_destination_store_source, "'rejections'" ) && str_contains( $pek_destination_store_source, "'api_error_message'" ) && str_contains( $pek_destination_store_source, "'raw_error'" ) && str_contains( $pek_destination_store_source, "'authorization'" ) && str_contains( $pek_destination_store_source, "'api_key'" ), 'PEK destination diagnostic store must allowlist safe diagnostic keys including api_error_message and keep recursive unsafe-key filtering.' );
plugin_architecture_assert( str_contains( $pek_admin_page_source, 'Код ошибки' ) && str_contains( $pek_admin_page_source, 'Ошибка ПЭК' ) && str_contains( $pek_admin_page_source, 'Response shape' ) && str_contains( $pek_admin_page_source, 'Rejections' ) && str_contains( $pek_admin_page_source, 'render_destination_named_section' ) && ! str_contains( $pek_admin_page_source, 'format_report_value( $report[ $section ]' ), 'PEK destination admin report must render named diagnostic sections and api_error_message instead of positional array output.' );
...
plugin_architecture_assert( str_contains( $pek_destination_diagnostic_service_source, 'Logger' ) && str_contains( $pek_destination_diagnostic_service_source, 'PEK destination pickup diagnostic failed.' ) && str_contains( $pek_destination_diagnostic_service_source, 'safe_api_error_message' ) && str_contains( $pek_destination_diagnostic_service_source, 'api_error_message' ) && str_contains( $pek_destination_diagnostic_service_source, 'response_shape' ) && ! str_contains( $pek_destination_diagnostic_service_source, 'raw_response' ), 'PEK destination diagnostic failures must use project Logger with safe structured context and redacted api_error_message.' );
plugin_architecture_assert( str_contains( $pek_api_client_source, 'logical_error_message' ) && str_contains( $pek_api_client_source, 'api_error_part' ) && ! str_contains( $pek_api_client_source, "(string) ( \$error['title']" ) && ! str_contains( $pek_api_client_source, "(string) ( \$error['message']" ), 'PEK API client must assemble logical error messages without string-casting malformed title/message values.' );
