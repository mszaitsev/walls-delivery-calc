...
plugin_architecture_assert( ! str_contains( $pek_location_resolver_source, "\$row['precision']" ) && ! str_contains( $pek_location_resolver_source, "\$row['Precision']" ) && str_contains( $pek_location_resolver_source, "'GeoData'" ) && str_contains( $pek_location_resolver_source, "\$geo['precision']" ) && str_contains( $pek_location_resolver_source, 'pek_missing_address_precision' ), 'PEK address precision must be read only from documented GeoData.precision.' );
...
plugin_architecture_assert( ! is_file( plugin_architecture_path( 'database/migrations/0050_create_pek_geography_hardening.php' ) ) && ! is_file( plugin_architecture_path( 'src/Carriers/Pek/Pickup/PekTerminalRowNormalizer.php' ) ), 'PEK 0.131.3 hardening must not add migrations or new production normalizer classes.' );
foreach ( plugin_architecture_php_files( 'src' ) as $file ) {
	$relative = str_replace( '\\', '/', substr( $file, strlen( plugin_architecture_root() ) + 1 ) );
	if ( 'src/Core/Plugin.php' === $relative ) {
		continue;
