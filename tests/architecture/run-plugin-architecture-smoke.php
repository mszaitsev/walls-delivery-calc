...
plugin_architecture_assert( str_contains( $plugin_source, 'PekShipmentStatusService::class' ) && str_contains( $plugin_source, 'ShipmentCreationAttemptService::class' ) && strpos( $pek_status_source, 'mark_terminal_before_delete( $order, $shipment, \'cancelled\' )' ) < strpos( $pek_status_source, 'delete_for_carrier( $order, PekSettings::CARRIER_KEY )' ) && str_contains( $pek_status_source, "DeliveryStatus::CANCELLED === (string) ( \$status['universal_status_code'] ?? '' )" ) && str_contains( $pek_status_source, "'terminal_source' => 'status_update'" ) && ! str_contains( $pek_status_source, 'order_cancellation(' ), 'PEK status update must perform backend CANCELLED cleanup by terminalizing the attempt before repository deletion and without carrier mutation.' );
...
plugin_architecture_assert( str_contains( $pek_cancellation_service_source, 'fresh_status_precheck' ) && strpos( $pek_cancellation_service_source, 'is_cancelled_status( $external_status' ) < strpos( $pek_cancellation_service_source, 'order_cancellation( array( $code ) )' ), 'PEK already-cancelled precheck must reconcile terminal state before invoking cancellation mutation.' );
...
plugin_architecture_assert( str_contains( $pek_request_builder_source, "'sealing'" ) && str_contains( $pek_request_builder_source, "'enabled' => true" ) && str_contains( $pek_request_builder_source, "'payer' => array( 'type' => 1 )" ) && ! str_contains( $pek_request_builder_source, "'storing'" ) && ! str_contains( $pek_request_builder_source, "'bag'" ) && ! str_contains( $pek_request_builder_source, "'smallBag'" ) && ! str_contains( $pek_request_builder_source, "'packageType'" ) && ! str_contains( $pek_request_builder_source, "'packagingType'" ), 'PEK services fix must keep sealing policy and must not add storing or undocumented bag/package aliases.' );
