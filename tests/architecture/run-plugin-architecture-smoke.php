...
plugin_architecture_assert( str_contains( $pek_destination_source, 'PekLocationResolver' ) && str_contains( $pek_destination_source, '$this->locations->resolve_for_shipment' ) && str_contains( $pek_location_resolver_source, 'resolve_for_shipment' ) && str_contains( $pek_location_resolver_source, 'stale_fallback' ) && str_contains( $pek_destination_source, "'fresh_location_mapping'" ) && ! str_contains( $pek_destination_source, "\$request->meta['pek_receiver_branch_id'] ?? '' )" ), 'PEK shipment destination must use fresh branch authority and must not fall back to stale/meta branch IDs.' );
...
plugin_architecture_assert( str_contains( $pek_sender_warehouse_source, 'normalize_warehouse_id' ) && str_contains( $pek_sender_warehouse_source, 'branches_all_for_warehouse( $warehouse_id )' ) && str_contains( $pek_sender_warehouse_source, 'branches_all()' ) && str_contains( $pek_sender_warehouse_source, "'unfiltered_fallback'" ) && str_contains( $pek_sender_warehouse_source, 'requested_id_hash' ) && str_contains( $pek_sender_warehouse_source, 'matched_id_hash' ), 'PEK sender warehouse validation must normalize canonical UUIDs and use filtered plus bounded unfiltered branches/all lookup with safe ID hashes.' );
plugin_architecture_assert( str_contains( $pek_sender_warehouse_source, 'divisionCapabilitiesPresent' ) && str_contains( $pek_sender_warehouse_source, 'effectiveCapabilitySource' ) && str_contains( $pek_sender_warehouse_source, '$warehouse_has_capabilities' ) && str_contains( $pek_sender_warehouse_source, '$division_has_capabilities' ) && str_contains( $pek_sender_warehouse_source, "'warehouse'" ) && str_contains( $pek_sender_warehouse_source, "'division'" ) && ! str_contains( $pek_sender_warehouse_source, '$warehouse_has_capabilities ?' ), 'PEK sender warehouse branches/all validation must support non-empty warehouse capabilities first and division capabilities as fallback with safe diagnostic source fields.' );
...
plugin_architecture_assert( str_contains( $pek_button_policy_source, 'PENDING_CREATION_IN_CARRIER' ) && str_contains( $pek_button_policy_source, "'manual_attach' => true" ) && str_contains( $pek_button_policy_source, "'remove' => true" ) && str_contains( $pek_button_policy_source, 'is_pre_acceptance_status' ), 'PEK pending creation record must allow manual reconciliation/local remove and cancellation UI must use explicit external-status allowlist.' );
...
plugin_architecture_assert( str_contains( $pek_request_builder_source, 'courier_location_match' ) && str_contains( $pek_request_builder_source, 'courier_branch_source' ) && str_contains( $pek_request_builder_source, 'courier_address_hash' ) && str_contains( $pek_request_builder_source, 'courier_location_level' ), 'PEK safe preview must expose courier destination binding evidence without raw address.' );
...
plugin_architecture_assert( strpos( $pek_credentials_source, '$encrypted = $this->encryption->encrypt( $key );' ) < strpos( $pek_credentials_source, "\$this->settings->set( PekSettings::LOGIN_KEY, \$login );\n\t\t\$this->settings->set( PekSettings::API_KEY_ENCRYPTED_KEY" ), 'PEK credentials save must encrypt requested API key before writing login/key.' );
