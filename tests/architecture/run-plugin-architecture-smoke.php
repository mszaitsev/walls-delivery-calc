...
plugin_architecture_assert( str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentAdapter::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentPersistenceMapper::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentDocumentProvider::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentModalExtension::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentPreflightQuoteService::class' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryShipmentService::class' ) && str_contains( $ozon_plugin_source, 'CarrierShipmentAdapterRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentDocumentProviderRegistry::class' ) && str_contains( $ozon_plugin_source, 'ShipmentModalExtensionRegistry::class' ), 'Ozon shipments must be wired through existing Shipment Framework registries in Plugin.php.' );
...
plugin_architecture_assert( str_contains( $ozon_shipment_adapter_source, 'OzonDeliveryShipmentActionPolicy::for_shipment' ) && str_contains( $ozon_shipment_service_source, 'OzonDeliveryShipmentActionPolicy::all_cancelled' ) && str_contains( $ozon_shipment_service_source, "'auto_poll' => true" ) && str_contains( $ozon_shipment_service_source, "'poll_interval_ms' => 5000" ) && str_contains( $ozon_shipment_service_source, "'poll_max_attempts' => 14" ) && str_contains( $ozon_shipment_service_source, 'delete_for_carrier' ) && str_contains( $ozon_shipment_action_policy_source, 'ready_for_shipping' ) && str_contains( $ozon_shipment_action_policy_source, 'all_cancelled' ) && str_contains( $ozon_shipment_action_policy_source, "'cancellation_started'" ) && str_contains( $ozon_shipment_action_policy_source, "'can_remove' => false" ) && str_contains( $ozon_shipment_adapter_source, "'cancellation_exhausted'" ) && str_contains( $ozon_shipment_adapter_source, 'Дождитесь подтверждения отмены Ozon' ), 'Ozon cancellation must use carrier-owned raw status action policy, shared 5s x 14 polling lifecycle, lock local removal while cancellation is active, and local cleanup only after all postings are CANCELED.' );
plugin_architecture_assert( str_contains( $ozon_admin_source, 'render_statuses' ) && str_contains( $ozon_admin_source, 'OzonDeliveryShipmentStatusMapping::documented_statuses()' ) && str_contains( plugin_architecture_source( 'src/DeliveryServices/Admin/DeliveryServicesAdminPage.php' ), "\$tabs['ozon_statuses'] = 'Статусы Ozon'" ), 'Ozon status admin tab must consume the production status mapping and remain code-owned.' );
