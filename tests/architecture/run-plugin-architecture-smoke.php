...
plugin_architecture_assert( is_file( plugin_architecture_path( 'src/Carriers/Contracts/CarrierCustomerCommentProviderInterface.php' ) ) && str_contains( plugin_architecture_source( 'src/Checkout/Runtime/CheckoutOrchestrator.php' ), 'CarrierCustomerCommentProviderInterface' ) && str_contains( plugin_architecture_source( 'src/Checkout/Runtime/CheckoutOrchestrator.php' ), 'rate_with_carrier_customer_comments' ) && str_contains( $ozon_plugin_source, 'OzonDeliveryCustomerCommentProvider::class' ) && str_contains( plugin_architecture_source( 'src/Carriers/OzonDelivery/Checkout/OzonDeliveryCustomerCommentProvider.php' ), 'Отслеживание посылки - в приложении Ozon, раздел Доставка' ) && str_contains( plugin_architecture_source( 'src/Carriers/OzonDelivery/Checkout/OzonDeliveryCustomerCommentProvider.php' ), '$rate->crossed_price ?? $rate->price' ), 'Carrier customer comments must use a generic post-rule extension point, while Ozon owns the exact customer strings and crossed-price fallback.' );
plugin_architecture_assert( str_contains( $ozon_plugin_source, 'JetLogisticCustomerCommentProvider::class' ) && str_contains( plugin_architecture_source( 'src/Carriers/Runtime/JetLogisticCarrier.php' ), 'CarrierCustomerCommentProviderInterface' ) && str_contains( plugin_architecture_source( 'src/Carriers/JetLogistic/Checkout/JetLogisticCustomerCommentProvider.php' ), 'Адрес склада выдачи - ' ) && str_contains( plugin_architecture_source( 'src/Carriers/JetLogistic/Checkout/JetLogisticCustomerCommentProvider.php' ), 'на сайте Jet Logistic' ) && str_contains( plugin_architecture_source( 'src/Checkout/Runtime/CheckoutOrchestrator.php' ), 'customer_comments_rendered_in_checkout' ), 'Jet Logistic customer comments must use the same carrier-owned post-rule extension point without duplicating its structured checkout link.' );
plugin_architecture_assert( str_contains( plugin_architecture_source( 'src/Checkout/WooCommerce/OrderShippingMetaPersister.php' ), 'pickup_snapshot_with_customer_comments' ) && str_contains( plugin_architecture_source( 'src/Orders/Application/OrderDeliveryReplacementService.php' ), 'pickup_snapshot_with_customer_comments' ) && str_contains( plugin_architecture_source( 'src/Pickup/Presentation/PickupPointCardRenderer.php' ), 'customer_comments' ) && str_contains( plugin_architecture_source( 'src/Pickup/Presentation/PickupPointCardRenderer.php' ), 'wdc-pickup-point-card__customer-comment' ), 'Order creation, admin recalculation and pickup card rendering must preserve generic pickup snapshot customer_comments without changing point description.' );
plugin_architecture_assert( str_contains( $order_delivery_customer_comments_display_source, '_wdc_platform_requires_pickup_point' ) && str_contains( $order_delivery_customer_comments_display_source, 'DeliveryType::PICKUP === $delivery_type && $requires_pickup_point' ) && ! str_contains( $order_shipping_meta_persister_source, 'jet_logistic' ) && ! str_contains( $order_shipping_meta_persister_source, 'JetLogistic' ) && ! str_contains( $order_delivery_customer_comments_display_source, 'jet_logistic' ) && ! str_contains( $order_delivery_customer_comments_display_source, 'JetLogistic' ), 'Standalone order delivery comments display must skip only selectable pickup rates by generic requires_pickup_point metadata and generic order persistence/display must not contain Jet Logistic branches.' );
