...
plugin_architecture_assert( str_contains( $pickup_checkout_php_source, "'pickupRateCapabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_php_source, 'private function pickup_rate_capabilities()' ) && str_contains( $pickup_checkout_php_source, "'prefetch_points'" ) && ! str_contains( $pickup_checkout_php_source, "'reload_on_viewport_change' => \$provider_capabilities" ) && str_contains( $pickup_checkout_rest_source, "'pickup_rate_capabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_rest_source, 'private function pickup_rate_capabilities()' ) && str_contains( $pickup_checkout_rest_source, "'prefetch_points'" ) && str_contains( $pickup_checkout_source, 'pickupRateCapabilities = normalizePickupRateCapabilities' ) && str_contains( $pickup_checkout_source, 'function mergePickupRateCapabilitiesFromResponse(response)' ) && str_contains( $pickup_checkout_source, 'function withRateCapabilities(context, method)' ) && str_contains( $pickup_checkout_source, 'function refreshModalContext(method)' ) && str_contains( $pickup_checkout_source, 'refreshCheckoutContextOnce(1200, { returnContext: true, currentFieldsOnly: true })' ) && str_contains( $pickup_checkout_source, 'withRateCapabilities(withPrefetch(withCarrierContext(resolvedContext, method), method), method)' ) && str_contains( $pickup_checkout_source, 'function prefetchIdentity(context, method)' ) && str_contains( $pickup_checkout_source, 'function prefetchIdentityMatches(cached, current)' ) && str_contains( $pickup_checkout_source, 'function prefetchPointsAllowed(method)' ) && str_contains( $pickup_map_source, 'var fixedDatasetLoaded = false' ) && str_contains( $pickup_map_source, 'true !== options.forceRemote' ) && str_contains( $pickup_map_source, 'function listFollowsViewport()' ) && str_contains( $pickup_map_source, 'if (!viewportReloadRequired() && visiblePoints.length)' ) && ! str_contains( $pickup_map_source, 'ozon_delivery' ) && ! str_contains( $pickup_checkout_source, "carrier === 'ozon_delivery'" ), 'Generic pickup map must refresh authoritative state before modal creation, keep active rate capabilities separate from destination context, disable large fixed-area prefetch by capability, and preserve fixed-area viewport behavior without Ozon-specific JS branches.' );
