...
plugin_architecture_assert( str_contains( $checkout_persister_source, "'block_type'" ) && str_contains( $checkout_persister_source, "'flat_type'" ) && str_contains( plugin_architecture_source( 'assets/frontend/checkout-address-suggestions.js' ), "'dadata_flat_type'" ), 'Generic checkout order persister must preserve DaData block/flat type fields without a PEK-specific branch.' );
...
plugin_architecture_assert( str_contains( $pek_settings_source, 'normalize_sender_phone_input' ) && str_contains( $pek_settings_source, "'/^[\\d+\\s()\\-]+$/'" ) && str_contains( $pek_settings_source, "'/^8\\d{10}$/'" ) && str_contains( $pek_settings_source, "'/^\\+7\\d{10}$/'" ), 'PEK sender phone settings must reject letters/control characters before RU normalization.' );
