...
plugin_architecture_assert( str_contains( $pek_api_client_source, 'public function calculate_price' ) && str_contains( $pek_api_client_source, "'/calculator/calculateprice/'" ) && str_contains( $pek_api_client_source, 'pek_unexpected_calculate_price_response' ) && str_contains( $pek_api_client_source, 'quote_calculator_logical' ), 'PEK API client must expose typed POST /calculator/calculateprice/ with quote-specific safe failure stages.' );
...
plugin_architecture_assert( str_contains( $pek_admin_page_source, 'diagnose_pek_quote' ) && str_contains( $pek_admin_page_source, 'render_quote_diagnostic_report' ) && str_contains( $pek_admin_page_source, 'quote_reports->clear_for_current_user();' ) && strpos( $pek_admin_page_source, 'quote_reports->clear_for_current_user();' ) < strpos( $pek_admin_page_source, 'quote_diagnostics->run' ), 'PEK admin quote diagnostic must be explicit-action only and clear stale reports before rerun.' );
