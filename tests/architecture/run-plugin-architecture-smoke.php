...
plugin_architecture_assert( str_contains( $pek_terminal_cache_source, 'FORMAT_VERSION' ) && str_contains( $pek_terminal_cache_source, 'delete_transient' ) && str_contains( $pek_terminal_cache_source, 'PickupPoint::from_array' ) && str_contains( $pek_terminal_cache_source, 'PekSettings::CARRIER_KEY' ) && str_contains( $pek_terminal_cache_source, 'safe_point_array' ) && str_contains( $pek_terminal_cache_source, 'safe_raw_reference' ), 'PEK destination terminal cache must be versioned and validate/project cached PEK PickupPoint payloads.' );
...
plugin_architecture_assert( ! is_file( plugin_architecture_path( 'database/migrations/0050_create_pek_geography_hardening.php' ) ) && ! is_file( plugin_architecture_path( 'src/Carriers/Pek/Pickup/PekTerminalRowNormalizer.php' ) ), 'PEK 0.131.4 hardening must not add migrations or new production normalizer classes.' );
foreach ( plugin_architecture_php_files( 'src' ) as $file ) {
	$relative = str_replace( '\\', '/', substr( $file, strlen( plugin_architecture_root() ) + 1 ) );
	if ( 'src/Core/Plugin.php' === $relative ) {
		continue;
