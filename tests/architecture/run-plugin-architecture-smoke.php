...
plugin_architecture_assert( str_contains( $pickup_map_source, 'return yandexCityListMode || !viewportReloadRequired();' ) && str_contains( $pickup_map_source, 'return !validPointCoordinates(point) || pointInsideBounds(point, lastBbox);' ), 'Fixed pickup datasets must filter only sidebar presentation by current bounds while retaining coordinate-less points and the full marker source.' );
