...
plugin_architecture_assert( str_contains( $ozon_shipment_adapter_source, 'return $this->service->attach_manual( $order, $payload );' ) && str_contains( $ozon_shipment_service_source, 'public function attach_manual' ) && str_contains( $ozon_shipment_service_source, '$this->api->posting_info( array( $posting_number ) )' ) && str_contains( $ozon_shipment_service_source, "'not_available_in_posting_info_contract'" ), 'Ozon manual attach must use the existing adapter/service path, validate through posting/info, and not fabricate actual cost.' );
