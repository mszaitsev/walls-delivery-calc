...
plugin_architecture_assert( str_contains( $order_recalculation_controller, "'diagnostic' => is_array( \$result['diagnostic']" ) && str_contains( $order_recalculation_service_source, 'pek_diagnostic' ) && str_contains( $order_recalculation_service_source, "'request_country_code'" ) && str_contains( $order_recalculation_service_source, "'pek_service_country_enabled'" ) && str_contains( $order_recalculation_service_source, "'pek_carrier_quote_attempted'" ) && str_contains( $order_recalculation_service_source, "'pek_rates_count'" ), 'Order-admin preview must expose bounded PEK diagnostic evidence through AJAX.' );
plugin_architecture_assert( str_contains( $checkout_orchestrator_source, "'service_scan'" ) && str_contains( $checkout_orchestrator_source, "'carrier_quotes'" ) && str_contains( $checkout_orchestrator_source, 'carrier_quote_diagnostic' ) && str_contains( $checkout_orchestrator_source, "'entry_created'" ) && str_contains( $checkout_orchestrator_source, "'cache_hit'" ), 'CheckoutOrchestrator must expose generic service-scan and carrier-quote diagnostics for admin preview.' );
plugin_architecture_assert( ! str_contains( $order_recalculation_service_source, 'raw_response' ) && ! str_contains( $order_recalculation_service_source, 'raw_request' ) && ! str_contains( $order_recalculation_service_source, 'phone' ) && ! str_contains( $order_recalculation_service_source, 'email' ), 'Order-admin PEK preview diagnostic must not expose raw payloads or recipient PII.' );
