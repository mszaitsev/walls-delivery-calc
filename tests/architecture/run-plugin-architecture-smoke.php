...
plugin_architecture_assert( str_contains( plugin_architecture_source( 'src/DeliveryServices/DeliveryServiceRepository.php' ), 'ensure_self_pickup_service' ) && str_contains( plugin_architecture_source( 'src/DeliveryServices/DeliveryServiceRepository.php' ), 'AVAILABILITY_ALL_COUNTRIES' ) && str_contains( $self_pickup_settings_source, 'DEFAULT_ADDRESS' ) && str_contains( $self_pickup_settings_source, 'DEFAULT_CUSTOMER_COMMENT' ) && str_contains( $self_pickup_settings_source, 'safe_url' ) && ! str_contains( $self_pickup_settings_source, 'wp_kses' ), 'SelfPickupSettings must own defaults/settings and safe structured-link storage without arbitrary HTML.' );
plugin_architecture_assert( str_contains( $self_pickup_discount_policy_source, 'round( $base_kopecks * $percent / 100 )' ) && str_contains( $self_pickup_discount_service_source, 'get_cart_contents_total' ) && str_contains( $self_pickup_discount_service_source, 'chosen_shipping_methods' ) && str_contains( $self_pickup_discount_service_source, 'add_fee' ) && ! str_contains( $self_pickup_discount_service_source, 'apply_coupon' ) && ! str_contains( $self_pickup_discount_service_source, 'remove_coupon' ) && ! str_contains( $self_pickup_discount_service_source, 'get_coupons' ) && ! str_contains( $self_pickup_discount_service_source, 'label ===' ), 'Self-pickup discount must be a negative fee based on stable WDC identity and post-coupon cart item totals, not coupons or labels.' );
plugin_architecture_assert( str_contains( $self_pickup_checkout_renderer_source, 'render_fixed_pickup_card' ) && str_contains( $self_pickup_checkout_renderer_source, 'fixed_pickup_point_snapshot' ) && str_contains( $self_pickup_order_persister_source, 'fixed_pickup_snapshot_for_rate' ) && str_contains( $self_pickup_order_persister_source, '_wdc_pickup_point_snapshot' ), 'Fixed fulfillment location snapshots must reuse generic checkout and order pickup presentation/persistence.' );
plugin_architecture_assert( str_contains( $shipment_draft_factory_source, 'non_shipment_state' ) && str_contains( $metabox_source, 'non_shipment_state' ) && str_contains( $payload_builder_source, 'non_shipment_state' ), 'Shipment admin presentation must support carrier-agnostic non-shipment informational state.' );
...
$generic_self_pickup_guard_source = plugin_architecture_source( 'src/Checkout/Runtime/CheckoutOrchestrator.php' ) . $metabox_source . $payload_builder_source . plugin_architecture_source( 'src/Shipments/Application/ShipmentCreationService.php' ) . plugin_architecture_source( 'assets/frontend/pickup-map/wdc-pickup-checkout.js' ) . plugin_architecture_source( 'assets/frontend/pickup-map/wdc-pickup-map.js' );
