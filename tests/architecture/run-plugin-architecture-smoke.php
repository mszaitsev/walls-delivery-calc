...
plugin_architecture_assert( str_contains( $pickup_checkout_source, 'reload_on_viewport_change: config.reload_on_viewport_change' ) && str_contains( $pickup_checkout_source, 'function contextReloadOnViewportChange()' ) && str_contains( $pickup_map_source, 'function listFollowsViewport()' ) && str_contains( $pickup_map_source, 'if (!viewportReloadRequired() && visiblePoints.length)' ) && ! str_contains( $pickup_map_source, 'ozon_delivery' ) && ! str_contains( $pickup_checkout_source, "carrier === 'ozon_delivery'" ), 'Generic pickup map must preserve fixed-area viewport capability without Ozon-specific JS branches.' );
