...
plugin_architecture_assert( str_contains( $pek_status_normalizer_source, '-1 === $value' ) && str_contains( $pek_status_normalizer_source, "'-1' === \$value" ) && str_contains( $pek_status_source, '$clear_status_id' ) && str_contains( $pek_status_source, "unset( \$shipment['pek_cargo_status_id'] )" ) && ! str_contains( $pek_status_normalizer_source, '$value < 0' ), 'PEK status normalizer must treat only exact integer/string cargoStatusId=-1 as an unassigned sentinel and must not persist it.' );
