...
$initial_schema_source = plugin_architecture_source( 'database/migrations/0001_initial_schema.php' );
...
plugin_architecture_assert( str_contains( $ozon_transport_source, 'MAX_REDIRECTS = 12' ) && str_contains( $ozon_transport_source, "'redirection' => 0" ), 'Ozon transport must own bounded DDoS redirect handling.' );
...
plugin_architecture_assert( str_contains( $ozon_pickup_provider_source, "'requires_rate_refresh' => true" ) && str_contains( plugin_architecture_source( 'src/Pickup/Rest/PickupPointsRestController.php' ), 'registry_boolean_value' ) && str_contains( plugin_architecture_source( 'src/Pickup/Rest/PickupPointsRestController.php' ), "'ozon_delivery' === \$carrier && '1' === \$this->param( \$request, 'progressive' )" ), 'Ozon provider must declare repricing through generic projection while REST keeps progressive loading behind an explicit Ozon-only request flag.' );
...
plugin_architecture_assert( ! str_contains( plugin_architecture_source( 'src/Carriers/Runtime/OzonDeliveryCarrier.php' ), 'checkout quote calculated' ) && ! str_contains( plugin_architecture_source( 'src/Carriers/Runtime/OzonDeliveryCarrier.php' ), 'safe_success_log_context' ), 'Successful Ozon checkout quotes must not create production log noise.' );
...
plugin_architecture_assert( str_contains( $ozon_shipment_adapter_source, 'return $this->service->attach_manual( $order, $payload );' ) && str_contains( $ozon_shipment_service_source, 'public function attach_manual' ) && str_contains( $ozon_shipment_service_source, '$this->posting_info_with_retry( array( $posting_number ) )' ) && str_contains( $ozon_shipment_service_source, '$this->api->posting_info( $posting_numbers, OzonDeliveryApiClient::SHIPMENT_REQUEST_TIMEOUT )' ) && str_contains( $ozon_shipment_service_source, 'MANUAL_ATTACH_ACTUAL_COST_SOURCE_DETAIL' ) && str_contains( $ozon_shipment_service_source, 'estimated_delivery_cost' ) && str_contains( $ozon_shipment_service_source, 'estimated_insurance_cost' ) && str_contains( $ozon_shipment_service_source, 'ShipmentActualCost' ) && ! str_contains( $ozon_shipment_service_source, "'not_available_in_posting_info_contract'" ), 'Ozon manual attach must use the existing adapter/service path, validate through bounded posting/info retries with the shipment timeout, and persist only documented posting/info delivery plus insurance as canonical actual cost.' );
...
plugin_architecture_assert( str_contains( $ozon_shipment_service_source, 'post_approve_status_snapshot' ) && str_contains( $ozon_shipment_service_source, '$this->posting_info_with_retry( $numbers )' ) && str_contains( $ozon_shipment_service_source, 'DeliveryStatus::CREATED_IN_CARRIER' ) && str_contains( $ozon_shipment_mapper_source, 'ozon_statuses' ), 'Ozon create+approve must enrich the persisted shipment with a bounded post-approve status snapshot without introducing a generic approve action.' );
...
plugin_architecture_assert( str_contains( $initial_schema_source, 'schedule text NOT NULL' ) && ! str_contains( $initial_schema_source, 'schedule_json' ) && str_contains( $initial_schema_source, 'active_geo_lookup' ) && 0 === ( new ReflectionFunction( require plugin_architecture_path( 'database/migrations/0001_initial_schema.php' ) ) )->getNumberOfRequiredParameters(), 'The 1.0 initial schema must contain final Ozon schedule storage and geo index.' );
plugin_architecture_assert( str_contains( $pickup_checkout_php_source, "'pickupRateCapabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_php_source, 'private function pickup_rate_capabilities()' ) && str_contains( $pickup_checkout_php_source, "'prefetch_points'" ) && ! str_contains( $pickup_checkout_php_source, "'reload_on_viewport_change' => \$provider_capabilities" ) && str_contains( $pickup_checkout_rest_source, "'pickup_rate_capabilities' => \$pickup_rate_capabilities" ) && str_contains( $pickup_checkout_rest_source, 'private function pickup_rate_capabilities()' ) && str_contains( $pickup_checkout_rest_source, "'prefetch_points'" ) && str_contains( $pickup_checkout_source, 'pickupRateCapabilities = normalizePickupRateCapabilities' ) && str_contains( $pickup_checkout_source, 'function mergePickupRateCapabilitiesFromResponse(response)' ) && str_contains( $pickup_checkout_source, 'function withRateCapabilities(context, method)' ) && str_contains( $pickup_checkout_source, 'var contextPromise = refreshCheckoutContextOnce(700, { returnContext: true })' ) && str_contains( $pickup_checkout_source, 'return freshContext || initialContext();' ) && str_contains( $pickup_checkout_source, 'var modal = window.WDCPickupModal.create(labels);' ) && str_contains( $pickup_checkout_source, 'var map = window.WDCPickupMap.create' ) && ! str_contains( $pickup_checkout_source, 'function refreshModalContext(method)' ) && ! str_contains( $pickup_checkout_source, 'function setPickupOpenButtonsLoading(container, loading)' ) && str_contains( $pickup_checkout_source, 'var baseContext = Object.assign({}, resolvedContext || {});' ) && str_contains( $pickup_checkout_source, 'var containerFamily = containerSelectedPickupFamily(container);' ) && str_contains( $pickup_checkout_source, 'withRateCapabilities(withPrefetch(withCarrierContext(baseContext, method), method), method)' ) && str_contains( $pickup_checkout_source, 'parts[parts.length - 1] !== \'pickup\'' ) && str_contains( $pickup_checkout_source, 'return parts[0] || \'\';' ) && ! str_contains( $pickup_checkout_source, "|| 'russian_post'" ) && str_contains( $pickup_checkout_source, 'function prefetchIdentity(context, method)' ) && str_contains( $pickup_checkout_source, 'function prefetchIdentityMatches(cached, current)' ) && str_contains( $pickup_checkout_source, '&& cached.carrier === current.carrier' ) && str_contains( $pickup_checkout_source, 'function pointPrefetchAllowed(method)' ) && str_contains( $pickup_checkout_source, "Object.prototype.hasOwnProperty.call(item, 'prefetch_points')" ) && str_contains( $pickup_checkout_source, 'capabilities.prefetch_points === false' ) && str_contains( $pickup_map_source, 'var fixedDatasetLoaded = false' ) && str_contains( $pickup_map_source, 'true !== options.forceRemote' ) && str_contains( $pickup_map_source, 'function listFollowsViewport()' ) && str_contains( $pickup_map_source, 'if (!viewportReloadRequired() && visiblePoints.length)' ) && ! str_contains( $pickup_map_source, 'ozon_delivery' ) && ! str_contains( $pickup_checkout_source, "carrier === 'ozon_delivery'" ), 'Generic pickup map must use the last live-confirmed stable opening lifecycle, keep active rate capabilities separate from destination context, route multi-segment pickup families without Russian Post fallback, disable point prefetch by generic capability, and preserve fixed-area viewport behavior without Ozon-specific JS branches.' );
...
$self_pickup_carrier_source = plugin_architecture_source( 'src/Carriers/Runtime/SelfPickupCarrier.php' );
$self_pickup_settings_source = plugin_architecture_source( 'src/Carriers/SelfPickup/SelfPickupSettings.php' );
$self_pickup_discount_policy_source = plugin_architecture_source( 'src/Carriers/SelfPickup/SelfPickupDiscountPolicy.php' );
$self_pickup_discount_service_source = plugin_architecture_source( 'src/Carriers/SelfPickup/SelfPickupDiscountService.php' );
$self_pickup_checkout_renderer_source = plugin_architecture_source( 'src/Checkout/WooCommerce/CheckoutRateRenderer.php' );
$self_pickup_order_persister_source = plugin_architecture_source( 'src/Checkout/WooCommerce/OrderShippingMetaPersister.php' );
$rate_sorter_source = plugin_architecture_source( 'src/Checkout/Sorting/RateSorter.php' );
...
plugin_architecture_assert( str_contains( $plugin_source, 'SelfPickupSettings::class' ) && str_contains( $plugin_source, 'SelfPickupDiscountPolicy::class' ) && str_contains( $plugin_source, 'SelfPickupDiscountService::class' ) && str_contains( $plugin_source, 'SelfPickupCarrier::class' ), 'Self-pickup checkout-only runtime and discount services must be DI-wired in Plugin.php.' );
plugin_architecture_assert( str_contains( $plugin_source, '$registry->register( $this->container->get( SelfPickupCarrier::class ) )' ) && str_contains( $self_pickup_settings_source, "CARRIER_KEY = 'self_pickup'" ) && str_contains( $self_pickup_settings_source, "SERVICE_KEY = 'self_pickup'" ), 'Self-pickup must register exactly one checkout runtime with stable carrier/service identity.' );
plugin_architecture_assert( str_contains( $self_pickup_carrier_source, 'Money::from_kopecks( 0 )' ) && str_contains( $self_pickup_carrier_source, 'DeliveryType::PICKUP' ) && str_contains( $self_pickup_carrier_source, "'fixed_pickup_point_snapshot'" ) && str_contains( $self_pickup_carrier_source, "'requires_pickup_point' => false" ) && ! str_contains( $self_pickup_carrier_source, 'CarrierPickupPointProviderInterface' ), 'SelfPickupCarrier must expose zero-price pickup-style checkout metadata without a selectable pickup provider.' );
plugin_architecture_assert( str_contains( plugin_architecture_source( 'src/DeliveryServices/DeliveryServiceRepository.php' ), 'ensure_self_pickup_service' ) && str_contains( plugin_architecture_source( 'src/DeliveryServices/DeliveryServiceRepository.php' ), 'AVAILABILITY_ALL_COUNTRIES' ) && str_contains( $self_pickup_settings_source, 'DEFAULT_ADDRESS' ) && str_contains( $self_pickup_settings_source, 'DEFAULT_CARD_TITLE' ) && str_contains( $self_pickup_settings_source, 'DEFAULT_CUSTOMER_COMMENT' ) && str_contains( $self_pickup_settings_source, "DEFAULT_DISCOUNT_FEE_LABEL = 'Скидка {s}%" ) && str_contains( $self_pickup_settings_source, 'safe_url' ) && ! str_contains( $self_pickup_settings_source, 'wp_kses' ), 'SelfPickupSettings must own defaults/settings, separate card title, dynamic fee-label placeholder, and safe structured-link storage without arbitrary HTML.' );
plugin_architecture_assert( str_contains( $self_pickup_discount_policy_source, '$available' ) && str_contains( $self_pickup_discount_policy_source, '$applied' ) && str_contains( $self_pickup_discount_policy_source, 'round( $base_kopecks * $percent / 100 )' ) && str_contains( $self_pickup_discount_service_source, 'current_promotion_result' ) && str_contains( $self_pickup_discount_service_source, 'get_cart_contents_total' ) && str_contains( $self_pickup_discount_service_source, 'chosen_shipping_methods' ) && str_contains( $self_pickup_discount_service_source, 'add_fee' ) && str_contains( $self_pickup_discount_service_source, 'resolve_fee_label' ) && ! str_contains( $self_pickup_discount_service_source, 'apply_coupon' ) && ! str_contains( $self_pickup_discount_service_source, 'remove_coupon' ) && ! str_contains( $self_pickup_discount_service_source, 'get_coupons' ) && ! str_contains( $self_pickup_discount_service_source, 'label ===' ), 'Self-pickup discount must separate promotion availability from applied fee, resolve {s}, and stay based on stable WDC identity and post-coupon cart item totals.' );
plugin_architecture_assert( str_contains( $rate_sorter_source, 'compare_cheapest_rates' ) && str_contains( $rate_sorter_source, '0 === $left_cost' ) && ! str_contains( $rate_sorter_source, 'self_pickup' ), 'CHEAPEST zero-price sorting must be generic in RateSorter and never branch on self-pickup.' );
plugin_architecture_assert( str_contains( $self_pickup_checkout_renderer_source, 'render_fixed_pickup_card' ) && str_contains( $self_pickup_checkout_renderer_source, 'fixed_pickup_point_snapshot' ) && str_contains( $self_pickup_checkout_renderer_source, 'rate_is_chosen' ) && str_contains( $self_pickup_checkout_renderer_source, 'text_before_link' ) && str_contains( $customer_renderer_source, 'text_before_link' ) && str_contains( $self_pickup_order_persister_source, 'fixed_pickup_snapshot_for_rate' ) && str_contains( $self_pickup_order_persister_source, '_wdc_pickup_point_snapshot' ), 'Fixed fulfillment location snapshots must reuse generic checkout/order pickup presentation, render only for chosen rates, and normalize structured-link spacing at presentation.' );
plugin_architecture_assert( str_contains( $delivery_services_admin_source, 'if ( ! $this->is_self_pickup_service( $service ) )' ) && str_contains( $delivery_services_admin_source, "\$tabs['rules'] = 'Правила'" ) && str_contains( $delivery_services_admin_source, 'Показывать промо-комментарий, когда акция доступна для корзины' ), 'Delivery Services admin must hide Rules tab only in the self-pickup admin presentation boundary while keeping Rules for regular services.' );
plugin_architecture_assert( str_contains( $shipment_draft_factory_source, 'non_shipment_state' ) && str_contains( $shipment_metabox_source, 'non_shipment_state' ) && str_contains( $payload_builder_source, 'non_shipment_state' ) && str_contains( $shipment_metabox_source, 'if ( ! $non_shipment_active )' ) && str_contains( $payload_builder_source, 'non_shipment_state_active' ), 'Shipment admin presentation must support carrier-agnostic non-shipment state that suppresses shipment status/body/actions while keeping service identity.' );
plugin_architecture_assert( ! str_contains( $plugin_source, 'SelfPickupShipmentAdapter' ) && ! str_contains( $plugin_source, 'SelfPickupShipmentPersistenceMapper' ) && ! str_contains( $plugin_source, 'SelfPickupShipmentDocumentProvider' ) && ! str_contains( $plugin_source, 'SelfPickupShipmentModalExtension' ), 'Self-pickup must not be wired into shipment adapter, mapper, document, or modal registries.' );
$generic_self_pickup_guard_source = plugin_architecture_source( 'src/Checkout/Runtime/CheckoutOrchestrator.php' ) . $shipment_metabox_source . $payload_builder_source . plugin_architecture_source( 'src/Shipments/Application/ShipmentCreationService.php' ) . plugin_architecture_source( 'assets/frontend/pickup-map/wdc-pickup-checkout.js' ) . plugin_architecture_source( 'assets/frontend/pickup-map/wdc-pickup-map.js' );
plugin_architecture_assert( 1 !== preg_match( '/[\'"]self_pickup[\'"]/', $generic_self_pickup_guard_source ), 'Generic checkout, shipment metabox, shipment creation, and pickup JS must not contain self-pickup-specific branches.' );
...
plugin_architecture_assert( str_contains( $pickup_points_rest_source, 'CheckoutPickupPointProviderQueryResolver' ) && str_contains( $pickup_points_rest_source, 'registry_points_response' ) && str_contains( $pickup_points_rest_source, 'provider_query_resolver' ) && str_contains( $pickup_points_rest_source, '->resolve_context(' ), 'Public pickup points REST must use trusted server rate context for registry-backed provider search.' );
plugin_architecture_assert( str_contains( $pickup_points_rest_source, 'registry_point_payload' ) && str_contains( $pickup_points_rest_source, "'ozon_delivery' === \$carrier && '1' === \$this->param( \$request, 'progressive' )" ) && ! str_contains( $checkout_pickup_rest_source, 'ozon_delivery' ), 'Generic pickup REST must keep provider DTO rendering generic and isolate Ozon pagination behind the explicit progressive transport flag.' );
...
plugin_architecture_assert( is_file( plugin_architecture_path( 'database/migrations/0001_initial_schema.php' ) ) && ! is_file( plugin_architecture_path( 'templates' . '/.gitkeep' ) ), 'The 1.0 initial schema must exist and the unused template placeholder must remain absent.' );
...
plugin_architecture_assert( str_contains( $jet_status_mapping_repository_source, 'DEFAULT_MAPPINGS' ) && str_contains( $jet_status_mapping_repository_source, 'Доставка груза на склад приемки' ) && str_contains( $jet_status_mapping_repository_source, 'Отправка груза со склада приемки' ) && str_contains( $jet_status_mapping_repository_source, 'DeliveryStatus::IN_TRANSIT' ) && str_contains( $initial_schema_source, '->ensure_default_mappings()' ), 'Jet default status mappings must be repository-owned and seeded by the 1.0 initial schema.' );
...
plugin_architecture_assert( substr_count( $initial_schema_source, '->install_schema()' ) >= 2 && str_contains( $initial_schema_source, 'PekLocationMappingRepository' ) && str_contains( $initial_schema_source, 'PekTerminalRepository' ), 'PEK schemas must be installed by the 1.0 initial schema.' );
...
foreach ( array(
	'1.0 initial schema' => $initial_schema_source,
...
$plugin_register_start = strpos( $plugin_source, 'public function register(): void' );
$plugin_register_end   = false !== $plugin_register_start ? strpos( $plugin_source, 'public function container(): Container', $plugin_register_start ) : false;
$plugin_register_body  = false !== $plugin_register_start && false !== $plugin_register_end ? substr( $plugin_source, $plugin_register_start, $plugin_register_end - $plugin_register_start ) : '';
plugin_architecture_assert( str_contains( $plugin_register_body, 'register_activation_hook' ) && str_contains( $plugin_register_body, 'register_deactivation_hook' ) && strpos( $plugin_register_body, 'run_migrations_safely' ) < strpos( $plugin_register_body, 'register_hooks' ), 'Plugin registration must install lifecycle hooks and complete migrations before resolving table-backed runtime services.' );
...
plugin_architecture_assert( 1 === count( glob( plugin_architecture_path( 'database/migrations/*.php' ) ) ?: array() ) && ! is_file( plugin_architecture_path( 'src/Carriers/Pek/Pickup/PekTerminalRowNormalizer.php' ) ), 'The 1.0 schema must remain squashed and PEK must not add an unrelated production normalizer class.' );
foreach ( plugin_architecture_php_files( 'src' ) as $file ) {
	$relative = str_replace( '\\', '/', substr( $file, strlen( plugin_architecture_root() ) + 1 ) );
	if ( 'src/Core/Plugin.php' === $relative ) {
		continue;
...
plugin_architecture_assert( ! is_dir( plugin_architecture_path( 'src/Shipments/Pek/Storage' ) ), 'PEK shipment correction must not add separate shipment storage.' );
...
$order_quote_request_mapper_source = plugin_architecture_source( 'src/Orders/Application/OrderQuoteRequestMapper.php' );
$dpd_quote_carrier_source = plugin_architecture_source( 'src/Carriers/Runtime/DpdQuoteCarrier.php' );
$dpd_tariff_service_source = plugin_architecture_source( 'src/Carriers/Dpd/Tariff/DpdTariffCalculationService.php' );
...
plugin_architecture_assert( str_contains( $order_recalculation_js, 'requiresRateRefresh' ) && str_contains( $order_recalculation_js, "restorePekPickup: 'pek' === pointCarrier" ) && str_contains( $order_recalculation_js, 'function restorePekPickupPreview' ), 'Order recalculation JS must refresh and restore PEK pickup selection after terminal choice.' );
...
plugin_architecture_assert( str_contains( $order_quote_request_mapper_source, 'selected_location_differs_from_saved' ) && str_contains( $order_quote_request_mapper_source, 'is_dpd_pickup_selection' ) && str_contains( $order_quote_request_mapper_source, '$saved_location_id <= 0 || $selected_location_id !== $saved_location_id' ), 'Order admin QuoteRequest mapper must suppress saved DPD terminal fallback when selected canonical location differs from saved order location.' );
plugin_architecture_assert( str_contains( $order_quote_request_mapper_source, 'saved_pickup_belongs_to_dpd' ) && str_contains( $order_quote_request_mapper_source, 'saved_dpd_terminal_code' ) && strpos( $order_quote_request_mapper_source, '! $this->saved_pickup_belongs_to_dpd( $order )' ) < strpos( $order_quote_request_mapper_source, 'return $this->saved_dpd_terminal_code( $order );' ), 'Order admin QuoteRequest mapper must verify saved pickup DPD ownership before reading saved DPD/generic terminal codes.' );
plugin_architecture_assert( str_contains( $dpd_quote_carrier_source, "'woocommerce_order_admin_preview' === (string) ( \$request->customer_context['source'] ?? '' )" ) && str_contains( $dpd_quote_carrier_source, 'pickup_selection_for_family( DpdSettings::CARRIER_KEY . \':pickup\' )' ), 'DPD quote carrier must isolate order-admin preview from checkout session fallback while preserving checkout session support.' );
plugin_architecture_assert( str_contains( $dpd_tariff_service_source, "\$delivery_terminal_source = 'selected';" ) && str_contains( $dpd_tariff_service_source, "'delivery_terminal_selection' => array()" ), 'DPD tariff diagnostics must mark invalid explicit terminalCode failures as selected with an empty selection.' );
...
$manual_pickup_provider_source = plugin_architecture_source( 'src/Carriers/Manual/ManualPickupPointProvider.php' );
$manual_pickup_repository_source = plugin_architecture_source( 'src/Carriers/Manual/ManualPickupPointRepository.php' );
$pickup_provider_query_source = plugin_architecture_source( 'src/Pickup/Providers/CarrierPickupPointQuery.php' ) . plugin_architecture_source( 'src/Pickup/Providers/CheckoutPickupPointProviderQueryResolver.php' );
$checkout_session_source = plugin_architecture_source( 'src/Checkout/WooCommerce/CheckoutSessionManager.php' );
$checkout_location_fingerprint_source = plugin_architecture_source( 'src/Checkout/WooCommerce/CheckoutLocationFingerprint.php' );
$manual_destination_fingerprint_start = strpos( $manual_carrier_source, 'private function destination_fingerprint' );
$manual_destination_fingerprint_end = false !== $manual_destination_fingerprint_start ? strpos( $manual_carrier_source, 'private function quote_id', $manual_destination_fingerprint_start ) : false;
$manual_destination_fingerprint_source = false !== $manual_destination_fingerprint_start && false !== $manual_destination_fingerprint_end ? substr( $manual_carrier_source, $manual_destination_fingerprint_start, $manual_destination_fingerprint_end - $manual_destination_fingerprint_start ) : '';
...
plugin_architecture_assert( str_contains( $plugin_source, 'ManualPickupPointProvider::class' ) && str_contains( $plugin_source, '$this->container->get( ManualPickupPointProvider::class )' ) && ! str_contains( $plugin_source, 'ManualDeliveryPickupRestController' ), 'Manual pickup must register one registry-backed provider and must not add a manual REST endpoint.' );
plugin_architecture_assert( str_contains( $checkout_location_fingerprint_source, 'final class CheckoutLocationFingerprint' ) && str_contains( $checkout_location_fingerprint_source, 'country=' ) && str_contains( $checkout_location_fingerprint_source, 'location_id' ) && str_contains( $checkout_location_fingerprint_source, 'fias_id' ) && str_contains( $checkout_location_fingerprint_source, 'gar_object_id' ) && str_contains( $checkout_location_fingerprint_source, 'place=' ) && str_contains( $checkout_location_fingerprint_source, 'postcode=' ), 'Checkout location fingerprint must have one reusable generic owner for pickup destination identity.' );
plugin_architecture_assert( str_contains( $checkout_session_source, 'CheckoutLocationFingerprint' ) && str_contains( $checkout_session_source, '$this->location_fingerprint->fingerprint( $context )' ) && str_contains( $manual_carrier_source, 'CheckoutLocationFingerprint' ) && str_contains( $manual_destination_fingerprint_source, '$this->location_fingerprint->fingerprint' ) && ! str_contains( $manual_destination_fingerprint_source, 'md5' ), 'Manual pickup carrier and checkout session manager must use the same canonical location fingerprint implementation without a carrier-owned hash algorithm.' );
plugin_architecture_assert( str_contains( $manual_pickup_provider_source, 'CarrierPickupPointProviderInterface' ) && str_contains( $manual_pickup_provider_source, '$query->normalized_service_key()' ) && str_contains( $manual_pickup_provider_source, 'resolve_selection' ) && ! str_contains( $manual_pickup_provider_source, 'location_id' ), 'Manual pickup provider must use trusted service_key and textual destination identity, not persistent location_id.' );
plugin_architecture_assert( str_contains( $manual_pickup_repository_source, 'wdc_manual_delivery_pickup_points' ) && str_contains( $manual_pickup_repository_source, 'service_id' ) && str_contains( $manual_pickup_repository_source, 'code' ) && ! str_contains( $manual_pickup_repository_source, 'dbDelta' ), 'Manual pickup repository must own storage access and must not create schema at runtime.' );
plugin_architecture_assert( str_contains( $manual_pickup_repository_source, "'point_title' => \$title" ) && str_contains( $manual_pickup_repository_source, "'point_name' => \$title" ) && str_contains( $manual_pickup_repository_source, "'display_code' => ''" ) && str_contains( $manual_pickup_repository_source, "'reload_on_viewport_change' => false" ) && str_contains( $manual_pickup_repository_source, "'requires_rate_refresh' => false" ) && ! str_contains( $manual_pickup_repository_source, "'display_code' => \$row['code']" ) && ! str_contains( $manual_pickup_repository_source, "'presentation_comment' => \$row['comment']" ), 'Manual pickup projection must expose admin title as the customer-facing name, keep stable code internal, avoid duplicating normal comments as presentation comments, declare a fixed local dataset capability, and mark selection as fulfillment-only without forced rate refresh.' );
plugin_architecture_assert( str_contains( $pickup_points_rest_source, '$type_label' ) && str_contains( $pickup_points_rest_source, '$point_title' ) && str_contains( $pickup_points_rest_source, "'point_comment' => \$point_comment" ) && str_contains( $pickup_points_rest_source, "'display_title' => \$display_title" ) && str_contains( $pickup_points_rest_source, "array_key_exists( 'requires_rate_refresh', \$raw ) ? \$this->registry_boolean_value( \$raw, 'requires_rate_refresh' ) : true" ), 'Generic registry pickup REST must keep type label, point title, display code, point_comment, and provider-owned requires_rate_refresh as separate presentation fields with a backward-compatible true default.' );
plugin_architecture_assert( str_contains( plugin_architecture_source( 'src/Pickup/Presentation/PickupPointCardRenderer.php' ), "'point_comment' => \$point_comment" ) && str_contains( plugin_architecture_source( 'src/Pickup/Presentation/PickupPointCardRenderer.php' ), 'Комментарий:' ) && str_contains( $pickup_map_source, 'function pointComment(point)' ) && str_contains( $pickup_map_source, 'wdc-pickup-list__comment' ) && str_contains( $pickup_map_source, 'sameText(presentation, comment)' ) && ! str_contains( $pickup_map_source, "carrier === 'manual'" ), 'Shared pickup card and map/list renderers must render ordinary point comments once with a semantic label without manual-specific frontend branches.' );
plugin_architecture_assert( str_contains( $pickup_provider_query_source, 'service_key' ) && str_contains( $pickup_provider_query_source, 'normalized_service_key' ) && ! str_contains( $pickup_provider_query_source, 'manual_service_key' ), 'Generic pickup provider query must carry service_key without a manual-specific alias.' );
$delivery_quote_cache_manager_source = plugin_architecture_source( 'src/Checkout/Cache/DeliveryQuoteCacheManager.php' );
plugin_architecture_assert( str_contains( $delivery_quote_cache_manager_source, 'woocommerce_update_product' ) && str_contains( $delivery_quote_cache_manager_source, 'woocommerce_update_product_variation' ) && str_contains( $delivery_quote_cache_manager_source, 'bump_delivery_rates_cache_version' ) && ! str_contains( $manual_carrier_source, 'woocommerce_update_product' ) && ! str_contains( plugin_architecture_source( 'src/Carriers/Manual/ManualDeliveryPricingService.php' ), 'woocommerce_update_product' ), 'Product shipping-data cache invalidation must be generic and must not live in manual carrier/pricing services.' );
$quote_cache_source = plugin_architecture_source( 'src/Checkout/Cache/QuoteCache.php' );
plugin_architecture_assert( str_contains( $quote_cache_source, 'package_items_signature' ) && str_contains( $quote_cache_source, 'get_total_quantity()' ) && ! str_contains( $checkout_orchestrator_source, 'manual_zero' ) && ! str_contains( plugin_architecture_source( 'src/Checkout/WooCommerce/WooCommercePackageMapper.php' ), 'manual_zero' ), 'Quote cache must distinguish zero-weight physical packages from empty packages without manual-specific checkout or mapper branches.' );
...
$manual_shipment_adapter_source = plugin_architecture_source( 'src/Shipments/Manual/ManualShipmentAdapter.php' );
$manual_shipment_service_source = plugin_architecture_source( 'src/Shipments/Manual/ManualShipmentService.php' );
$shipment_manual_attach_ajax_source = plugin_architecture_source( 'src/Shipments/Admin/Ajax/ShipmentManualAttachAjaxController.php' );
plugin_architecture_assert( str_contains( $plugin_source, 'ManualShipmentAdapter::class' ) && str_contains( $plugin_source, 'ManualShipmentService::class' ) && str_contains( $plugin_source, '$this->container->get( ManualShipmentAdapter::class )' ), 'Manual shipments must register one carrier-level adapter and one carrier-owned service through Plugin.php.' );
plugin_architecture_assert( ! str_contains( $plugin_source, 'ManualShipmentDocumentProvider' ) && ! str_contains( $plugin_source, 'ManualShipmentModalExtension' ) && ! str_contains( $plugin_source, 'ManualShipmentPersistenceMapper' ), 'Manual shipments must not register document, modal-extension, or fake persistence mapper providers.' );
plugin_architecture_assert( str_contains( $manual_shipment_service_source, 'OrderShipmentRepository' ) && str_contains( $manual_shipment_service_source, 'ShipmentActualCostService' ) && str_contains( $manual_shipment_service_source, "'service_key' => \$context['service_key']" ) && str_contains( $manual_shipment_service_source, "'service_title' => \$context['service_title']" ), 'Manual shipment service must use OrderShipmentRepository, delegate actual cost to the common service, and persist concrete historical service identity.' );
plugin_architecture_assert( str_contains( $manual_shipment_adapter_source, 'CarrierShipmentAdapterInterface' ) && str_contains( $manual_shipment_adapter_source, "error_code: 'manual_create_unsupported'" ) && str_contains( $manual_shipment_adapter_source, 'supports_status_auto_sync(): bool' ) && str_contains( $manual_shipment_adapter_source, 'return false;' ) && ! str_contains( $manual_shipment_adapter_source, 'document_actions' ), 'Manual shipment adapter must expose framework participation while keeping preview/create/status API/autosync/documents unsupported.' );
plugin_architecture_assert( ! str_contains( plugin_architecture_source( 'src/Shipments/Application/ShipmentCreationService.php' ), 'ManualShipment' ) && ! str_contains( $shipment_events_source . $shipment_status_source, "carrier === 'manual'" ), 'Manual shipment support must not add manual branches to ShipmentCreationService or generic shipment JS.' );
plugin_architecture_assert( str_contains( $shipment_metabox_source, 'data-wdc-manual-attach-actual-cost-input' ) && str_contains( $shipment_manual_attach_ajax_source, "'actual_cost' => \$actual_cost" ), 'Manual attach UI may pass optional actual cost through the existing shared manual-attach endpoint.' );
...
$checkout_address_runtime_source = plugin_architecture_source( 'src/Checkout/Address/CheckoutAddressRuntime.php' );
plugin_architecture_assert( str_contains( $wc_package_mapper_source, 'normalize_package_address( $destination )' ) && ! str_contains( $wc_package_mapper_source, 'resolve_checkout_address( $destination )' ) && str_contains( $checkout_address_runtime_source, 'public function normalize_package_address' ), 'WooCommerce package normalization must use the explicit read-only address runtime boundary.' );
$package_normalization_start = strpos( $checkout_address_runtime_source, 'public function normalize_package_address' );
$package_normalization_end = strpos( $checkout_address_runtime_source, 'public function fingerprint_from_checkout_data', $package_normalization_start );
$package_normalization_source = false !== $package_normalization_start && false !== $package_normalization_end ? substr( $checkout_address_runtime_source, $package_normalization_start, $package_normalization_end - $package_normalization_start ) : '';
plugin_architecture_assert( '' !== $package_normalization_source && ! str_contains( $package_normalization_source, 'save_selected_city' ) && ! str_contains( $package_normalization_source, 'save_city_context' ) && ! str_contains( $package_normalization_source, 'save_fallback_city' ) && ! str_contains( $package_normalization_source, 'resolve_city' ), 'Read-only package normalization must not mutate or reconcile canonical checkout city identity.' );
$checkout_session_manager_source = plugin_architecture_source( 'src/Checkout/WooCommerce/CheckoutSessionManager.php' );
$location_formatter_source = plugin_architecture_source( 'src/Locations/Services/LocationDisplayNameFormatter.php' );
$checkout_postcode_source = plugin_architecture_source( 'src/Locations/Services/CheckoutPostcode.php' );
$checkout_location_ajax_source = plugin_architecture_source( 'src/Checkout/Locations/CheckoutLocationAjax.php' );
$checkout_city_resolver_source = plugin_architecture_source( 'src/Checkout/Locations/CheckoutCityResolver.php' );
$shipping_method_registrar_source = plugin_architecture_source( 'src/Checkout/WooCommerce/ShippingMethodRegistrar.php' );
$checkout_city_selector_source = plugin_architecture_source( 'assets/frontend/checkout-city-selector.js' );
plugin_architecture_assert( str_contains( $checkout_address_runtime_source, 'previous_location_fingerprint' ) && str_contains( $checkout_address_runtime_source, 'reconcile_pickup_selections_after_address_change' ) && str_contains( $checkout_address_runtime_source, 'trusted_session_location_for_transient_checkout' ) && ! str_contains( $checkout_address_runtime_source, "'' === \$current_rate_id && array() !== \$this->session_manager->pickup_selections()" ), 'Address fingerprint repair must compare canonical destination after reconciliation and must not treat a missing current shipping method as destination change evidence.' );
plugin_architecture_assert( str_contains( $checkout_session_manager_source, 'public function current_location_fingerprint' ) && str_contains( $checkout_session_manager_source, 'public function clear_normalized_address_result' ), 'Checkout session manager must expose its existing canonical fingerprint owner and separate normalized address state from location identity.' );
plugin_architecture_assert( str_contains( $location_formatter_source, "'' !== \$postcode ? ', ' . \$postcode : ''" ) && ! str_contains( $location_formatter_source, 'searchable_text' ), 'Checkout location option formatter must append postal code as presentation without changing location search data.' );
plugin_architecture_assert( 1 === substr_count( $checkout_postcode_source, '999999999' ) && str_contains( $location_formatter_source, 'CheckoutPostcode::usable_value' ) && str_contains( $checkout_location_ajax_source, 'CheckoutPostcode::usable_value' ) && str_contains( $checkout_city_resolver_source, 'CheckoutPostcode::usable_value' ) && str_contains( $checkout_address_runtime_source, 'CheckoutPostcode::usable_value' ), 'Technical missing-postcode sentinel must have one checkout presentation owner across labels, payload, resolution, and runtime.' );
plugin_architecture_assert( str_contains( $shipping_method_registrar_source, "'canonical_city_context' => \$this->canonical_city_context_config()" ) && str_contains( $shipping_method_registrar_source, '$context_id !== $selected_id' ) && str_contains( $checkout_city_selector_source, 'function restoreCanonicalSessionSelection()' ) && strpos( $checkout_city_selector_source, 'restoreCanonicalSessionSelection()' ) < strpos( $checkout_city_selector_source, 'scheduleAutoResolve();', strpos( $checkout_city_selector_source, 'function init()' ) ), 'Canonical Woo session selection must be coherence-checked server-side and restored before textual checkout reconciliation.' );
plugin_architecture_assert( str_contains( $pickup_map_source, 'function listFollowsViewport()' ) && str_contains( $pickup_map_source, 'return true;' ) && str_contains( $pickup_map_source, 'return validPointCoordinates(point) && (!lastBbox || pointInsideBounds(point, lastBbox));' ), 'Every pickup sidebar must be the current viewport projection and must exclude points without valid coordinates while retaining the full marker source.' );
$destination_coordinates_start = strpos( $wc_package_mapper_source, 'private function destination_coordinates' );
$destination_coordinates_end = strpos( $wc_package_mapper_source, 'private function selected_location_fias_id', $destination_coordinates_start );
$destination_coordinates_source = false !== $destination_coordinates_start && false !== $destination_coordinates_end ? substr( $wc_package_mapper_source, $destination_coordinates_start, $destination_coordinates_end - $destination_coordinates_start ) : '';
plugin_architecture_assert( strpos( $destination_coordinates_source, 'find_by_id' ) < strpos( $destination_coordinates_source, 'coordinate_pair_from_contexts' ), 'Known canonical location IDs must resolve active DB coordinates before session fallback coordinates.' );
...
$pickup_family_resolver_source = plugin_architecture_source( 'src/Checkout/WooCommerce/PickupFamilyResolver.php' );
plugin_architecture_assert( str_contains( $pickup_family_resolver_source, 'final class PickupFamilyResolver' ) && str_contains( $pickup_family_resolver_source, 'from_delivery_rate' ) && str_contains( $pickup_family_resolver_source, 'from_meta' ) && str_contains( $pickup_family_resolver_source, "\$carrier . ':' . \$service . ':pickup'" ) && str_contains( $pickup_family_resolver_source, "\$carrier . ':pickup'" ), 'Pickup family derivation must have one generic owner for explicit, multi-service, and single-service pickup rates.' );
plugin_architecture_assert( str_contains( $checkout_provider_resolver_source, 'private function woocommerce_rate' ) && str_contains( $checkout_provider_resolver_source, 'WC()->shipping()' ) && str_contains( $checkout_provider_resolver_source, 'get_packages' ), 'Checkout pickup provider resolver must be able to resolve provider context from the authoritative WooCommerce package rates.' );
plugin_architecture_assert( strpos( $checkout_provider_resolver_source, '$woocommerce_rate = $this->woocommerce_rate( $id );' ) < strpos( $checkout_provider_resolver_source, 'foreach ( $this->session_manager->rates()' ), 'Authoritative WooCommerce rates must be checked before stale WDC duplicate rate snapshots.' );
plugin_architecture_assert( str_contains( $checkout_provider_resolver_source, 'WooCommerceRateMetaNormalizer::rate_snapshot' ) && str_contains( $checkout_provider_resolver_source, 'WooCommerceRateMetaNormalizer::rate_id' ), 'Checkout pickup provider resolver must reuse the shared WooCommerce rate metadata normalizer.' );
...
plugin_architecture_assert( strpos( $checkout_provider_resolver_source, "\$rate['carrier_key']" ) < strpos( $checkout_provider_resolver_source, "\$meta['carrier_key']" ) && str_contains( $checkout_provider_resolver_source, 'PickupFamilyResolver::from_meta' ), 'Checkout pickup provider resolver must prefer production top-level rate envelope and delegate family derivation to the generic resolver.' );
plugin_architecture_assert( str_contains( $checkout_provider_resolver_source, "true !== \$context['requires_pickup_point']" ) && str_contains( $checkout_provider_resolver_source, "'pickup' !== \$context['delivery_type']" ), 'Checkout pickup provider resolver must reject non-pickup/courier rate envelopes.' );
plugin_architecture_assert( str_contains( $checkout_provider_resolver_source, "\$context['service_key']" ) && str_contains( $checkout_provider_resolver_source, "'' === \$context['service_key']" ), 'Checkout pickup provider resolver must require a trusted non-empty service key from the stored rate envelope.' );
...
$checkout_pickup_rest_source = plugin_architecture_source( 'src/Pickup/Rest/CheckoutPickupPointRestController.php' );
plugin_architecture_assert( str_contains( $checkout_pickup_rest_source, "resolve_context( \$method_id, \$carrier, \$family )" ) && str_contains( $checkout_pickup_rest_source, "\$fingerprint = (string) \$context['destination_fingerprint'];" ) && str_contains( $checkout_pickup_rest_source, "provider_rate_context_missing" ), 'Checkout pickup save must enforce trusted destination fingerprint from the same resolved rate context.' );
plugin_architecture_assert( str_contains( $checkout_pickup_rest_source, "array_key_exists( 'requires_rate_refresh', \$raw )" ) && str_contains( $checkout_pickup_rest_source, "\$requires_rate_refresh = true;" ) && str_contains( $checkout_pickup_rest_source, "'requires_rate_refresh' => \$requires_rate_refresh" ), 'Checkout pickup save must derive requires_rate_refresh from the server-resolved provider point metadata with true as the backward-compatible default.' );
...
plugin_architecture_assert( str_contains( $checkout_session_source, 'PickupFamilyResolver::legacy_from_rate_id' ) && str_contains( $checkout_session_source, 'PickupFamilyResolver::normalize_family' ) && str_contains( $pickup_family_resolver_source, '$parts[0] = $carrier' ) && ! str_contains( $pickup_family_resolver_source, '? $carrier . \':pickup\'' ), 'Checkout session pickup-family normalization must delegate to the generic resolver and preserve multi-segment service-specific pickup families.' );
...
plugin_architecture_assert( str_contains( $pek_carrier_source_for_cache, 'is_technical_mode_failure' ) && str_contains( $pek_carrier_source_for_cache, 'PEK checkout quote mode failed.' ) && str_contains( $pek_carrier_source_for_cache, "'endpoint'" ) && str_contains( $pek_carrier_source_for_cache, "'http_status'" ), 'PEK must log only technical mode failures with a safe allowlisted context.' );
...
plugin_architecture_assert( str_contains( $checkout_rate_renderer_source, 'WooCommerceRateMetaNormalizer::meta' ) && str_contains( $wc_rate_meta_normalizer_source, 'final class WooCommerceRateMetaNormalizer' ) && str_contains( $wc_rate_meta_normalizer_source, 'get_meta_data' ) && str_contains( $wc_rate_meta_normalizer_source, 'get_data' ), 'WooCommerce shipping-rate metadata normalization must be shared between renderer and pickup provider resolver.' );
$checkout_validation_source = plugin_architecture_source( 'src/Checkout/WooCommerce/CheckoutValidation.php' );
...
plugin_architecture_assert( str_contains( $pickup_map_js_source, 'minLoadingZoom = initialLoadingZoom - 1' ) && str_contains( $pickup_map_js_source, 'function zoomLoadingBlocked()' ) && str_contains( $pickup_map_js_source, 'Увеличьте карту, чтобы увидеть пункты выдачи' ) && str_contains( $pickup_map_css_source, '.wdc-pickup-map__zoom-warning' ), 'Generic pickup map must throttle viewport reloads from the second zoom-out step and expose the zoom-in warning.' );
plugin_architecture_assert( str_contains( $pickup_map_js_source, 'SIDEBAR_POINT_LIMIT = 100' ) && str_contains( $pickup_map_js_source, 'points = points.slice(0, SIDEBAR_POINT_LIMIT)' ) && str_contains( $pickup_map_js_source, 'provider.renderMarkers(visiblePoints' ) && str_contains( $pickup_map_css_source, '.wdc-pickup-list__overflow' ), 'Generic pickup map must cap only sidebar presentation while keeping the full marker set.' );
$leaflet_map_provider_source = plugin_architecture_source( 'assets/frontend/pickup-map/providers/wdc-map-provider-leaflet.js' );
$yandex_map_provider_source = plugin_architecture_source( 'assets/frontend/pickup-map/providers/wdc-map-provider-yandex.js' );
plugin_architecture_assert( str_contains( $leaflet_map_provider_source, 'getZoom: function ()' ) && str_contains( $yandex_map_provider_source, 'getZoom: function ()' ), 'Both generic map adapters must expose current zoom.' );
...
plugin_architecture_assert( str_contains( $pickup_checkout_js_source, 'var authoritativePickupSelections = {};' ) && str_contains( $pickup_checkout_js_source, 'authoritativePickupSelections = Object.assign({}, selectedPickupPoints)' ) && str_contains( $pickup_checkout_js_source, 'mergePickupSelectionsFromResponse(state, { authoritativeState: true })' ), 'Checkout pickup inline notice latch must use post-calculation /checkout/state as the authoritative source for selected-point success.' );
