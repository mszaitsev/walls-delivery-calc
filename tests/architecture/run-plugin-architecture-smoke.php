...
plugin_architecture_assert( str_contains( $pek_destination_store_source, "'failure_stage'" ) && str_contains( $pek_destination_store_source, "'response_shape'" ) && str_contains( $pek_destination_store_source, "'rejections'" ) && str_contains( $pek_destination_store_source, "'authorization'" ) && str_contains( $pek_destination_store_source, "'api_key'" ), 'PEK destination diagnostic store must allowlist new safe diagnostic keys and keep recursive unsafe-key filtering.' );
plugin_architecture_assert( str_contains( $pek_admin_page_source, 'Код ошибки' ) && str_contains( $pek_admin_page_source, 'Response shape' ) && str_contains( $pek_admin_page_source, 'Rejections' ) && str_contains( $pek_admin_page_source, 'render_destination_named_section' ) && ! str_contains( $pek_admin_page_source, 'format_report_value( $report[ $section ]' ), 'PEK destination admin report must render named diagnostic sections instead of positional array output.' );
...
plugin_architecture_assert( str_contains( $pek_destination_diagnostic_service_source, 'Logger' ) && str_contains( $pek_destination_diagnostic_service_source, 'PEK destination pickup diagnostic failed.' ) && str_contains( $pek_destination_diagnostic_service_source, 'response_shape' ) && ! str_contains( $pek_destination_diagnostic_service_source, 'raw_response' ), 'PEK destination diagnostic failures must use project Logger with safe structured context.' );
