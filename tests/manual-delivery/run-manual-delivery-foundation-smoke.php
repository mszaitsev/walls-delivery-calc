...
$initial_schema_source = (string) file_get_contents( $root . '/database/migrations/0001_initial_schema.php' );
...
wdc_manual_assert( str_contains( $initial_schema_source, 'country_code varchar(2)' ) && str_contains( $initial_schema_source, 'ux_manual_region_country' ) && str_contains( $initial_schema_source, 'ux_manual_location_country' ), 'The initial schema must use country-aware manual geography identities and unique indexes.' );
wdc_manual_assert( str_contains( $initial_schema_source, 'wdc_manual_delivery_weight_ranges' ) && str_contains( $initial_schema_source, 'ux_manual_weight_range' ) && str_contains( $initial_schema_source, 'from_weight_g' ) && str_contains( $initial_schema_source, 'price_kopecks' ), 'The initial schema must create manual delivery weight ranges with grams, kopecks, and unique range identity.' );
wdc_manual_assert( str_contains( $initial_schema_source, 'wdc_manual_delivery_pickup_points' ) && str_contains( $initial_schema_source, 'ux_manual_pickup_service_code' ) && str_contains( $initial_schema_source, 'location_name' ) && str_contains( $initial_schema_source, 'region_name' ), 'The initial schema must create manual pickup points with stable per-service code and textual locality identity.' );
