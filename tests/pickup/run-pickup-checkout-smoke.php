...
pickup_checkout_assert( str_contains( $checkout_js, 'var visibleCity = fieldValue' ) && str_contains( $checkout_js, 'var city = hiddenDisplay || visibleCity' ), 'contextFromFields must use visible city query when hidden city fields are empty.' );
pickup_checkout_assert( str_contains( $checkout_js, 'var postcode = hiddenPostcode || visiblePostcode' ) && str_contains( $checkout_js, 'query: fieldContext.query || runtimeContext.query || localizedContext.query' ), 'Initial context must use hidden/visible query first and localized config only as fallback.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'updateCurrentContext(context)' ) && str_contains( $checkout_js, 'applyContextToHidden(context)' ) && str_contains( $checkout_js, 'schedulePrefetch()' ), 'Location-selected event must update runtime context, hidden fields, and prefetch immediately.' );
