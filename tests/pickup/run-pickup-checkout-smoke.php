...
pickup_checkout_assert( str_contains( $checkout_js, 'function runCrossLocationSelection(point, location)' ) && str_contains( $checkout_js, 'var updatedCheckout = waitForUpdatedCheckout(12000)' ) && str_contains( $checkout_js, 'updatedCheckout.then(function ()' ) && str_contains( $checkout_js, 'commitPoint(point, selectedMethod, { updateCheckoutAfterSave: false' ), 'Cross-location flow must wait for updated_checkout before saving the pending point.' );
$cross_flow_source = substr( $checkout_js, strpos( $checkout_js, 'function runCrossLocationSelection(point, location)' ), 1400 );
pickup_checkout_assert( strpos( $cross_flow_source, 'var updatedCheckout = waitForUpdatedCheckout(12000)' ) < strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) && strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) < strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) && strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) < strpos( $cross_flow_source, 'commitPoint(point, selectedMethod' ), 'Cross-location flow must listen for updated_checkout, update checkout, then save the selected pickup point.' );
pickup_checkout_assert( str_contains( $checkout_js, 'selectCheapestPickupRate' ) && str_contains( $checkout_js, 'isPickupRateValue(input.value || \'\')' ) && str_contains( $checkout_js, 'rates.sort(function (a, b)' ) && str_contains( $checkout_js, 'rateCost(a) - rateCost(b)' ) && str_contains( $checkout_js, 'rateCostFromValue' ), 'Cross-location flow must choose the cheapest available Russian Post pickup rate after recalculation.' );
