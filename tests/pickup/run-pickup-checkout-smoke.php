...
pickup_checkout_assert( str_contains( $map_js, 'var activePoint = previewPoint && pointInList(previewPoint, visiblePoints) ? previewPoint : (committedPoint && pointInList(committedPoint, visiblePoints) ? committedPoint : null)' ), 'Preview point must have active marker priority over committedPoint during marker reload.' );
...
pickup_checkout_assert( str_contains( $map_js, 'scrollListRowIntoView(point)' ) && str_contains( $map_js, 'function scrollContainerForList(start)' ) && str_contains( $map_js, 'function rowOffsetTop(row, container)' ), 'Marker/list preview must scroll the active row inside the list container.' );
pickup_checkout_assert( str_contains( $map_js, 'container.scrollTo({ top: nextTop, behavior: \'smooth\' })' ) && str_contains( $map_js, 'container.scrollTop = nextTop' ) && str_contains( $map_js, 'nextTop === null' ), 'List scroll must update only the scroll container and leave visible rows alone.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'onPopupSelect' ) && str_contains( $leaflet_provider_js, 'onMapClick' ) && str_contains( $leaflet_provider_js, "map.on('moveend zoomend', boundsChanged)" ) && str_contains( $leaflet_provider_js, "map.on('click', mapClicked)" ), 'Leaflet provider must support popup select delegation and map-click close without closing on drag/move.' );
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'var maxClusterZoom = 17' ) && str_contains( $leaflet_provider_js, 'map.getZoom() >= maxClusterZoom' ) && str_contains( $leaflet_provider_js, 'return { points: [point]' ), 'Leaflet grid clustering must disable clusters at strong zoom.' );
...
pickup_checkout_assert( str_contains( $yandex_provider_js, 'onPopupSelect' ) && str_contains( $yandex_provider_js, 'onMapClick' ) && str_contains( $yandex_provider_js, "map.events.add('click', mapClicked)" ), 'Yandex provider must support popup select delegation and map-click close without closing on boundschange.' );
pickup_checkout_assert( str_contains( $yandex_provider_js, 'var maxClusterZoom = 17' ) && str_contains( $yandex_provider_js, 'var useClusterer = map.getZoom() < maxClusterZoom' ) && str_contains( $yandex_provider_js, 'map.geoObjects.add(placemark)' ), 'Yandex provider must bypass Clusterer at strong zoom so points remain selectable.' );
