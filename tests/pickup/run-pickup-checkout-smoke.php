...
pickup_checkout_assert( str_contains( $map_js, 'previewPoint = matchingPoint(committedPoint, visiblePoints)' ) && str_contains( $map_js, "list.querySelectorAll('[data-wdc-point-id]')" ) && str_contains( $map_js, 'scrollListRowIntoView(previewPoint)' ), 'Checkout preview restore must be able to find and scroll a selected point regardless of its position in the full list.' );
