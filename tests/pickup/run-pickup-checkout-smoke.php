...
$invoice_plugin = file_get_contents( $root . '/walls-invoice-payment.php' ) ?: '';
...
pickup_checkout_assert( str_contains( $domestic_tariff_css, 'gap: 4px 0' ) && str_contains( $domestic_tariff_css, 'content: "- "' ) && str_contains( $domestic_tariff_css, 'content: ": "' ) && ! str_contains( $domestic_tariff_css, 'content: " - "' ) && ! str_contains( $domestic_tariff_css, 'content: " : "' ), 'Nested WDC rate CSS must render exactly one space before dash and no extra space before colon.' );
...
pickup_checkout_assert( str_contains( $map_js, 'provider.onPointClick(function (point) { preview(point, { focus: false, forcePopup: true, userAction: true }); });' ) && str_contains( $map_js, '(!popupManuallyClosed || options.forcePopup)' ), 'Clicking an active marker after a manual popup close must force the preview balloon open again.' );
