...
pickup_checkout_assert( str_contains( $checkout_js, 'function requiresRateRefreshAfterPickupSave(point)' ) && str_contains( $checkout_js, "pickupFamily(point) === 'dpd:pickup'" ), 'Only DPD pickup selection should request checkout recalculation after save.' );
pickup_checkout_assert( str_contains( $save_point_source, 'if (pointMatchesDestinationQuick(point, checkoutContext) || !window.WDCPickupApi.resolveLocation)' ) && str_contains( $save_point_source, 'commitPoint(point, method, { updateCheckoutAfterSave: requiresRateRefreshAfterPickupSave(point) });' ), 'Same-FIAS pickup points must save immediately without resolve-location confirm flow, with DPD-only recalculation after save.' );
pickup_checkout_assert( 3 === substr_count( $save_point_source, 'commitPoint(point, method, { updateCheckoutAfterSave: requiresRateRefreshAfterPickupSave(point) });' ), 'Same-city quick match, resolve-location no-change, and resolve-location fallback must all use the DPD-only update_checkout guard.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function refreshDistancesFromOrigin()' ) && str_contains( $map_js, 'visiblePoints = sortPoints(enrichPoints(visiblePoints));' ) && str_contains( $map_js, 'provider.renderMarkers(visiblePoints' ) && str_contains( $map_js, 'renderList(visiblePoints);' ), 'Changing pickup distance origin must recalculate, sort, rerender markers, and rerender list before nearest text is shown.' );
