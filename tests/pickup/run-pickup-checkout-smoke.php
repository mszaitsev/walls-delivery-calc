...
pickup_checkout_assert( str_contains( $modal_js, 'title="Определить моё местоположение"' ) && str_contains( $modal_js, 'aria-label="Определить моё местоположение"' ) && str_contains( $modal_js, '<span aria-hidden="true">⌖</span>' ), 'Pickup geolocation overlay control must expose title, aria-label, and a locate icon.' );
...
pickup_checkout_assert( str_contains( $geolocation_setup_source, 'function resetGeolocationButton()' ) && str_contains( $geolocation_setup_source, "geolocationButton.classList.remove('is-loading')" ) && str_contains( $geolocation_setup_source, "geolocationButton.title = 'Определить моё местоположение'" ) && str_contains( $geolocation_setup_source, '<span aria-hidden="true">⌖</span>' ), 'Geolocation success/error must restore the overlay control default icon and title.' );
...
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-search' ) && str_contains( $map_css, '.wdc-pickup-search__icon' ) && str_contains( $map_css, '.wdc-pickup-search__button' ) && ! str_contains( $map_css, '.wdc-pickup-search__geo' ), 'Pickup search row must style only icon, input, and submit button.' );
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-modal__map-pane' ) && str_contains( $map_css, 'position: relative' ) && str_contains( $map_css, '.wdc-pickup-map__locate' ) && str_contains( $map_css, 'right: 16px' ) && str_contains( $map_css, 'bottom: 16px' ) && str_contains( $map_css, '.wdc-pickup-map__locate.is-loading' ), 'Pickup CSS must position and style the map overlay geolocation control and loading state.' );
