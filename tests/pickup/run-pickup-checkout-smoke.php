...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'renderSearchMarker' ) && str_contains( $leaflet_provider_js, 'wdc-map-search-pin' ) && ! str_contains( $leaflet_provider_js, 'wdc-map-marker-pin--search' ), 'Leaflet search marker must use a separate red pin class, not the pickup marker class.' );
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'interactive: false' ) && str_contains( $leaflet_provider_js, 'zIndexOffset: -100' ), 'Leaflet search marker must be non-clickable and below pickup markers.' );
...
pickup_checkout_assert( str_contains( $yandex_provider_js, 'searchMarkerLayout' ) && str_contains( $yandex_provider_js, 'wdc-map-search-pin' ) && str_contains( $yandex_provider_js, 'wdcShift' ), 'Yandex search marker must use a separate layout/class and not become a pickup marker.' );
...
pickup_checkout_assert( str_contains( $map_css, '.wdc-map-search-pin' ) && str_contains( $map_css, '#e53935' ) && str_contains( $map_css, '.wdc-map-search-icon' ) && str_contains( $map_css, 'z-index: 0 !important' ) && str_contains( $map_css, '.wdc-map-search-pin.is-shift-right' ), 'CSS must render a distinct red search pin with lower z-index and shift classes.' );
