...
pickup_checkout_assert( str_contains( $map_js, 'listMeta(points.length, points.length)' ) && ! str_contains( $map_js, 'listMeta(points.length, shown.length)' ), 'Map list meta must report the real backend point count without a capped shown count.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function applySearchResult(result)' ) && str_contains( $map_js, 'provider.renderMarkers(visiblePoints' ) && str_contains( $map_js, "card.textContent = labels.addressFound || 'Адрес найден.'" ), 'Successful addressSearch must apply the search marker without replacing visiblePoints from result.points.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function pointSnapshot(point)' ) && str_contains( $map_js, 'JSON.parse(snapshot)' ) && str_contains( $map_js, 'function pointMatchKeys(point)' ) && str_contains( $map_js, 'var snapshot = pointSnapshot(point)' ) && str_contains( $map_js, 'point && point.display_code' ) && str_contains( $map_js, 'snapshot.display_code' ) && str_contains( $map_js, 'snapshot.postcode' ) && str_contains( $map_js, 'snapshot.postal_code' ) && str_contains( $map_js, 'snapshot.point_postcode' ), 'Checkout selected Russian Post matching must include display_code/postcode fields from top-level, object snapshot, and JSON-string snapshot payloads.' );
...
pickup_checkout_assert( str_contains( $map_js, 'var matchingPreviewPoint = previewPoint ? matchingPoint(previewPoint, visiblePoints) : null' ) && str_contains( $map_js, 'previewPoint = matchingPreviewPoint' ) && str_contains( $map_js, 'committedPoint = matchingPreviewPoint' ) && ! str_contains( $map_js, 'if (committedPoint && pointInList(committedPoint, [matchingPreviewPoint]))' ) && str_contains( $map_js, "list.querySelectorAll('[data-wdc-point-id]')" ) && str_contains( $map_js, 'scrollListRowIntoView(previewPoint)' ), 'Checkout preview restore must replace the saved point with the matching REST row before active marker/list scroll.' );
