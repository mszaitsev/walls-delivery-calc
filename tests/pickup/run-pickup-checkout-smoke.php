...
pickup_checkout_assert( str_contains( $pickup_docs, 'Known checkout styling conflict' ) && str_contains( $pickup_docs, 'Оплата по счету от ИП/ООО' ), 'Pickup docs must mention the known external plugin styling conflict.' );
...
pickup_checkout_assert( str_contains( $modal_js, 'data-wdc-geolocation' ) && str_contains( $modal_js, 'Моё местоположение' ) && str_contains( $modal_js, 'hidden>Моё местоположение' ), 'Pickup modal search template must include the initially hidden geolocation button.' );
...
pickup_checkout_assert( str_contains( $geolocation_setup_source, 'geolocationButton.disabled = true' ) && str_contains( $geolocation_setup_source, "map.setStatus('Определяем местоположение...')" ) && str_contains( $geolocation_setup_source, "window.navigator.geolocation.getCurrentPosition(function (position)" ), 'Geolocation click must disable the button, show a short status, and call getCurrentPosition.' );
...
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-search' ) && str_contains( $map_css, '.wdc-pickup-search__icon' ) && str_contains( $map_css, '.wdc-pickup-search__button' ) && str_contains( $map_css, '.wdc-pickup-search__geo' ), 'Pickup search row must style icon, input, submit button, and geolocation button.' );
...
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-list__status' ) && str_contains( $map_css, '.wdc-pickup-list__status.is-error' ) && str_contains( $map_css, '.wdc-map-user-marker' ), 'Pickup CSS must style geolocation list statuses and the user marker.' );
...
pickup_checkout_assert( str_contains( $yandex_provider_js, 'userMarkerLayout' ) && str_contains( $yandex_provider_js, 'renderUserLocationMarker' ) && str_contains( $yandex_provider_js, 'wdc-map-user-marker' ) && str_contains( $yandex_provider_js, "interactivityModel: 'default#transparent'" ), 'Yandex provider must render a transparent-interaction user location marker separate from pickup markers.' );
