...
$GLOBALS['wpdb']->locations = array(
	array( 'id' => 100, 'fias_id' => '11111111-1111-1111-1111-111111111111', 'gar_id' => '100', 'gar_object_id' => 100, 'country_code' => 'RU', 'region_name' => 'NSO', 'region_code' => '54', 'city_name' => 'Novosibirsk', 'settlement_name' => 'Novosibirsk', 'display_name' => 'Novosibirsk', 'postal_code' => '630001', 'latitude' => 55.01, 'longitude' => 82.91, 'active' => 1 ),
...
pickup_checkout_assert( str_contains( $checkout_js, 'function runCrossLocationSelection(point, location)' ) && str_contains( $checkout_js, 'var updatedCheckout = waitForUpdatedCheckout(12000)' ) && str_contains( $checkout_js, 'updatedCheckout.then(function ()' ) && str_contains( $checkout_js, 'commitCrossLocationPoint(point, selectedRate.methodId)' ), 'Cross-location flow must wait for updated_checkout before saving the pending point.' );
$cross_flow_source = substr( $checkout_js, strpos( $checkout_js, 'function runCrossLocationSelection(point, location)' ), 1400 );
pickup_checkout_assert( strpos( $cross_flow_source, 'var updatedCheckout = waitForUpdatedCheckout(12000)' ) < strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) && strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) < strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) && strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) < strpos( $cross_flow_source, 'commitCrossLocationPoint(point, selectedRate.methodId)' ), 'Cross-location flow must listen for updated_checkout, update checkout, then save the selected pickup point.' );
pickup_checkout_assert( str_contains( $checkout_js, 'selectCheapestPickupRate' ) && str_contains( $checkout_js, 'methodId:' ) && str_contains( $checkout_js, 'changed:' ) && str_contains( $checkout_js, 'var changed = !selected.checked' ) && str_contains( $checkout_js, 'isPickupRateValue(input.value || \'\')' ) && str_contains( $checkout_js, 'rates.sort(function (a, b)' ) && str_contains( $checkout_js, 'rateCost(a) - rateCost(b)' ) && str_contains( $checkout_js, 'rateCostFromValue' ), 'Cross-location flow must choose the cheapest available Russian Post pickup rate and report whether it changed.' );
pickup_checkout_assert( str_contains( $checkout_js, 'if (!selectedRate.changed)' ) && str_contains( $checkout_js, 'setLoading(\'Пересчитываем тариф...\')' ) && str_contains( $checkout_js, 'waitForUpdatedCheckout(12000).then(function ()' ) && str_contains( $checkout_js, 'Не удалось завершить пересчет доставки. Попробуйте выбрать пункт выдачи еще раз.' ), 'Cross-location flow must wait for a second updated_checkout after changing pickup rate and handle timeout without saving.' );
pickup_checkout_assert( str_contains( $checkout_js, 'После пересчета выбранный способ доставки стал недоступен' ) && str_contains( $checkout_js, 'resetPickupSelectionOnServer();' ), 'Cross-location flow must show a notice and avoid saving the point when pickup rates disappear.' );
pickup_checkout_assert( str_contains( $checkout_js, 'enableDestinationResetSuppression' ) && str_contains( $checkout_js, 'disableDestinationResetSuppression' ) && str_contains( $checkout_js, 'suppressDestinationResetTimer' ) && ! str_contains( $checkout_js, 'setTimeout(function () { suppressNextDestinationReset = false; }, 0)' ), 'Confirmed cross-location pickup flow must suppress destination reset until checkout recalculation or timeout.' );
pickup_checkout_assert( str_contains( $checkout_js, 'setCheckoutFieldValue' ) && str_contains( $checkout_js, 'wdc_platform_location_fias_id' ), 'Confirmed cross-location pickup flow must update checkout fields without resetting the selected point.' );
