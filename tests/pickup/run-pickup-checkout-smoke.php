...
pickup_checkout_assert( str_contains( $map_js, 'function pointMatchKeys(point)' ) && str_contains( $map_js, 'point && point.cdek_code' ) && str_contains( $map_js, 'point && point.delivery_point' ) && str_contains( $map_js, 'point && point.postcode' ) && str_contains( $map_js, 'point && point.postal_code' ) && str_contains( $map_js, 'function hasSharedPointKey(left, right)' ), 'Checkout selected pickup matching must use carrier-specific codes/postcodes, not only id.' );
