...
pickup_checkout_assert( str_contains( $checkout_js, 'window.WDCPickupApi.save(pointId, method, window.wdcPickupCheckout && window.wdcPickupCheckout.selectedPickupPoint).catch(function () {});' ) && ! str_contains( $checkout_js, "catch(function () { clearPickupSelectionUi" ), 'Same-family rate sync must not clear selected pickup UI if REST save fails.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function containerMethod(container)' ) && str_contains( $checkout_js, 'function pickupMatchesFamily(point, method)' ) && str_contains( $checkout_js, "family === 'cdek:pickup'" ) && str_contains( $checkout_js, "family === 'russian_post_domestic:pickup'" ), 'Checkout pickup UI must keep active state carrier-aware for CDEK and Russian Post families.' );
pickup_checkout_assert( str_contains( $checkout_js, 'clearContainerSelection(container)' ) && str_contains( $checkout_js, 'containerSelectedPointCode(container)' ) && str_contains( $checkout_js, "button.disabled = !visible" ), 'Switching shipping method must hide inactive pickup cards and re-enable the active empty chooser.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function runCrossLocationSelection(point, location)' ) && str_contains( $checkout_js, 'enableDestinationResetSuppression(60000)' ) && str_contains( $checkout_js, 'var updatedCheckout = waitForUpdatedCheckout(60000)' ) && str_contains( $checkout_js, 'updatedCheckout.then(function ()' ) && str_contains( $checkout_js, 'window.WDCPickupApi.save(point.id, currentMethod, pointPayload(point))' ), 'Cross-location flow must close the map, wait for updated_checkout, then save the pending point.' );
$cross_flow_source = substr( $checkout_js, strpos( $checkout_js, 'function runCrossLocationSelection(point, location)' ), 1400 );
pickup_checkout_assert( false !== strpos( $cross_flow_source, 'applyConfirmedPickupLocationChange(location);' ) && strpos( $cross_flow_source, 'applyConfirmedPickupLocationChange(location);' ) < strpos( $cross_flow_source, 'close();' ) && strpos( $cross_flow_source, 'close();' ) < strpos( $cross_flow_source, 'var updatedCheckout = waitForUpdatedCheckout(60000)' ) && strpos( $cross_flow_source, 'var updatedCheckout = waitForUpdatedCheckout(60000)' ) < strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) && strpos( $cross_flow_source, 'triggerCheckoutUpdate();' ) < strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) && strpos( $cross_flow_source, 'updatedCheckout.then(function ()' ) < strpos( $cross_flow_source, 'window.WDCPickupApi.save(point.id, currentMethod, pointPayload(point))' ), 'Cross-location flow must update destination, close the modal, listen for updated_checkout, update checkout, then save the selected pickup point.' );
...
pickup_checkout_assert( str_contains( $commit_source, 'window.WDCPickupApi.save(point.id, shippingMethodId || method, pointPayload(point))' ) && str_contains( $commit_source, 'applySelection(container, response.pickup_point || {})' ) && str_contains( $commit_source, 'close();' ), 'Same-city save must still call REST save, apply the selected point UI, and close the modal.' );
