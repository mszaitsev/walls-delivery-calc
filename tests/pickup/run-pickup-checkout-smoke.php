...
pickup_checkout_assert( ! isset( $localized_minimal['initialContext']['selectedPoint'] ), 'Initial context must not expose a minimal pickup point without address as a valid selected point.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'window.WDCPickupApi.save(pointId, method, selected).catch(function () {});' ) && str_contains( $checkout_js, 'pickupMatchesFamily(selected, method)' ) && ! str_contains( $checkout_js, "catch(function () { clearPickupSelectionUi" ), 'Same-family rate sync must save only a same-family selected point and must not clear selected pickup UI if REST save fails.' );
