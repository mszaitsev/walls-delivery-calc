...
WC()->session->set( 'chosen_shipping_methods', array( RussianPostDomesticSettings::PICKUP_SERVICE_KEY . ':air' ) );
$errors = new WdcPickupCheckoutErrors();
( new CheckoutValidation( $session, null, $repo ) )->validate( array( 'shipping_city' => 'Новосибирск', 'wdc_pickup_point_id' => '10', 'wdc_pickup_point_code' => '630001-a' ), $errors );
pickup_checkout_assert( array() === $errors->errors, 'validation must restore Russian Post pickup selection from submitted hidden point fields when the session is empty.' );
...
pickup_checkout_assert( str_contains( $validation_source, 'update_pickup_selection_rate_id( $selected_rate_id )' ) && str_contains( $validation_source, "['_selected_rate_id']" ), 'Checkout validation must pass same-family pickup selections and refresh the stored rate_id to the selected rate suffix.' );
...
pickup_checkout_assert( str_contains( $address_runtime_source, 'should_preserve_pickup_selection_for_rate_switch' ) && str_contains( $address_runtime_source, 'update_pickup_selection_rate_id' ) && str_contains( $address_runtime_source, 'clear_pickup_selection' ), 'Address runtime must preserve pickup selection during same-family rate recalculation and only clear it for real destination/carrier changes.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function resetSelection(reason)' ) && str_contains( $checkout_js, 'function resetPickupSelectionOnServer(reason)' ) && str_contains( $checkout_js, 'function clearPickupSelectionUi(reason)' ) && substr_count( $checkout_js, 'if (isPlacingOrder)' ) >= 3, 'JS reset functions must all no-op while place_order is active.' );
