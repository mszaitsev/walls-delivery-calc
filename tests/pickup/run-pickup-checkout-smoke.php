...
pickup_checkout_assert( str_contains( $session_source, 'clear_pickup_selection_blocked' ) && str_contains( $session_source, 'clear_pickup_selection' ) && str_contains( $session_source, "'current_rate_family'" ) && str_contains( $session_source, "'session_has_pickup'" ) && ! str_contains( $session_source, "'selection_rate_id'" ) && ! str_contains( $session_source, "'has_point'" ), 'Checkout session clear debug context must be compact and avoid selection dumps.' );
...
pickup_checkout_assert( strpos( $rate_renderer_source, 'wdc-domestic-tariff-selector__days' ) < strpos( $rate_renderer_source, 'wdc-domestic-tariff-selector__price' ) && strpos( $rate_renderer_source, 'wdc-domestic-tariff-selector__price' ) < strpos( $rate_renderer_source, 'wdc-domestic-tariff-selector__crossed-price' ), 'Domestic tariff selector markup must render rate name, days, price, and crossed price in visual order.' );
pickup_checkout_assert( str_contains( $rate_renderer_source, 'wdc-domestic-tariff-selector__separator" aria-hidden="true"> - </span>' ) && str_contains( $rate_renderer_source, 'wdc-domestic-tariff-selector__separator" aria-hidden="true">: </span>' ), 'Domestic tariff selector must render explicit PHP separators between title/days and days/price.' );
...
$tariff_selector->invoke( $rate_renderer, array(
	...
	'tariff_variants' => array(
		array(
			...
			'planned_delivery_comment' => '7-8 дней',
			'price_rub' => 488,
			'crossed_price' => array( 'amount_kopecks' => 85671 ),
		),
		array(
...
pickup_checkout_assert( str_contains( $tariff_html, '<span class="wdc-domestic-tariff-selector__title">Посылка онлайн</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true"> - </span><span class="wdc-domestic-tariff-selector__days">7-8 дней</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true">: </span><span class="wdc-domestic-tariff-selector__price">488 руб.</span>' ), 'Domestic tariff selector HTML must render title, " - ", days, ": ", and price in exact order.' );
pickup_checkout_assert( str_contains( $tariff_html, '<span class="wdc-domestic-tariff-selector__title">Без срока</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true">: </span><span class="wdc-domestic-tariff-selector__price">100 руб.</span>' ) && ! str_contains( $tariff_html, '<span class="wdc-domestic-tariff-selector__title">Без срока</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true"> - </span>' ), 'Domestic tariff selector must not render the dash separator when delivery days are empty.' );
pickup_checkout_assert( str_contains( $tariff_html, '<span class="wdc-domestic-tariff-selector__title">Без цены</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true"> - </span><span class="wdc-domestic-tariff-selector__days">2 дня</span></label>' ) && ! str_contains( $tariff_html, '<span class="wdc-domestic-tariff-selector__days">2 дня</span><span class="wdc-domestic-tariff-selector__separator" aria-hidden="true">: </span>' ), 'Domestic tariff selector must not render the colon separator when price is empty.' );
...
pickup_checkout_assert( ! str_contains( $domestic_tariff_css, 'justify-content: space-between' ) && ! str_contains( $domestic_tariff_css, 'margin-left: auto' ) && ! str_contains( $domestic_tariff_css, 'flex: 1 1' ) && str_contains( $domestic_tariff_css, 'flex: 0 1 auto' ), 'Nested WDC rate CSS must keep title, days, price, and crossed price in natural inline flow without right alignment.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function resetSelection(reason)' ) && str_contains( $checkout_js, 'function resetPickupSelectionOnServer(reason)' ) && str_contains( $checkout_js, 'function clearPickupSelectionUi(reason)' ) && substr_count( $checkout_js, 'if (isPickupResetGuarded())' ) >= 3, 'JS reset functions must all no-op while place_order or guard grace is active.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'previousFamily' ) && str_contains( $checkout_js, 'nextFamily' ) && str_contains( $checkout_js, "resetSelection('method_family_changed')" ), 'JS must reset only when the shipping method family actually changes.' );
pickup_checkout_assert( str_contains( $checkout_js, 'window.WDCPickupApi.save(pointId, method).catch(function () {});' ) && ! str_contains( $checkout_js, "catch(function () { clearPickupSelectionUi" ), 'Same-family rate sync must not clear selected pickup UI if REST save fails.' );
...
pickup_checkout_assert( str_contains( $checkout_js, "debug('updated_checkout skipped context refresh during place order'" ) && str_contains( $checkout_js, "window.jQuery(document.body).on('checkout_error', releasePlaceOrderGuardSoon)" ), 'updated_checkout must not refresh checkout context while place_order is guarded; checkout_error releases the guard after grace period.' );
pickup_checkout_assert( str_contains( $checkout_js, "debug('resetSelection called'" ) && str_contains( $checkout_js, "debug('resetPickupSelectionOnServer called'" ) && str_contains( $checkout_js, "debug('shipping method change'" ), 'JS must log reset reasons and shipping method family changes when debug is enabled.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'var latSource = validCoordinate(fieldContext.lat, fieldContext.lng)' ) && str_contains( $checkout_js, "debugDeep('chosen lat/lng source'" ), 'Initial context deep debug must report the chosen coordinate source.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function debugDeep()' ) && str_contains( $checkout_js, 'window.wdcPickupCheckout.deepDebug' ), 'Pickup checkout JS must expose verbose context logging only behind deepDebug.' );
pickup_checkout_assert( ! str_contains( $checkout_js, "debug('contextFromFields'" ) && ! str_contains( $checkout_js, "debug('currentContext'" ) && ! str_contains( $checkout_js, "debug('prefetch cache key'" ) && ! str_contains( $checkout_js, "debug('openModal context'" ), 'Normal frontend debug must not log noisy context or prefetch payloads.' );
pickup_checkout_assert( str_contains( $checkout_js, "debugDeep('contextFromFields'" ) && str_contains( $checkout_js, "debugDeep('currentContext'" ) && str_contains( $checkout_js, "debugDeep('prefetch cache key'" ) && str_contains( $checkout_js, "debugDeep('openModal context'" ), 'Frontend deepDebug must retain detailed context diagnostics when explicitly enabled.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'function clearPickupSelectionUi(reason)' ) && str_contains( $checkout_js, 'function resetPickupSelectionOnServer(reason)' ), 'Pickup reset must split UI clear and server reset.' );
...
pickup_checkout_assert( str_contains( $commit_source, 'window.WDCPickupApi.save(point.id, shippingMethodId || method)' ) && str_contains( $commit_source, 'applySelection(container, response.pickup_point || {})' ) && str_contains( $commit_source, 'close();' ), 'Same-city save must still call REST save, apply the selected point UI, and close the modal.' );
...
pickup_checkout_assert( str_contains( $save_point_source, 'if (pointMatchesDestinationQuick(point, checkoutContext) || !window.WDCPickupApi.resolveLocation)' ) && str_contains( $save_point_source, 'commitPoint(point, method, { updateCheckoutAfterSave: false });' ), 'Same-FIAS pickup points must save immediately without resolve-location confirm flow or checkout recalculation.' );
pickup_checkout_assert( 3 === substr_count( $save_point_source, 'commitPoint(point, method, { updateCheckoutAfterSave: false });' ), 'Same-city quick match, resolve-location no-change, and resolve-location fallback must all save without update_checkout.' );
...
pickup_checkout_assert( str_contains( $checkout_js, "debugDeep('initialContext selected source'" ), 'Initial context deep debug must report the selected source.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'debugDeep(\'refreshCheckoutContextOnce result\'' ) && str_contains( $checkout_js, 'debugDeep(\'openModal context\'' ), 'Pickup deep debug logs must include state refresh and open context.' );
...
pickup_checkout_assert( str_contains( $map_js, 'loadBounds(bboxAround(searchAddress.lat, searchAddress.lng), {' ) && str_contains( $map_js, 'force: true' ) && ! str_contains( $map_js, 'previewNearest: true' ), 'Frontend must force a bbox load around the found address without auto-previewing the nearest point.' );
...
pickup_checkout_assert( str_contains( $map_js, 'renderMarkers(points, labels.empty || \'\')' ) && str_contains( $map_js, 'if (options.previewNearest && visiblePoints[0])' ), 'Bbox response after search must replace visiblePoints; nearest preview remains opt-in only.' );
...
pickup_checkout_assert( str_contains( $geolocation_map_source, 'searchAddress = null' ) && str_contains( $geolocation_map_source, 'provider.setCenter(lat, lng, 15)' ) && str_contains( $geolocation_map_source, 'loadBounds(bboxAround(lat, lng), { force: true })' ), 'Geolocation success must replace any address origin marker, center the map, and force bbox load around user coordinates.' );
...
pickup_checkout_assert( str_contains( $map_js, 'suppressNextMapClick = true' ) && str_contains( $map_js, 'popupManuallyClosed = false;' ) && str_contains( $map_js, 'preview(point, { focus: false, forcePopup: true, userAction: true });' ) && str_contains( $map_js, '(!popupManuallyClosed || options.forcePopup)' ), 'Clicking an active marker after a manual popup close must clear the manual flag and force the preview balloon open again.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'marker.openPopup();' ) && str_contains( $leaflet_provider_js, 'window.setTimeout(function () { suppressPopupClose = false; }, 0);' ), 'Leaflet popup reopen must suppress close events through the reopen tick so the X-close state cannot block marker click.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'activePointId = id' ) && str_contains( $leaflet_provider_js, 'updateActiveMarkers();' ) && str_contains( $leaflet_provider_js, 'popupAnchor: [0, -58]' ) && str_contains( $leaflet_provider_js, 'offset: window.L.point(0, -5)' ), 'Leaflet provider must keep the active marker visible/red and anchor popup above it.' );
