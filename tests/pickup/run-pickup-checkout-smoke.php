...
pickup_checkout_assert( str_contains( $map_js, 'var yandexCityListMode = isYandexDeliveryContext(context)' ) && str_contains( $map_js, 'function pointInsideBounds(point, bbox)' ) && str_contains( $map_js, 'return yandexCityListMode || !viewportReloadRequired();' ) && str_contains( $map_js, 'return !validPointCoordinates(point) || pointInsideBounds(point, lastBbox);' ) && str_contains( $map_js, 'renderList(listPointsForCurrentBounds(), visiblePoints.length)' ), 'Generic fixed-dataset map lists must filter coordinate-backed points by current viewport bounds while retaining coordinate-less points.' );
