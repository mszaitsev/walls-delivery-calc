...
pickup_checkout_assert( str_contains( $map_js, 'function preview(point)' ) && str_contains( $map_js, 'previewPoint = point' ) && str_contains( $map_js, 'confirmButton.disabled = !selectedPoint || pointId(selectedPoint) !== pointId(point)' ), 'Initial/search preview must not enable confirmation unless it is already the selected point.' );
