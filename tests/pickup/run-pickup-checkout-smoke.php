...
pickup_checkout_assert( false === ( $recovery_state['pickup_rate_capabilities']['ozon_delivery:pickup']['prefetch_points'] ?? true ), 'checkout state GET must expose fixed-area prefetch_points=false separately from destination context.' );
...
$session->save_rates(
	array(
		'ozon_delivery:pickup' => array(
			...
			'rate_meta' => array(
				'pickup_family' => 'ozon_delivery:pickup',
				'pickup_provider_query' => array( 'reload_on_viewport_change' => false, 'prefetch_points' => false ),
			),
		),
	)
);
...
pickup_checkout_assert( false === ( $localized_ozon['pickupRateCapabilities']['ozon_delivery:pickup']['reload_on_viewport_change'] ?? true ) && false === ( $localized_ozon['pickup_rate_capabilities']['ozon_delivery:pickup']['reload_on_viewport_change'] ?? true ) && false === ( $localized_ozon['pickupRateCapabilities']['ozon_delivery:pickup']['prefetch_points'] ?? true ), 'Pickup checkout localization must expose fixed-area pickup rate capabilities separately from initialContext.' );
...
$comment_card = $card_renderer->render( array( 'point_title' => 'Тестовый ПВЗ', 'address' => 'ул. Ленина, 15', 'point_comment' => 'Отличный ПВЗ', 'description' => 'Отличный ПВЗ' ), true, false, false );
pickup_checkout_assert( 1 === substr_count( $comment_card, 'Отличный ПВЗ' ) && str_contains( $comment_card, 'Комментарий:' ) && ! str_contains( $comment_card, 'Описание:</span> <span data-wdc-pickup-description-text>Отличный ПВЗ' ), 'Pickup point card renderer must render point_comment once with the Comment label and dedupe identical description.' );
$description_card = $card_renderer->render( array( 'point_title' => 'Legacy point', 'address' => 'ул. Ленина, 15', 'description' => 'Legacy description' ), false, false, false );
pickup_checkout_assert( str_contains( $description_card, 'Описание:' ) && str_contains( $description_card, 'Legacy description' ) && ! str_contains( $description_card, 'Комментарий:' ), 'Pickup point card renderer must preserve generic description behavior when point_comment is absent.' );
...
pickup_checkout_assert( false === $session->clear_pickup_selection_if_allowed( 'address_fingerprint_changed', $courier_group_id ), 'address fingerprint refresh while a courier method is active must not perform a global pickup bucket reset.' );
pickup_checkout_assert( '630001-a' === (string) ( $session->pickup_selections()[ $pickup_group_id ]['point_code'] ?? '' ) && 'KEM7' === (string) ( $session->pickup_selections()['cdek:pickup']['point_code'] ?? '' ), 'courier address fingerprint refresh must preserve dormant pickup family buckets.' );
...
pickup_checkout_assert( str_contains( $validation_source, 'update_pickup_selection_rate_id( $selected_rate_id, $active_family )' ) && str_contains( $validation_source, "['_selected_rate_id']" ) && str_contains( $validation_source, '$data[\'shipping_method\']' ), 'Checkout validation must pass same-family pickup selections, read posted shipping_method, and refresh the stored rate_id to the selected rate suffix using the authoritative pickup family when it is known.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'var contextPromise = refreshCheckoutContextOnce(700, { returnContext: true })' ) && str_contains( $checkout_js, 'return freshContext || initialContext();' ) && str_contains( $checkout_js, 'var modal = window.WDCPickupModal.create(labels);' ) && str_contains( $checkout_js, 'var map = window.WDCPickupMap.create' ), 'Open modal must use the last live-confirmed stable pickup-map opening lifecycle.' );
pickup_checkout_assert( str_contains( $checkout_js, 'function pointPrefetchAllowed(method)' ) && str_contains( $checkout_js, "Object.prototype.hasOwnProperty.call(item, 'prefetch_points')" ) && str_contains( $checkout_js, 'capabilities.prefetch_points === false' ), 'Pickup checkout frontend must disable background point prefetch only through the generic prefetch_points capability.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'var baseContext = Object.assign({}, resolvedContext || {});' ) && str_contains( $checkout_js, 'var containerFamily = containerSelectedPickupFamily(container);' ) && str_contains( $checkout_js, 'var context = withRateCapabilities(withPrefetch(withCarrierContext(baseContext, method), method), method)' ) && str_contains( $checkout_js, 'function prefetchIdentity(context, method)' ) && str_contains( $checkout_js, 'function prefetchIdentityMatches(cached, current)' ) && str_contains( $checkout_js, 'preloadedPoints: prefetchCache.points' ), 'Open modal must pass only destination/method/family-matched cached preloaded points to the map and apply active rate capabilities after destination context merge.' );
...
pickup_checkout_assert( str_contains( $api_js, 'function requestUrl(base, path)' ) && str_contains( $api_js, "url.indexOf('?') === -1 ? '?' + query : '&' + query" ), 'Pickup API URL builder must support WordPress plain-permalink rest_route query syntax.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function presentationComment(point)' ) && str_contains( $map_js, 'function pointComment(point)' ) && str_contains( $map_js, 'wdc-pickup-popup__title-comment' ) && str_contains( $map_js, 'wdc-pickup-list__title-comment' ) && str_contains( $map_js, 'wdc-pickup-list__comment' ), 'Map popup and side list must keep presentation_comment distinct from ordinary point_comment.' );
pickup_checkout_assert( str_contains( $map_js, 'function pointInsideBounds(point, bbox)' ) && str_contains( $map_js, 'function listFollowsViewport()' ) && str_contains( $map_js, 'return true;' ) && str_contains( $map_js, 'return validPointCoordinates(point) && (!lastBbox || pointInsideBounds(point, lastBbox));' ) && str_contains( $map_js, 'renderList(listPointsForCurrentBounds(), visiblePoints.length)' ), 'Every carrier sidebar must be the current viewport projection and must exclude coordinate-less points independently of reload_on_viewport_change.' );
...
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-popup__title-comment' ) && str_contains( $map_css, '.wdc-pickup-list__title-comment' ) && str_contains( $map_css, '.wdc-pickup-list__comment' ), 'Pickup CSS must style presentation_comment in popup/list and ordinary point_comment in the side list.' );
...
pickup_checkout_assert( str_contains( $map_js, 'data-wdc-point-id' ) && str_contains( $map_js, 'wdc-pickup-list__item' ) && str_contains( $map_js, 'SIDEBAR_POINT_LIMIT = 100' ) && str_contains( $map_js, 'points = points.slice(0, SIDEBAR_POINT_LIMIT)' ) && str_contains( $map_js, 'provider.renderMarkers(visiblePoints' ), 'Map modal must cap only sidebar cards while rendering the full pickup result as markers.' );
pickup_checkout_assert( str_contains( $map_js, 'listMeta(totalCount, points.length)' ) && str_contains( $map_js, "return 'Показано ' + shown + ' из ' + total" ) && str_contains( $map_js, 'Показаны ближайшие 100 пунктов.' ) && str_contains( $map_css, '.wdc-pickup-list__overflow' ), 'Map list meta and overflow hint must explain the 100-card presentation cap against the full loaded point count.' );
...
pickup_checkout_assert( str_contains( $api_js, "points/address-search?" ) && str_contains( $map_js, 'dataSource.addressSearch(query, context' ) && ! str_contains( $map_js, 'dataSource.search(query, context' ), 'Pickup address search must use the injectable address-search adapter and never textual pickup-point search.' );
...
pickup_checkout_assert( str_contains( $map_js, "carrier === 'yandex_delivery' || family === 'yandex_delivery:pickup'" ) && str_contains( $map_js, "['station:', point && point.platform_station_id]" ) && str_contains( $map_js, "['station:', snapshot.platform_station_id]" ) && str_contains( $map_js, "['code:', snapshot.point_code]" ), 'Checkout selected Yandex matching must use namespaced provider station/id/code identity.' );
...
pickup_checkout_assert( str_contains( $map_js, 'provider.openPointPopup(point, renderPointPopup(point, selected), { ensureVisible: false, forceReopen: true });' ) && str_contains( $map_js, 'provider.onPopupSelect(function (point) { commit(point, { focus: false }); });' ), 'Marker clicks must imperatively reopen popups while popup select remains the explicit commit path.' );
$list_click_source = substr( $map_js, strpos( $map_js, "list.addEventListener('click'" ) ?: 0, 420 );
pickup_checkout_assert( str_contains( $list_click_source, 'preview(point, {' ) && str_contains( $list_click_source, 'userAction: true' ) && str_contains( $list_click_source, 'focus: true' ) && str_contains( $list_click_source, 'forcePopup: true' ) && str_contains( $list_click_source, 'ensureVisible: true' ) && ! str_contains( $list_click_source, "CustomEvent('wdc:point-selected'" ) && ! str_contains( $list_click_source, 'commit(point' ), 'Side card click must use explicit focused preview navigation without committing selection.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function initialSearch(query)' ) && str_contains( $map_js, 'return runSearch(query);' ) && ! str_contains( $map_js, 'dataSource.searchInitial' ) && ! str_contains( $map_js, 'dataSource.search(query' ), 'Initial map query must use the same address-search flow and must not preview a textual point-search result.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'var lastPoints = []' ) && str_contains( $leaflet_provider_js, 'lastPoints = Array.isArray(points) ? points.slice() : []' ) && str_contains( $leaflet_provider_js, "map.on('zoomend', scheduleClusterRebuild)" ) && str_contains( $leaflet_provider_js, 'cancelScheduledClusterRebuild' ), 'Leaflet provider must retain the full marker dataset and coalesce grid cluster rebuilds on zoomend.' );
