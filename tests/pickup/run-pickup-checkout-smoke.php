...
pickup_checkout_assert( str_contains( $map_js, 'suppressNextMapClick = true' ) && str_contains( $map_js, 'preview(point, { focus: false, forcePopup: true, userAction: true });' ) && str_contains( $map_js, '(!popupManuallyClosed || options.forcePopup)' ), 'Clicking an active marker after a manual popup close must force the preview balloon open again.' );
