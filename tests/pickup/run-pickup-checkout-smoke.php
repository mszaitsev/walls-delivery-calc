...
pickup_checkout_assert( str_contains( $checkout_js, 'previousFamily' ) && str_contains( $checkout_js, 'nextFamily' ) && str_contains( $checkout_js, 'restoreSelectedPickupUi();' ) && ! str_contains( $checkout_js, "resetSelection('method_family_changed')" ), 'JS must restore the active family bucket on shipping method changes without clearing another carrier selection.' );
pickup_checkout_assert( str_contains( $checkout_js, 'var selected = selectedPickupPointForFamily(shippingMethodFamily(method));' ) && str_contains( $checkout_js, 'window.WDCPickupApi.save(pointId, method, selected).catch(function () {});' ) && str_contains( $checkout_js, 'pickupMatchesFamily(selected, method)' ) && ! str_contains( $checkout_js, "catch(function () { clearPickupSelectionUi" ), 'Same-family rate sync must save only the selected point from the matching pickup_family bucket and must not clear selected pickup UI if REST save fails.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'pickup save payload' ) && str_contains( $checkout_js, 'pickup save response' ) && str_contains( $checkout_js, 'function pickupDebugSummary(point)' ), 'Checkout pickup save must expose debug summaries for carrier/family/code/address when debug is enabled.' );
...
pickup_checkout_assert( str_contains( $checkout_js, 'setSelectedPickupPoint(selectedPoint)' ) && str_contains( $checkout_js, 'window.wdcPickupCheckout.selectedPickupPoints = selectedPickupPoints' ) && str_contains( $checkout_js, 'selectedPoint: activeSelected || config.selectedPoint' ) && str_contains( $checkout_js, 'function normalizeSelectedPoint(point)' ), 'Checkout JS must keep the saved pickup point available by pickup_family for the next map open.' );
...
pickup_checkout_assert( str_contains( $map_js, "carrier === 'russian_post_domestic' || carrier === 'russian_post'" ) && str_contains( $map_js, 'point.postcode || point.postal_code' ) && str_contains( $map_js, "carrier === 'cdek'" ) && str_contains( $map_js, 'point.cdek_code || point.point_code' ), 'Map popup/list titles must show Russian Post postcode and CDEK code.' );
pickup_checkout_assert( str_contains( $domestic_tariff_js, 'function syncDisabledState()' ) && str_contains( $domestic_tariff_js, "wrapper.classList.toggle('is-inactive', !active)" ) && str_contains( $domestic_tariff_js, 'input.disabled = !active' ) && str_contains( $domestic_tariff_js, "input[name^=\"shipping_method\"]" ), 'Grouped tariff JS must disable inactive method rates and re-enable the active family.' );
