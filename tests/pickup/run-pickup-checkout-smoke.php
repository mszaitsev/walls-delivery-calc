...
pickup_checkout_assert( str_contains( $map_css, '.wdc-pickup-modal__map-pane' ) && str_contains( $map_css, 'position: relative' ) && str_contains( $map_css, '.wdc-pickup-map__locate' ) && str_contains( $map_css, 'right: 16px' ) && str_contains( $map_css, 'bottom: 72px' ) && ! str_contains( $map_css, "bottom: 16px;\n\tz-index: 650" ) && str_contains( $map_css, '.wdc-pickup-map__locate.is-loading' ), 'Pickup CSS must position the geolocation overlay above map attribution and keep loading state.' );
