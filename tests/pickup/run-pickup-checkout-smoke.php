...
pickup_checkout_assert( 'Отделение Почты России' === (string) ( $localized_leaflet['pickupPointTypes']['OPS']['label'] ?? '' ) && ! array_key_exists( 'markerLabel', $localized_leaflet['pickupPointTypes']['OPS'] ?? array() ) && ! array_key_exists( 'cardLabel', $localized_leaflet['pickupPointTypes']['OPS'] ?? array() ) && true === (bool) ( $localized_leaflet['pickupPointTypes']['PVZ']['enabled'] ?? false ) && true === (bool) ( $localized_leaflet['pickupPointTypes']['APS']['enabled'] ?? false ), 'JS localization must include default pickupPointTypes label/enabled flags without markerLabel/cardLabel.' );
...
pickup_checkout_assert( str_contains( $point_type_source, "'label' => 'Отделение Почты России'" ) && str_contains( $point_type_source, "russian_post_domestic_pickup_type_{\$key}_card_label" ) && str_contains( $point_type_source, "\$result['OPS']['enabled'] = true" ) && ! str_contains( $point_type_source, '_marker_label' ), 'Pickup type settings must provide one label, read old card_label values, auto-enable OPS, and stop writing marker_label.' );
$legacy_type_settings = new RussianPostPickupPointTypeSettings( new SettingsRepository() );
$legacy_type_settings_values = $legacy_type_settings->sanitize_admin_values( array( 'russian_post_domestic_pickup_type_ops_enabled' => '1', 'russian_post_domestic_pickup_type_ops_card_label' => 'Старое название' ) );
pickup_checkout_assert( 'Старое название' === (string) $legacy_type_settings_values['russian_post_domestic_pickup_type_ops_card_label']['value'] && ! array_key_exists( 'russian_post_domestic_pickup_type_ops_marker_label', $legacy_type_settings_values ), 'Admin save must keep the single card/list/balloon label and stop saving marker_label.' );
...
pickup_checkout_assert( str_contains( $map_js, 'distanceMeters(initialLat, initialLng' ) && str_contains( $map_js, 'formatDistance' ) && str_contains( $map_js, "' м'" ) && str_contains( $map_js, "' км'" ), 'Map list must sort by initial lat/lng distance and format meters/kilometers.' );
...
pickup_checkout_assert( str_contains( $map_js, 'var previewPoint = null' ) && str_contains( $map_js, 'var committedPoint = null' ) && str_contains( $map_js, 'selected: function () { return committedPoint; }' ), 'Map JS must keep previewPoint separate from committed checkout point.' );
...
pickup_checkout_assert( strpos( $map_js, "CustomEvent('wdc:point-selected'" ) > strpos( $map_js, 'function commit(point, options)' ) && strpos( $map_js, "CustomEvent('wdc:point-selected'" ) < strpos( $map_js, 'function closePreview()' ), 'wdc:point-selected must only be dispatched by commit, not preview/renderMarkers.' );
pickup_checkout_assert( str_contains( $map_js, 'provider.onPointClick(function (point) { preview(point, { focus: false }); });' ) && str_contains( $map_js, 'provider.onPopupSelect(function (point) { commit(point, { focus: false }); });' ) && str_contains( $map_js, 'preview(point, { focus: true });' ), 'Marker/list row clicks must open popup previews, while popup select is an explicit commit path.' );
...
pickup_checkout_assert( str_contains( $map_js, 'preview(point, { focus: false })' ) && ! str_contains( $map_js, 'preview(points[0], false)' ), 'Initial search must render only a preview popup.' );
pickup_checkout_assert( str_contains( $map_js, "' preview'" ) && str_contains( $map_js, 'var selected = committedPoint' ) && str_contains( $map_js, "var active = selected || previewed" ), 'Preview row must be highlighted, while selected class must depend on committedPoint.' );
pickup_checkout_assert( str_contains( $map_js, 'var activePoint = committedPoint && pointInList(committedPoint, visiblePoints) ? committedPoint : (previewPoint && pointInList(previewPoint, visiblePoints) ? previewPoint : null)' ), 'Committed point must have active marker priority over previewPoint during marker reload.' );
...
pickup_checkout_assert( str_contains( $map_js, 'data-wdc-pickup-list-select' ) && str_contains( $map_js, 'commit(point, { focus: true });' ), 'List rows must include a compact select button that commits immediately.' );
pickup_checkout_assert( str_contains( $map_js, 'provider.openPointPopup(previewPoint, renderPointPopup(previewPoint' ) && ! str_contains( $map_js, "\n\t\t\tcommittedPoint = null" ), 'renderMarkers must reopen the preview popup after bbox reload and must not reset committedPoint when it leaves the bbox.' );
pickup_checkout_assert( str_contains( $map_js, 'provider.onMapClick(function () { closePreview(); });' ) && str_contains( $map_js, 'function closePreview()' ) && str_contains( $map_js, 'provider.closePopup()' ), 'Map click must close preview popup without resetting committedPoint.' );
pickup_checkout_assert( str_contains( $map_js, 'copy._wdcCardLabel = pointTypeLabel(copy)' ) && str_contains( $map_js, 'config.label || config.cardLabel' ) && ! str_contains( $map_js, '_wdcMarkerLabel' ) && ! str_contains( $map_js, 'pointTypeMarkerLabel' ), 'Frontend must derive card/list/balloon label from label with cardLabel fallback and avoid marker labels.' );
...
pickup_checkout_assert( ! str_contains( $leaflet_provider_js, 'pointMarkerLabel' ) && ! str_contains( $leaflet_provider_js, '_wdcMarkerLabel' ) && ! str_contains( $leaflet_provider_js, 'ОПС' ) && ! str_contains( $leaflet_provider_js, 'ПВЗ' ) && ! str_contains( $leaflet_provider_js, 'Почтомат' ), 'Leaflet marker HTML must not render marker labels or type text.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'openPointPopup' ) && str_contains( $leaflet_provider_js, 'bindPopup' ) && str_contains( $leaflet_provider_js, 'onPopupSelect' ) && str_contains( $leaflet_provider_js, 'onMapClick' ) && str_contains( $leaflet_provider_js, "map.on('moveend zoomend', boundsChanged)" ) && str_contains( $leaflet_provider_js, "map.on('click', mapClicked)" ), 'Leaflet provider must support popups, popup select delegation, and map-click close without closing on drag/move.' );
...
pickup_checkout_assert( str_contains( $yandex_provider_js, 'templateLayoutFactory.createClass' ) && str_contains( $yandex_provider_js, 'wdc-map-marker-pin__inner"></span>' ) && str_contains( $yandex_provider_js, 'wdc-map-marker-pin__tail' ) && ! str_contains( $yandex_provider_js, 'pointMarkerLabel' ) && ! str_contains( $yandex_provider_js, '_wdcMarkerLabel' ), 'Yandex marker custom layout must render textless markers.' );
pickup_checkout_assert( str_contains( $yandex_provider_js, 'openPointPopup' ) && str_contains( $yandex_provider_js, "placemark.properties.set('balloonContent', html)" ) && str_contains( $yandex_provider_js, 'onPopupSelect' ) && str_contains( $yandex_provider_js, 'onMapClick' ) && str_contains( $yandex_provider_js, "map.events.add('click', mapClicked)" ), 'Yandex provider must support point balloons, popup select delegation, and map-click close without closing on boundschange.' );
