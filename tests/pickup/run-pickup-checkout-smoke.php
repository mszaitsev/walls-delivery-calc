...
pickup_checkout_assert( strpos( $map_js, "CustomEvent('wdc:point-selected'" ) > strpos( $map_js, 'function commit(point, options)' ) && strpos( $map_js, "CustomEvent('wdc:point-selected'" ) < strpos( $map_js, 'function markPopupManuallyClosed()' ), 'wdc:point-selected must only be dispatched by commit, not preview/renderMarkers.' );
...
pickup_checkout_assert( str_contains( $map_js, 'function markPopupManuallyClosed()' ) && str_contains( $map_js, 'popupManuallyClosed = true' ) && str_contains( $map_js, 'provider.onPopupClose(function () { markPopupManuallyClosed(); });' ), 'Popup close and empty map click must mark popupManuallyClosed.' );
...
pickup_checkout_assert( str_contains( $map_js, 'provider.onMapClick(function () { markPopupManuallyClosed(); });' ) && str_contains( $map_js, 'function markPopupManuallyClosed()' ) && str_contains( $map_js, 'provider.closePopup()' ), 'Map click must close popup without resetting committedPoint or preview marker/list state.' );
