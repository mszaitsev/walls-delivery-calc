...
pickup_checkout_assert( str_contains( $map_js, 'provider.setActivePoint(pointId(point))' ) && str_contains( $map_js, 'provider.focusPoint(point)' ), 'Map and list selection must synchronize active marker and provider focus.' );
pickup_checkout_assert( str_contains( $map_js, "' active selected'" ), 'Selected list row must receive active/selected CSS classes.' );
pickup_checkout_assert( str_contains( $map_js, 'var previewPoint = null' ) && str_contains( $map_js, 'var selectedPoint = null' ) && str_contains( $map_js, 'selected: function () { return selectedPoint; }' ), 'Map JS must keep previewPoint separate from confirmed selectedPoint.' );
pickup_checkout_assert( str_contains( $map_js, 'function preview(point, options)' ) && str_contains( $map_js, 'previewPoint = point' ) && str_contains( $map_js, 'provider.setActivePoint(pointId(point))' ) && str_contains( $map_js, 'confirmButton.disabled = !selectedPoint || pointId(selectedPoint) !== pointId(point)' ) && str_contains( $map_js, 'provider.openPointPopup(point, renderPointPopup(point' ), 'Marker/list preview must highlight the marker and open a popup without enabling confirmation unless it is already the selected point.' );
pickup_checkout_assert( str_contains( $map_js, 'function select(point, options)' ) && str_contains( $map_js, 'selectedPoint = point' ) && str_contains( $map_js, "CustomEvent('wdc:point-selected'" ), 'Explicit marker/list selection must set selectedPoint and dispatch wdc:point-selected.' );
pickup_checkout_assert( strpos( $map_js, "CustomEvent('wdc:point-selected'" ) > strpos( $map_js, 'function select(point, options)' ) && strpos( $map_js, "CustomEvent('wdc:point-selected'" ) < strpos( $map_js, 'function renderMarkers(points, emptyText)' ), 'wdc:point-selected must only be dispatched by explicit selection, not preview/renderMarkers.' );
pickup_checkout_assert( str_contains( $map_js, 'provider.onPointClick(function (point) { preview(point, { focus: false }); });' ) && str_contains( $map_js, 'provider.onPopupSelect(function (point) { select(point, { focus: false }); });' ) && str_contains( $map_js, 'preview(point, { focus: true });' ), 'Marker/list clicks must open popup previews, while popup select is the explicit selection path.' );
pickup_checkout_assert( strpos( $map_js, 'provider.setActivePoint(pointId(point))' ) > strpos( $map_js, 'function preview(point, options)' ) && strpos( $map_js, 'provider.setActivePoint(pointId(point))' ) < strpos( $map_js, 'function select(point, options)' ), 'Marker/list preview must set active marker without dispatching selection.' );
...
pickup_checkout_assert( str_contains( $map_js, 'activePointId: selectedPoint ? pointId(selectedPoint) : (previewPoint ? pointId(previewPoint) : null)' ), 'Confirmed selection must have active marker priority over previewPoint.' );
...
pickup_checkout_assert( str_contains( $leaflet_provider_js, 'window.L.divIcon' ) && str_contains( $leaflet_provider_js, 'wdc-map-marker-pin' ) && str_contains( $leaflet_provider_js, 'wdc-map-marker-pin__inner' ) && str_contains( $leaflet_provider_js, 'wdc-map-marker-pin__tail' ) && str_contains( $leaflet_provider_js, 'setActivePoint' ) && str_contains( $leaflet_provider_js, 'focusPoint' ), 'Leaflet provider must support custom pin markers and active/focus abstraction.' );
...
pickup_checkout_assert( str_contains( $yandex_provider_js, 'templateLayoutFactory.createClass' ) && str_contains( $yandex_provider_js, 'wdc-map-marker-pin__inner' ) && str_contains( $yandex_provider_js, 'wdc-map-marker-pin__tail' ) && str_contains( $yandex_provider_js, 'wdcLabel: escapeHtml(pointMarkerLabel(point))' ) && str_contains( $yandex_provider_js, 'point._wdcMarkerLabel' ), 'Yandex marker custom layout must use markerLabel from enriched points.' );
pickup_checkout_assert( str_contains( $yandex_provider_js, 'openPointPopup' ) && str_contains( $yandex_provider_js, "placemark.properties.set('balloonContent', html)" ) && str_contains( $yandex_provider_js, 'onPopupSelect' ) && str_contains( $yandex_provider_js, 'data-wdc-pickup-popup-select' ), 'Yandex provider must support opening point balloons and popup select delegation.' );
