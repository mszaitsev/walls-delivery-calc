...
pickup_checkout_assert( str_contains( $checkout_js, 'function containerMatchesActivePickup(container, activeMethodValue, activeFamilyValue)' ) && str_contains( $checkout_js, "document.querySelectorAll('[data-wdc-pickup-checkout]').length === 1" ) && str_contains( $checkout_js, "container.setAttribute('data-shipping-method-id', method)" ) && str_contains( $checkout_js, 'lastRestoreDebug.containerMatches' ), 'Checkout JS must restore active pickup selections into generic or stale pickup containers and expose container matching debug fields.' );
