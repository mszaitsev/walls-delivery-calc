...
$manual_family_as_russian_post = $controller->points( array( 'carrier' => 'russian_post', 'shipping_method_id' => 'manual:manual_nsk', 'pickup_family' => 'manual:manual_nsk:pickup', 'bbox' => '82.90,55.00,82.93,55.03' ) );
pickup_rest_assert( $manual_family_as_russian_post instanceof WP_Error && 'provider_rate_context_mismatch' === $manual_family_as_russian_post->get_error_code(), 'Pickup REST must reject manual pickup family routed as Russian Post instead of returning Russian Post points.' );
...
$postcode_rows = array();
for ( $i = 0; $i < 1100; ++$i ) {
	$postcode_rows[] = array( 'id' => 1000 + $i, 'carrier_key' => 'russian_post', 'point_code' => 'kh-' . $i, 'point_type' => 'OPS', 'address' => 'Хакасия, удаленное отделение ' . $i, 'city_name' => 'Абакан', 'region_name' => 'Хакасия', 'postcode' => '655130', 'latitude' => 53.72 + ( $i % 10 ) * 0.001, 'longitude' => 91.44 + ( $i % 10 ) * 0.001, 'work_time' => '09-18', 'active' => 1 );
}
$postcode_rows[] = array( 'id' => 3001, 'carrier_key' => 'russian_post', 'point_code' => '630049-anchor', 'point_type' => 'OPS', 'address' => 'Новосибирск, Красный проспект, 25', 'city_name' => 'Новосибирск', 'region_name' => 'НСО', 'postcode' => '630049', 'latitude' => 55.0801, 'longitude' => 82.9302, 'work_time' => '09-18', 'active' => 1 );
$postcode_rows[] = array( 'id' => 3002, 'carrier_key' => 'russian_post', 'point_code' => '630049-near', 'point_type' => 'PVZ', 'address' => 'Новосибирск, Дуси Ковальчук, 1', 'city_name' => 'Новосибирск', 'region_name' => 'НСО', 'postcode' => '630049', 'latitude' => 55.0805, 'longitude' => 82.9310, 'work_time' => '10-19', 'active' => 1 );
$postcode_rows[] = array( 'id' => 3003, 'carrier_key' => 'russian_post', 'point_code' => '630050-near', 'point_type' => 'OPS', 'address' => 'Новосибирск, Нарымская, 2', 'city_name' => 'Новосибирск', 'region_name' => 'НСО', 'postcode' => '630050', 'latitude' => 55.0820, 'longitude' => 82.9340, 'work_time' => '10-19', 'active' => 1 );
$GLOBALS['wpdb']->pickup_rows = array_merge( $postcode_rows, $GLOBALS['wpdb']->pickup_rows );
$GLOBALS['wpdb']->tables[ $repo->main_table() ] = $GLOBALS['wpdb']->pickup_rows;
$manual_postcode_result = $controller->address_search( array( 'carrier' => 'manual', 'query' => '630049', 'country_code' => 'RU' ) );
pickup_rest_assert( 'postcode' === $manual_postcode_result['search_type'] && true === $manual_postcode_result['address_search_available'] && array() === $manual_postcode_result['points'] && 'Новосибирск, Красный проспект, 25' === (string) $manual_postcode_result['address']['value'] && 55.0801 === (float) $manual_postcode_result['address']['lat'], 'Non-Russian-Post pickup postcode search must resolve an address marker from the local Russian Post postcode table without DaData.' );
pickup_rest_assert( 3 === count( $GLOBALS['wdc_pickup_rest_http_requests'] ?? array() ), 'Strict RU postcode search for non-Russian-Post carriers must not call DaData.' );
$rp_postcode_630049 = $controller->address_search( array( 'carrier' => 'russian_post', 'query' => '630049', 'country_code' => 'RU' ) );
pickup_rest_assert( 'postcode' === $rp_postcode_630049['search_type'] && 'Новосибирск, Красный проспект, 25' === (string) $rp_postcode_630049['address']['value'], 'Russian Post postcode search must use the exact office row as the address anchor.' );
pickup_rest_assert( count( $rp_postcode_630049['points'] ) >= 3 && array( 3001, 3002, 3003 ) === array_slice( array_column( $rp_postcode_630049['points'], 'id' ), 0, 3 ), 'Russian Post postcode nearest list must start with geographically nearest Novosibirsk rows even when more than 1000 unrelated rows were inserted earlier.' );
pickup_rest_assert( (int) $rp_postcode_630049['points'][0]['distance_meters'] <= (int) $rp_postcode_630049['points'][1]['distance_meters'] && (int) $rp_postcode_630049['points'][1]['distance_meters'] <= (int) $rp_postcode_630049['points'][2]['distance_meters'], 'Russian Post postcode nearest distances must be sorted ascending.' );
...
pickup_rest_assert( str_contains( (string) file_get_contents( dirname( __DIR__, 2 ) . '/src/Pickup/Search/PickupAddressSearchService.php' ), '$nearest = $include_points ? $this->points->find_nearest_rows( $anchor' ), 'Postcode exact matches must expand to nearest points around the anchor only when the carrier endpoint requested points.' );
$GLOBALS['wdc_pickup_rest_http_queue'][] = array(
	'response' => array( 'code' => 200 ),
