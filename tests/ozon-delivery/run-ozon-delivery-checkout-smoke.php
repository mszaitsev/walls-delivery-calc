...
$failure_runtime = new OzonDeliveryCarrier( $settings, $credentials, $failure_service, new Logger(), null, $courier_location_resolver );
...
$tracking_comment = 'Отслеживание посылки - в приложении Ozon, раздел Доставка';
$refusal_109_comment = 'При отказе от посылки после её отправки покупатель оплачивает полную стоимость обратной доставки 109 руб.';
$refusal_149_comment = 'При отказе от посылки после её отправки покупатель оплачивает полную стоимость обратной доставки 149 руб.';
$refusal_decimal_comment = 'При отказе от посылки после её отправки покупатель оплачивает полную стоимость обратной доставки 149,50 руб.';
$snapshot_builder = new DeliveryCustomerCommentSnapshotBuilder();
oz_checkout_assert( array( $tracking_comment ) === $ozon_rate->comments && array( $refusal_109_comment ) === $snapshot_builder->materialized_template_comments( $ozon_rate ), 'Ozon rate must own the tracking comment and generic customer comment template must use final rate price when promo crossed_price is absent.' );
oz_checkout_assert( array( $tracking_comment ) === $two_rate_quote->rates[1]->comments && array( $refusal_109_comment ) === $snapshot_builder->materialized_template_comments( $two_rate_quote->rates[1] ), 'Ozon customer comment templates must apply to the courier rate as well as pickup.' );
oz_checkout_assert( array( $refusal_149_comment ) === $snapshot_builder->materialized_template_comments( oz_checkout_rate_with_price( $ozon_rate, 9900, 14900 ) ), 'Generic customer comment template must use crossed_price as the refusal amount when promo shipping applies.' );
oz_checkout_assert( array( $refusal_decimal_comment ) === $snapshot_builder->materialized_template_comments( oz_checkout_rate_with_price( $ozon_rate, 9900, 14950 ) ), 'Generic customer comment template must format refusal amount with comma kopecks and no float drift.' );
...
$builder_property = $orchestrator_reflection->getProperty( 'customer_comment_snapshot_builder' );
$builder_property->setAccessible( true );
$builder_property->setValue( $orchestrator_stub, $snapshot_builder );
$template_method = $orchestrator_reflection->getMethod( 'rate_with_materialized_customer_comment_templates' );
$template_method->setAccessible( true );
$snapshot_method = $orchestrator_reflection->getMethod( 'rate_with_customer_comment_snapshot' );
$snapshot_method->setAccessible( true );
$commented_rate = $snapshot_method->invoke( $orchestrator_stub, $template_method->invoke( $orchestrator_stub, oz_checkout_rate_with_price( $ozon_rate, 9900, 14900 ), count( $ozon_rate->comments ) ) );
$expected_ozon_comments = array(
	array( 'type' => 'text', 'text' => $tracking_comment ),
	array( 'type' => 'text', 'text' => $refusal_149_comment ),
);
oz_checkout_assert( $commented_rate instanceof DeliveryRate && $expected_ozon_comments === ( $commented_rate->meta['customer_comments'] ?? null ), 'Generic orchestrator snapshot must preserve carrier-owned Ozon customer comments in canonical structured rate meta.' );
...
oz_checkout_assert( in_array( $tracking_comment, $mapped_commented_rate['comments'] ?? array(), true ) && in_array( $refusal_149_comment, $mapped_commented_rate['comments'] ?? array(), true ) && $expected_ozon_comments === ( $mapped_commented_rate['customer_comments'] ?? null ), 'WooCommerce mapper must expose Ozon comments for checkout rendering and authoritative canonical customer comment persistence.' );
...
$same_quote = ( new OzonDeliveryCarrier( $settings, $credentials, $quote_service, new Logger(), null, $courier_location_resolver ) )->quote( $mapped_request );
...
$other_runtime = new OzonDeliveryCarrier( $settings, $credentials, $quote_service, new Logger(), null, new OzonDeliveryCourierLocationResolver( new LocationRepository( $other_location_db ), $pickup_repository ) );
...
oz_checkout_assert( $preliminary_cache_context['ozon_delivery_courier_location_fingerprint'] !== ( $other_runtime->quote_cache_context( $other_request )['ozon_delivery_courier_location_fingerprint'] ?? null ), 'Ozon courier cache fingerprint must change when selected location coordinates change.' );
$no_coordinate_db = new wpdb();
...
$no_coordinate_runtime = new OzonDeliveryCarrier( $settings, $credentials, $no_coordinate_service, new Logger(), null, $no_coordinate_resolver );
$courier_only_missing = $no_coordinate_runtime->quote( oz_checkout_with_delivery_type( $mapped_request, DeliveryType::COURIER ) );
oz_checkout_assert( ! $courier_only_missing->success && array() === $courier_only_missing->rates && array() === $no_coordinate_http->calls && 'ozon_courier_location_coordinates_missing' === $courier_only_missing->error_code, 'Ozon courier must be unavailable without DB location coordinates and must not call order_checkout or browser-coordinate fallback.' );
$pickup_survives_missing_courier = $no_coordinate_runtime->quote( new \WallsShop\WDC\Domain\Quote\QuoteRequest( $mapped_request->country_code, $mapped_request->destination, $mapped_request->package, $mapped_request->payment_method, $mapped_request->order_total, $mapped_request->calculation_date, array_diff_key( $mapped_request->customer_context, array( 'delivery_type' => true ) ) ) );
oz_checkout_assert( 1 === count( $pickup_survives_missing_courier->rates ) && DeliveryType::PICKUP === $pickup_survives_missing_courier->rates[0]->delivery_type, 'Ozon pickup rate must still appear when courier is unavailable because selected location has no coordinates.' );
$session->save_city_context( array( 'location_id' => 650000, 'city_name' => 'Новосибирск', 'country_code' => 'RU' ) );
