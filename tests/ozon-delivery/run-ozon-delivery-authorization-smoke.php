...
oa_assert( str_contains( $token, "'grant_type' => 'client_credentials'") && str_contains( $token, "'scope' => OzonDeliverySettings::TOKEN_SCOPE" ), 'OAuth must use the documented client_credentials JSON body.' );
...
oa_assert( ! str_contains( $token, 'refresh_token' ) && ! str_contains( $token, 'expires_at' ) && ! str_contains( $token, 'set_transient' ) && ! str_contains( $token, 'update_option' ), 'token service must not invent token lifecycle or persistence.' );
