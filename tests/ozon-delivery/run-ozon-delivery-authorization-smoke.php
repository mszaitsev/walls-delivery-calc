...
oa_assert( str_contains( $source, 'MAX_REDIRECTS = 12' ) && str_contains( $source, 'redirect_rejected' ) && str_contains( $source, 'redirect_limit' ), 'redirect hop limit and fail-closed rejection are required.' );
...
$GLOBALS['oa_requests'] = array(); $GLOBALS['oa_responses'] = array_fill( 0, 12, array( 'status' => 302, 'headers' => array( 'location' => 'https://xapi.ozon.ru/challenge' ) ) ); $GLOBALS['oa_responses'][] = array( 'status' => 200, 'body' => '{}' ); $transport->request( 'POST', 'https://xapi.ozon.ru/oauth/token', array() ); oa_assert( 13 === count( $GLOBALS['oa_requests'] ), 'a valid same-host Ozon chain with exactly MAX_REDIRECTS redirects must be accepted.' );
foreach ( array( '', 'http://xapi.ozon.ru/challenge', 'https://attacker.example/challenge' ) as $location ) {
	...
	try { $transport->request( 'POST', 'https://xapi.ozon.ru/oauth/token', array( 'headers' => array( 'Authorization' => 'Bearer temporary-token' ), 'body' => 'sensitive-token-body' ) ); oa_assert( false, 'unsafe redirect must fail.' ); } catch ( OzonDeliveryApiException $e ) { oa_assert( $e->safe_code === 'redirect_rejected' && false === $e->retryable && count( $GLOBALS['oa_requests'] ) === 1, 'unsafe or missing redirect must not receive sensitive follow-up request and must not be retryable.' ); }
}
...
try { $transport->request( 'POST', 'https://xapi.ozon.ru/oauth/token' ); oa_assert( false, 'redirect loop must fail.' ); } catch ( OzonDeliveryApiException $e ) { oa_assert( $e->safe_code === 'redirect_limit' && true === $e->retryable && count( $GLOBALS['oa_requests'] ) === 13, 'one valid same-host redirect beyond the bounded hop limit must become retryable redirect_limit.' ); }
