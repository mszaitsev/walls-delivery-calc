...
$untrusted_exact_request = new QuoteRequest( 'RU', $request->destination, $request->package, $request->payment_method, $request->order_total, $request->calculation_date, array( 'recipient_phone' => '+79991234567', 'selected_location_id' => 123, 'dadata_status' => 'resolved', 'dadata_street' => 'Тверская улица', 'dadata_house' => '1', 'dadata_geo_lat' => '55.75511', 'dadata_geo_lon' => '37.622396' ) );
$untrusted_exact_payload = $quote_builder->build_courier( $untrusted_exact_request, $packaging );
oz_quote_assert( 'ozon_pickup_proxy' === (string) ( $untrusted_exact_payload['diagnostics']['courier_coordinate_source'] ?? '' ) && 55.0416 === (float) ( $untrusted_exact_payload['body']['delivery']['courier']['coordinates']['latitude'] ?? 0 ), 'Browser-posted DaData coordinates without server trust must be ignored and fall back to the proxy point.' );
$exact_request = new QuoteRequest( 'RU', $request->destination, $request->package, $request->payment_method, $request->order_total, $request->calculation_date, array( 'recipient_phone' => '+79991234567', 'selected_location_id' => 123, 'dadata_trusted' => true, 'dadata_selected_location_id' => 123, 'dadata_status' => 'resolved', 'dadata_street' => 'Тверская улица', 'dadata_house' => '1', 'dadata_geo_lat' => '55.75511', 'dadata_geo_lon' => '37.622396' ) );
