...
oz_two_phase_assert( str_contains( $repository, 'INSERT IGNORE' ) && str_contains( $initial_schema, 'UNIQUE KEY generation_point' ) && str_contains( $repository, 'count_ids( $generation_id )' ), 'Discovery ID persistence must be unique and idempotent.' );
...
oz_two_phase_assert( str_contains( $repository, 'commit_enrichment_batch' ) && str_contains( $repository, 'insert_points_bulk' ) && str_contains( $repository, 'mark_ids_enriched' ) && str_contains( $repository, 'mark_ids_rejected' ) && str_contains( $repository, 'count( $accepted_ids )' ) && str_contains( $repository, 'count( $normalized_rejects )' ) && str_contains( $repository, "'enrichment_processed_count'" ) && str_contains( $repository, "'accepted_count'" ) && str_contains( $repository, "'rejected_count'" ), 'Enrichment commits must use bounded set-based writes and update terminal counters atomically.' );
...
oz_two_phase_assert( str_contains( $initial_schema, 'wdc_ozon_delivery_pickup_ids' ) && str_contains( $initial_schema, "status varchar(24) NOT NULL DEFAULT 'pending'" ) && str_contains( $initial_schema, 'reject_code varchar(40) NULL' ) && str_contains( $initial_schema, 'KEY generation_status_id (generation_id,status,id)' ), 'The initial schema must create the compact staging ID table.' );
oz_two_phase_assert( str_contains( $initial_schema, 'phase varchar(16)' ) && str_contains( $initial_schema, 'discovery_page_count' ) && str_contains( $initial_schema, 'discovered_count' ) && str_contains( $initial_schema, 'discovery_completed_at' ) && str_contains( $initial_schema, 'enrichment_processed_count' ) && str_contains( $initial_schema, 'lock_owner varchar(64) NULL' ), 'The initial schema must contain final two-phase progress and lock fields.' );
