...
oz_two_phase_assert( str_contains( $repository, 'ready_for_activation' ) && str_contains( $repository, "accepted_count'] + (int) \$generation['rejected_count'] === \$discovered" ) && str_contains( $repository, "enrichment_processed_count'] === \$discovered" ) && str_contains( $repository, 'pending_count( $generation_id )' ), 'Activation must require all frozen IDs to be terminal.' );
...
oz_two_phase_assert( str_contains( $scheduler, 'stop_manual' ) && str_contains( $scheduler, 'building_generation' ) && str_contains( $scheduler, 'cancel_generation' ) && str_contains( $scheduler, 'unschedule( self::STEP_HOOK' ) && str_contains( $scheduler, '$this->lock->release( $owner )' ), 'Scheduler must expose manual stop without changing the generic step hook.' );
oz_two_phase_assert( str_contains( $lock, 'current_owner' ) && str_contains( $scheduler, '$this->lock->release( $owner )' ), 'Cancellation must release only the current owner lock when available.' );
