...
oz_pickup_scheduler_assert( str_contains( $scheduler, 'public function stop_manual' ) && str_contains( $scheduler, '$this->importer->building_generation()' ) && str_contains( $scheduler, '$this->lock->current_owner()' ) && str_contains( $scheduler, '$this->importer->cancel_generation' ) && str_contains( $scheduler, 'unschedule( self::STEP_HOOK, array( (string) $generation[\'job_id\'], $owner ), self::GROUP )' ), 'Manual stop must mark the current building generation cancelled and unschedule only exact current owner step args when available.' );
