...
oz_pickup_scheduler_assert( str_contains( $scheduler, "\$delay = ! empty( \$result['retry'] )" ) && str_contains( $scheduler, "\$result['retry_after']" ) && str_contains( $scheduler, 'time() + $delay' ) && strpos( $scheduler, '$this->lock->release( $owner )' ) < strpos( $scheduler, '$delay = ! empty' ), 'Retry steps must be scheduled with importer-provided backoff while keeping the generation lock held.' );
