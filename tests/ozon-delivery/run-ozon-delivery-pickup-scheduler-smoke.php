...
oz_pickup_scheduler_assert( str_contains( $scheduler, 'schedule_recurring' ) && str_contains( $scheduler, '86400' ) && str_contains( $scheduler, 'TimezoneService' ) && ! str_contains( $scheduler, 'wp_timezone' ) && str_contains( $scheduler, 'reschedule' ), 'Daily schedule must use the WDC timezone owner and support rescheduling.' );
...
oz_pickup_scheduler_assert( str_contains( $scheduler, 'null === $generation_id' ) && str_contains( $scheduler, 'pickup_step_schedule_failed' ) && substr_count( $scheduler, 'fail_job_and_release' ) >= 3 && str_contains( $scheduler, "if ( \$result['complete'] )" ) && str_contains( $scheduler, '$this->lock->owns( $owner )' ), 'First and next step enqueue failures must fail the job, release only the matching owner lock and retain normal completion.' );
...
oz_pickup_scheduler_assert( str_contains( $run_step, "\$delay = ! empty( \$result['retry'] )" ) && str_contains( $run_step, "\$result['retry_after']" ) && str_contains( $run_step, 'time() + $delay' ) && strpos( $run_step, '$this->lock->release( $owner )' ) < strpos( $run_step, '$delay = ! empty' ), 'Retry steps must be scheduled with importer-provided backoff while keeping the generation lock held.' );
