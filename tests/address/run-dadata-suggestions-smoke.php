...
foreach ( array( 'activeCheckoutPrefix', 'selectorFor( activePrefix, \'address_1\' )', 'selectorFor( activePrefix, \'city\' )', 'selectorFor( activePrefix, \'address_2\' )', 'textarea[name="', 'shipping', 'billing', 'address_1', 'shipping_city', 'billing_city', 'shipping_address_1', 'billing_address_1', 'postcode', 'house_after_street', 'resolve', 'street_selected', 'resolved', 'dadata_status', 'dadata_unrestricted_value', 'dadata_region_fias_id', 'dadata_city_kladr_id', 'dadata_street_fias_id', 'dadata_house_fias_id', 'dadata_fias_level', 'update_checkout', 'updated_checkout', 'wc_fragments_refreshed', 'wdc_platform_dadata_address_suggest', 'address suggestions script loaded', 'config enabled', 'config disabled', 'DaData подсказки:', 'api key ready:', 'encryption ready:', 'active mode:', 'active address field:', 'active city field:', 'shipping mode active', 'billing mode active', 'using address field selector', 'address field found', 'address field not found', 'address input event', 'DaData input detected', 'ajax request start', 'ajax success items count', 'ajax fail', 'suggestion popup opened', 'suggestion selected', 'resolve request start', 'resolve request success', 'debounceDelay = 300', 'debounceTimers', 'itemStore', 'data-key', 'setHiddenData' ) as $needle ) {
...
dadata_suggestions_assert( str_contains( $js, 'shippingChecked && ( shippingAddressVisible || shippingCityVisible )' ), 'activeCheckoutPrefix must use shipping only when shipping fields are visible and ship-to-different is checked.' );
dadata_suggestions_assert( str_contains( $js, 'var prefix = activeCheckoutPrefix();' ), 'renderDebugBlock must use the active checkout prefix.' );
