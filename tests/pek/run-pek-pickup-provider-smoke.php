...
$free_marker = $pek_formatter->format( $points[0], str_repeat( 'a', 64 ), 10, 'RU' );
$paid_marker = $pek_formatter->format( $points[1], str_repeat( 'a', 64 ), 10, 'RU' );
pek_pickup_assert( 'terminal' === (string) ( $free_marker['marker_type'] ?? '' ) && 'highlighted' === (string) ( $paid_marker['marker_type'] ?? '' ) && 'highlighted' === (string) ( $paid_marker['snapshot']['marker_type'] ?? '' ), 'PEK own/free terminal must stay blue while paid partner uses generic highlighted marker semantics.' );
