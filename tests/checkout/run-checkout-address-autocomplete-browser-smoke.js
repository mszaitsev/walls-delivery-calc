  ...
  try {
    for (const width of [1200, 390]) {
      ...
      await page.setContent(`<style>body{font:16px Arial;margin:20px}form{max-width:600px}input{box-sizing:border-box;width:100%;padding:12px}label{display:block;margin:12px 0} .woocommerce-input-wrapper{display:block}</style>
        ...
        <input type="hidden" name="wdc_platform_location_id" value="1"><input type="hidden" name="wdc_platform_location_fias_id" value="city-fias"><input type="hidden" name="wdc_platform_location_selected_source" value="database"><button type="submit">Оплатить заказ</button></form>`);
      await page.addScriptTag({ path: process.env.JQUERY_PATH });
      await page.evaluate(() => {
        window.wdcPlatformAddressSuggestions = { enabled:true, min_chars:3 };
        window.requests=[]; window.updates=0; window.submits=0;
      ...
      assert.equal(await page.locator('.wdc-address-autocomplete-spinner').count(),1);
      assert.equal(await page.locator('.wdc-address-autocomplete-spinner').evaluate(n=>getComputedStyle(n).animationName),'wdc-address-autocomplete-spin');
      await page.emulateMedia({reducedMotion:'reduce'});
      assert.equal(await page.locator('.wdc-address-autocomplete-spinner').evaluate(n=>getComputedStyle(n).animationName),'none');
      ...
      await input.fill('Красный проспект 13 кв 2');
      await page.waitForFunction(()=>window.requests.length===5);
      assert.equal(await page.evaluate(()=>window.requests[4].data.query),'Красный проспект 13');
      await page.evaluate(()=>window.requests[4].d.resolve({success:true,items:[{input_value:'Красный проспект, д 13',display_label:'Красный проспект, д 13',level:'house',is_final:true,data:{house:'13'}}]}));
      await page.locator('.wdc-address-autocomplete-option').click();
      assert.equal(await input.inputValue(),'Красный проспект, д 13, кв 2');
      await input.press('Enter');
      assert.equal(await input.inputValue(),'Красный проспект, д 13, кв 2');
      assert(await input.evaluate(n=>n!==document.activeElement));
      assert.equal(await page.evaluate(()=>window.submits),0);
      await input.fill('ул Ленина, д 10, ');
      await input.press('Enter');
      assert.equal(await input.inputValue(),'ул Ленина, д 10');
      assert.equal(await page.evaluate(()=>window.submits),0);
      await input.fill('ул Ленина, д 10,,, ');
      ...
      assert.equal(await input.inputValue(),'ул Ленина, д 10');
      assert(await input.evaluate(n=>n!==document.activeElement));
      await input.fill('ул Ленина, д 10, '); await input.press('Tab');
      assert.equal(await input.inputValue(),'ул Ленина, д 10');
      assert(await page.locator('#billing_phone').evaluate(n=>n===document.activeElement));
      await page.evaluate(()=>jQuery(document.body).trigger('updated_checkout'));
      assert.equal(await input.inputValue(),'ул Ленина, д 10');
      await page.evaluate(()=>{ document.querySelector('[name="wdc_platform_location_id"]').value='2'; document.body.dispatchEvent(new CustomEvent('wdc:location-selected')); });
      assert.equal(await input.inputValue(),'');
      await input.fill('ул Ленина, д 10, ');
      await page.evaluate(()=>{ document.querySelector('#billing_country').value='KZ'; jQuery('#billing_country').trigger('change'); });
      assert.equal(await input.inputValue(),'');
      await input.fill('Свободный адрес, '); await input.press('Enter');
      assert.equal(await input.inputValue(),'Свободный адрес');
      assert.equal(await page.evaluate(()=>window.submits),0);
      await page.close();
    }
