...
runtime_smoke_assert( ! str_contains( $rate_renderer_source, "\$meta['planned_delivery_comment']" ), 'Rate renderer must not output planned_delivery_comment as a separate block for single methods.' );
...
runtime_smoke_assert( str_contains( $rate_mapper_source, "rtrim( \$title ) . ' — ' . \$final_delivery_label" ) && ! str_contains( $rate_mapper_source, "\$label .= ' - ' . \$planned_delivery_comment" ), 'Single-rate labels must append final delivery days with an em dash and must not use planned_delivery_comment label concatenation.' );
