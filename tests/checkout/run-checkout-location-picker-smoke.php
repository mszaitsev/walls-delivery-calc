...
$technical_postcode_payload = $ajax->payload( 'Безиндексная' )['groups'][0]['items'][0] ?? array();
checkout_location_picker_assert( '' === (string) ( $technical_postcode_payload['postal_code'] ?? 'missing' ), 'Checkout location payload must expose technical postcode sentinel as missing.' );
checkout_location_picker_assert( ! str_contains( (string) ( $technical_postcode_payload['option_label'] ?? '' ), '999999999' ), 'Checkout picker option must not expose technical postcode sentinel.' );
checkout_location_picker_assert( 'д. Безиндексная - Московская обл.' === (string) ( $technical_postcode_payload['option_label'] ?? '' ), 'Technical postcode omission must preserve clean option punctuation.' );
...
$profile_underscore = $profile_matcher->match( 'RU', 'Соль_Илецк', 'Оренбургская область' );
checkout_location_picker_assert( 'resolved' === $profile_underscore['status'] && $profile_underscore['location'] instanceof Location && 'fias-sol-iletsk' === $profile_underscore['location']->fias_id, 'Profile matcher candidate lookup supports underscore separators before exact PHP normalization.' );
...
checkout_location_picker_assert( 12 === $wpdb->location_profile_candidate_lookup_calls, 'Profile matcher uses one bounded candidate lookup per supported-country reconciliation.' );
...
checkout_location_picker_assert( is_string( $city_js ) && str_contains( $city_js, 'applySelectedLocation( body.selected, { updateCheckout: true, explicit: false, source: \'auto\', updateFields: true } )' ) && str_contains( $city_js, 'setFieldValueSilently' ), 'Auto-resolve canonicalizes visible fields silently before one final update_checkout.' );
...
checkout_location_picker_assert( is_string( $city_js ) && str_contains( $city_js, 'if ( ! hasSelectedLocation() && ! hasManualLocationSelection() )' ) && str_contains( $city_js, 'scheduleAutoResolve();' ), 'updated_checkout restores notice without auto-resolve when canonical or manual selection exists.' );
...
checkout_location_picker_assert( is_string( $city_js ) && str_contains( $city_js, 'setHidden( \'wdc_platform_location_selected_source\', \'manual\' )' ) && str_contains( $city_js, 'Указан неизвестный населенный пункт. Доставка может не рассчитаться' ), 'Manual fallback is explicitly marked and displayed as unknown manual city.' );
