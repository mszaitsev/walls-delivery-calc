...
final class DeliveryServicesAdminPage {
	...
	public function __construct(
		...
		private ?YandexLocationMapperV2Service $yandex_location_mapper_v2 = null,
	) {
		$this->logger = $logger;
	}
	...
	public function handle_actions(): void {
		...
		if ( in_array( $action, array(
			...
			if ( 'save_main' === $action && $this->settings instanceof DeliveryServiceSettingsRepository ) {
				...
				if ( $service instanceof DeliveryService && $this->is_self_pickup_service( $service ) && null !== $service->id && $this->self_pickup_settings instanceof SelfPickupSettings ) {
					$this->self_pickup_settings->save_main_from_admin( (int) $service->id, $_POST );
					$this->clear_delivery_quote_cache();
				}
			...
			if ( 'save_calculation' === $action && $this->settings instanceof DeliveryServiceSettingsRepository ) {
				...
				if ( $service instanceof DeliveryService && $this->is_self_pickup_service( $service ) && null !== $service->id && $this->self_pickup_settings instanceof SelfPickupSettings ) {
					$this->self_pickup_settings->save_discount_from_admin( (int) $service->id, $_POST );
					$this->clear_delivery_quote_cache();
				}
			}
	...
	private function render_main_tab( DeliveryService $service ): void {
		...
		<form method="post" style="max-width: 760px;">
			...
			<table class="form-table" role="presentation">
				...
				<?php endif; ?>
				<?php if ( $this->is_self_pickup_service( $service ) && $this->self_pickup_settings instanceof SelfPickupSettings && null !== $service->id ) : ?>
					<?php $this->render_self_pickup_main_rows( (int) $service->id ); ?>
				<?php endif; ?>
			</table>
	...
	private function render_calculation_tab( DeliveryService $service ): void {
		...
		<form method="post" style="max-width: 860px;">
			...
			<table class="form-table" role="presentation">
				...
				<?php endif; ?>
				<?php if ( $this->is_self_pickup_service( $service ) && $this->self_pickup_settings instanceof SelfPickupSettings && null !== $service->id ) : ?>
					<?php $this->render_self_pickup_discount_rows( (int) $service->id ); ?>
				<?php endif; ?>
			</table>
	...
	private function handle_yandex_region_mapping_v2_action( string $action ): void {
		...
		$service_key = sanitize_key( wp_unslash( $_POST['service_key'] ?? YandexDeliverySettings::SERVICE_KEY ) );
		if ( in_array( $action, array( 'sync_yandex_region_mapping_v2', 'save_yandex_region_mapping_v2' ), true ) && $this->is_yandex_geo_pipeline_active() ) {
	...
	private function handle_yandex_location_manual_override_v2_action( string $action ): void {
		...
		if ( 'save_yandex_location_manual_override_v2' === $action ) {
			...
			$remap = 1 === (int) ( $report['saved'] ?? 0 ) ? $this->remap_yandex_location_geo_id_if_safe( $geo_id ) : array( 'status' => 'not_started' );
			$message = 1 === (int) ( $report['saved'] ?? 0 ) ? $this->yandex_location_override_result_message( 'Override сохранен.', $remap ) : 'Override не сохранен.';
			$this->save_yandex_region_mapping_v2_result( 1 === (int) ( $report['saved'] ?? 0 ) ? 'success' : 'error', 'Ручной override Яндекс mapping v2', $message, array_merge( array( 'yandex_geo_id' => $geo_id, 'location_id' => $location_id, 'targeted_remap' => $remap ), $report ) );
		} elseif ( 'deactivate_yandex_location_manual_override_v2' === $action ) {
			$id = isset( $_POST['override_id'] ) ? (int) $_POST['override_id'] : 0;
			$override = $this->yandex_location_manual_override_v2_repository->find_by_id( $id );
			$ok = array() !== $override && $this->yandex_location_manual_override_v2_repository->deactivate_override( $id );
			$geo_id = (int) ( $override['yandex_geo_id'] ?? 0 );
			$remap = $ok ? $this->remap_yandex_location_geo_id_if_safe( $geo_id ) : array( 'status' => 'not_started' );
	...
	/** @return array<string,mixed> */
	private function remap_yandex_location_geo_id_if_safe( int $geo_id ): array {
		if ( $geo_id <= 0 || ! $this->yandex_delivery_geo_v2_repository instanceof YandexDeliveryGeoV2Repository || ! $this->yandex_location_mapping_v2_repository instanceof YandexLocationMappingV2Repository || ! $this->yandex_location_mapper_v2 instanceof YandexLocationMapperV2Service ) {
			return array( 'status' => 'unavailable' );
		}
		if ( $this->is_yandex_geo_pipeline_active() || $this->is_yandex_location_mapping_v2_active() ) {
			return array( 'status' => 'deferred_active_pipeline' );
		}
		$geo = $this->yandex_delivery_geo_v2_repository->find_by_geo_id( $geo_id );
		if ( ! is_array( $geo ) ) {
			return array( 'status' => 'source_missing' );
		}
		$this->yandex_location_mapper_v2->reset_manual_override_cache();
		$rows = $this->yandex_location_mapper_v2->map_geo_row( $geo );
		if ( array() === $rows || ! $this->yandex_location_mapping_v2_repository->replace_geo_rows( $geo_id, $rows ) ) {
			return array( 'status' => 'replace_failed' );
		}
		return array( 'status' => 'updated', 'mapping_status' => (string) ( $rows[0]['status'] ?? '' ), 'rows' => count( $rows ) );
	}

	private function is_yandex_location_mapping_v2_active(): bool {
		if ( ! $this->yandex_location_mapping_v2_runner instanceof YandexLocationMappingV2Runner ) {
			return false;
		}
		return in_array( (string) ( $this->yandex_location_mapping_v2_runner->current_state()['status'] ?? '' ), array( 'mapping', 'paused' ), true );
	}

	/** @param array<string,mixed> $remap */
	private function yandex_location_override_result_message( string $prefix, array $remap ): string {
		return match ( (string) ( $remap['status'] ?? '' ) ) {
			'updated' => $prefix . ' Live mapping для geoId обновлен.',
			'deferred_active_pipeline' => $prefix . ' Будет применен текущим или следующим запуском mapping, поскольку pipeline сейчас активен.',
			'source_missing' => $prefix . ' Immediate mapping не выполнен: canonical geo_v2 row не найден.',
			'replace_failed' => $prefix . ' Immediate mapping не выполнен: live mapping не удалось атомарно заменить.',
			default => $prefix . ' Immediate mapping недоступен; override будет применен следующим запуском mapping.',
		};
	}
	...
	private function render_yandex_location_manual_overrides_v2_section( DeliveryService $service, array $review_items, array $no_match_items, array $overrides, int $review_total, int $review_page, int $override_total, int $override_page ): void {
		...
		<p class="description" style="max-width: 960px;"><?php echo esc_html__( 'Админ может сохранить правильный WDC location для needs_review/no_match. Когда pipeline свободен, один geoId пересчитывается сразу; при активном pipeline override применяется его текущим или следующим запуском.', 'walls-delivery-calc' ); ?></p>
		<?php $review_from = 0 === $review_total ? 0 : ( ( $review_page - 1 ) * self::YANDEX_LOCATION_REVIEW_PAGE_SIZE ) + 1; ?>
		<?php $review_to = min( $review_total, $review_page * self::YANDEX_LOCATION_REVIEW_PAGE_SIZE ); ?>
		<p><strong><?php echo esc_html( sprintf( __( 'К ручной проверке: %1$d geoId. Показано %2$d–%3$d из %1$d.', 'walls-delivery-calc' ), $review_total, $review_from, $review_to ) ); ?></strong></p>
...
Get-ChildItem "D:\russian-post-passport-all"</code></pre>
	...
	private function render_self_pickup_main_rows( int $service_id ): void {
		if ( ! $this->self_pickup_settings instanceof SelfPickupSettings ) {
			return;
		}
		...
		<?php $this->structured_comment_rows( SelfPickupSettings::CUSTOMER_COMMENT_KEY, __( 'Комментарий покупателю', 'walls-delivery-calc' ), $comment ); ?>
		<?php
	}

	private function render_self_pickup_discount_rows( int $service_id ): void {
		if ( ! $this->self_pickup_settings instanceof SelfPickupSettings ) {
			return;
		}
		$minimum = number_format( $this->self_pickup_settings->discount_minimum_kopecks( $service_id ) / 100, 2, '.', '' );
		$comment = $this->self_pickup_settings->discount_comment( $service_id );
		?>
		<tr><th colspan="2"><h3><?php echo esc_html__( 'Скидка за самовывоз', 'walls-delivery-calc' ); ?></h3></th></tr>
	...
	private function structured_comment_rows( string $name, string $label, array $comment ): void {
		?>
		<tr>
			<th scope="row"><?php echo esc_html( $label ); ?></th>
			<td>
				<textarea class="large-text" rows="3" name="<?php echo esc_attr( $name ); ?>[text]"><?php echo esc_textarea( (string) ( $comment['text'] ?? '' ) ); ?></textarea>
				<p><input class="regular-text" name="<?php echo esc_attr( $name ); ?>[text_before]" placeholder="<?php echo esc_attr__( 'Текст до ссылки', 'walls-delivery-calc' ); ?>" value="<?php echo esc_attr( (string) ( $comment['text_before'] ?? '' ) ); ?>"></p>
				<p><input class="regular-text" name="<?php echo esc_attr( $name ); ?>[label]" placeholder="<?php echo esc_attr__( 'Текст ссылки', 'walls-delivery-calc' ); ?>" value="<?php echo esc_attr( (string) ( $comment['label'] ?? '' ) ); ?>"></p>
				<p><input class="regular-text" name="<?php echo esc_attr( $name ); ?>[url]" placeholder="https://example.com" value="<?php echo esc_url( (string) ( $comment['url'] ?? '' ) ); ?>"></p>
				<p><input class="regular-text" name="<?php echo esc_attr( $name ); ?>[text_after]" placeholder="<?php echo esc_attr__( 'Текст после ссылки', 'walls-delivery-calc' ); ?>" value="<?php echo esc_attr( (string) ( $comment['text_after'] ?? '' ) ); ?>"></p>
			</td>
		</tr>
		<?php
	}
	...
	private function is_self_pickup_service( ?DeliveryService $service ): bool {
		return $service instanceof DeliveryService && SelfPickupSettings::SERVICE_KEY === $service->service_key && SelfPickupSettings::CARRIER_KEY === $service->carrier_key;
	}
