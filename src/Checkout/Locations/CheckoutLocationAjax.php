...
final class CheckoutLocationAjax {
	...
	public function handle(): void {
		...
		$this->send_success( $this->payload( $query, $force_region_code ) );
	}
	...
	public function handle_resolve(): void {
		...
		$resolved = $this->search->resolve_checkout_fields( $region_text, $city_text );
	...
	public function payload( string $query, string $force_region_code = '' ): array {
		...
		$result = $this->search->search_for_picker( $query, $limit, $region_limit, $force_region_code );
		$groups = array();
