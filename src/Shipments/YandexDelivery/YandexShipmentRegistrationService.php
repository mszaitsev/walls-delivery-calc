...
final class YandexShipmentRegistrationService {
	...
	public function create( ShipmentCreateRequest $request ): ShipmentCreateResult {
		try {
			...
			return new ShipmentCreateResult( false, error_code: (string) ( $exception->details()['error_code'] ?? 'yandex_api_error' ), error_message: $exception->getMessage(), raw_reference: $exception->details() );
	...
	public function update_status( object $order ): array {
		...
		try {
			...
			$this->repository->save( $order, $this->merge_info( $shipment, $info ) );
	...
	public function cancel( object $order ): array {
		...
		try {
			...
			$info = $this->client->request_info( $request_id );
			$updated = $this->merge_info( $shipment, $info );
			$updated['yandex_cancel_state'] = $cancel_state->raw;
			$this->repository->save( $order, $updated );
			$this->add_order_note( $order, 'Отправление Яндекс отменено. Статус: ' . $info->status );

			return array( 'success' => true, 'message' => 'Отмена Яндекс.Доставки запрошена.', 'status' => $info->status );
		} catch ( YandexDeliveryApiException $exception ) {
			return array( 'success' => false, 'message' => $exception->getMessage(), 'details' => $exception->details() );
		}
	}
	...
	private function merge_info( array $shipment, YandexDeliveryRequestInfo $info ): array {
		...
		return array_merge( $shipment, $fields, array( 'status' => 'created', 'status_title' => 'Статус Яндекс.Доставки: ' . $info->status, 'updated_at' => $this->now() ) );
	}
