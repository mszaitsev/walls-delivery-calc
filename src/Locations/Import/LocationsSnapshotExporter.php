...
final class LocationsSnapshotExporter {
	...
	public function export_to_file( string $path, string $version = '0.15.7', int $page_size = 1000 ): int {
	...
	public function stream_download( string $version = '0.15.7' ): void {
	 ...
	 */
	public function create_job( string $path, string $version = '0.15.7' ): array {
		return array(
			...
			if ( 0 === (int) ( $job['table_index'] ?? 0 ) && 0 === (int) ( $job['offset'] ?? 0 ) && ( ! file_exists( $path ) || 0 === (int) filesize( $path ) ) ) {
				file_put_contents(
					...
					$this->encode(
						array(
							'type'       => 'meta',
							'version'    => (string) ( $job['version'] ?? '0.15.7' ),
							'tables'     => $this->tables,
							'created_at' => current_time( 'mysql' ),
						)
					) . "\n"
