	...
	public function render_embedded( DeliveryService $service, array $notice = array() ): void {
		...
		<form method="post" style="max-width: 760px;">
			...
			<table class="form-table" role="presentation">
				...
				<tr><th scope="row">Страховка, %</th><td><input type="text" class="small-text" name="<?php echo esc_attr( JetLogisticSettings::INSURANCE_PERCENT_KEY ); ?>" value="<?php echo esc_attr( (string) $this->settings->insurance_percent() ); ?>"><p class="description"><?php echo esc_html__( 'Процент от стоимости товаров после скидок.', 'walls-delivery-calc' ); ?></p></td></tr>
				<tr><th scope="row">Минимальная страховка, ₽</th><td><input type="text" class="small-text" name="<?php echo esc_attr( JetLogisticSettings::INSURANCE_MIN_RUB_KEY ); ?>" value="<?php echo esc_attr( (string) $this->settings->insurance_min_rub() ); ?>"><p class="description"><?php echo esc_html__( 'Минимальная сумма страхового сбора Jet Logistic.', 'walls-delivery-calc' ); ?></p></td></tr>
				<tr><th scope="row">Курьер по Алматы</th><td><label><input type="checkbox" name="<?php echo esc_attr( JetLogisticSettings::ALMATY_FREE_COURIER_KEY ); ?>" value="1" <?php checked( $this->settings->almaty_free_courier() ); ?>> <?php echo esc_html__( 'Доставка в Алматы курьером бесплатно', 'walls-delivery-calc' ); ?></label><p class="description"><?php echo esc_html__( 'Если включено, для доставки в Алматы стоимость городской курьерской доставки Jet считается равной 0 ₽ независимо от price_delivery в ответе API.', 'walls-delivery-calc' ); ?></p></td></tr>
