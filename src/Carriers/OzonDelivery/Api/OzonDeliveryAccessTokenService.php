...
final class OzonDeliveryAccessTokenService {
	public function __construct( private OzonDeliveryCredentials $credentials, private OzonDeliveryHttpClientInterface $http, private OzonDeliveryMessageSanitizer $sanitizer, private OzonDeliveryTokenCache $cache ) {}
	public function get_token(): string {
		$cached = $this->cache->get( $this->credentials->account_fingerprint() );
		if ( $cached instanceof OzonDeliveryToken ) {
			return $cached->access_token;
		}
		return $this->obtain_fresh()->access_token;
	}
	public function obtain_fresh(): OzonDeliveryToken {
		...
		$token = new OzonDeliveryToken( trim( $data['access_token'] ), $this->expires_at( $data['expires_in'] ?? null ), $this->scope( $data['scope'] ?? array() ), $this->token_type( $data['token_type'] ?? '' ) );
		$this->cache->store( $this->credentials->account_fingerprint(), $token );
		return $token;
	}
	public function token_is_cached( OzonDeliveryToken $token ): bool { return null !== $token->expires_at && $token->expires_at > time() + 60 && $this->cache->get( $this->credentials->account_fingerprint() ) instanceof OzonDeliveryToken; }
	private function http_error( OzonDeliveryApiResponse $response, mixed $data ): OzonDeliveryApiException { $error = is_array( $data ) && isset( $data['error'] ) && is_array( $data['error'] ) ? $data['error'] : ( is_array( $data ) && ! array_is_list( $data ) ? $data : array() ); $code = $this->sanitizer->code( $error['code'] ?? '' ); return new OzonDeliveryApiException( 'oauth_token', '' !== $code ? $code : 'oauth_http_error', $response->status_code, 429 === $response->status_code || $response->status_code >= 500, $this->sanitizer->sanitize( $error['message'] ?? null, 'OAuth-запрос Ozon Delivery завершился ошибкой.' ), $this->response_metadata( $response, $data ) ); }
	...
	private function expires_at( mixed $value ): ?int { if ( is_int( $value ) || is_string( $value ) && 1 === preg_match( '/^[0-9]+$/', $value ) ) { $expires_at = (int) $value; return $expires_at > time() ? $expires_at : null; } return null; }
	/** @return array<int,string> */ private function scope( mixed $value ): array { if ( ! is_array( $value ) ) { return array(); } $scope = array(); foreach ( array_slice( $value, 0, 10 ) as $item ) { $item = is_string( $item ) ? trim( $item ) : ''; if ( '' !== $item && 1 === preg_match( '/^[A-Za-z0-9._-]{1,100}$/', $item ) ) { $scope[] = $item; } } return array_values( array_unique( $scope ) ); }
	private function token_type( mixed $value ): string { $value = is_string( $value ) ? strtolower( trim( $value ) ) : ''; return 1 === preg_match( '/^[a-z0-9._-]{1,60}$/', $value ) ? $value : ''; }
}
