...
final class OzonDeliveryPickupScheduler {
	...
	public function run_step( string $job_id, string $owner ): void { if ( ! $this->lock->renew( $owner ) ) { return; } $result = $this->importer->run_step( $job_id ); if ( $result['complete'] ) { $this->lock->release( $owner ); return; } $this->scheduler->schedule_single( time() + 1, self::STEP_HOOK, array( $job_id, $owner ), self::GROUP ); }
	...
	private function start(): bool { $owner = $this->lock->acquire(); if ( null === $owner ) { return false; } $job_id = bin2hex( random_bytes( 16 ) ); $this->importer->start( $job_id ); if ( null === $this->scheduler->schedule_single( time(), self::STEP_HOOK, array( $job_id, $owner ), self::GROUP ) ) { $this->lock->release( $owner ); return false; } return true; }
