...
final class OzonDeliveryPickupScheduler {
	...
	public function run_step( string $job_id, string $owner ): void { if ( ! $this->lock->renew( $owner ) ) { $this->fail_job_after_lock_renew_failure( $job_id, $owner ); return; } try { $result = $this->importer->run_step( $job_id ); } catch ( \Throwable ) { $this->fail_job_and_release( $job_id, $owner, 'pickup_scheduler_step_failed', 'Не удалось выполнить фоновый шаг синхронизации ПВЗ Ozon.' ); return; } if ( $result['complete'] ) { if ( $this->lock->owns( $owner ) ) { $this->lock->release( $owner ); } return; } $delay = ! empty( $result['retry'] ) ? max( 1, (int) ( $result['retry_after'] ?? 1 ) ) : 1; if ( null === $this->scheduler->schedule_single( time() + $delay, self::STEP_HOOK, array( $job_id, $owner ), self::GROUP ) ) { $this->fail_job_and_release( $job_id, $owner, 'pickup_step_schedule_failed', 'Не удалось запланировать следующий шаг синхронизации ПВЗ Ozon.' ); } }
	...
	private function fail_job_and_release( string $job_id, string $owner, string $code, string $message ): void { try { $this->importer->fail_job( $job_id, $code, $message ); } catch ( \Throwable ) { } if ( $this->lock->owns( $owner ) ) { $this->lock->release( $owner ); } }
