...
final class OzonDeliveryPickupRepository {
	...
	/** @return array<int,array<string,mixed>> */ public function find_active_in_radius( float $latitude, float $longitude, float $latitude_delta, float $longitude_delta, int $limit ): array { $active = $this->active_generation(); if ( ! $active ) { return array(); } $limit = max( 1, min( 100, $limit ) ); $south = $latitude - $latitude_delta; $north = $latitude + $latitude_delta; $west = $longitude - $longitude_delta; $east = $longitude + $longitude_delta; $sql = $this->wpdb->prepare( "SELECT point_id,name,type,full_address,latitude,longitude,schedule,is_active,is_bulky,min_weight_g,max_weight_g,max_width_mm,max_length_mm,max_height_mm FROM {$this->points_table()} WHERE generation_id=%d AND is_active=1 AND latitude IS NOT NULL AND longitude IS NOT NULL AND latitude BETWEEN %f AND %f AND longitude BETWEEN %f AND %f ORDER BY point_id ASC LIMIT %d", $active['id'], $south, $north, $west, $east, $limit ); $rows = $this->wpdb->get_results( $sql, ARRAY_A ); return is_array( $rows ) ? $rows : array(); }
