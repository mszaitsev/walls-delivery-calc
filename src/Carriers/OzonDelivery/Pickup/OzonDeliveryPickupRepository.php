...
final class OzonDeliveryPickupRepository {
	...
	public function start( string $job_id ): int { $now = current_time( 'mysql', true ); $this->wpdb->insert( $this->generations_table(), array( 'state' => 'building', 'job_id' => $job_id, 'started_at' => $now, 'progress_updated_at' => $now ) ); return (int) $this->wpdb->insert_id; }
