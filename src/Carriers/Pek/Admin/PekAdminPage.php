...
final class PekAdminPage {
	...
	private function render_quote_diagnostic_report( array $report ): void {
		...
		$this->render_destination_named_section( 'Cargo policy', is_array( $safe_request['cargo_policy'] ?? null ) ? $safe_request['cargo_policy'] : array(), array( 'product_weight_g' => 'Вес товаров без упаковки, г', 'total_weight_g' => 'Расчётный вес с упаковкой, г', 'light_cargo_threshold_g' => 'Порог обязательной пломбировки, г', 'light_cargo_sealing_required' => 'Пломбировка запрошена', 'sealing_positions_count' => 'Количество пломб', 'protective_transport_packaging_requested' => 'Защитная транспортировочная упаковка запрошена', 'product_weight_known' => 'Вес товаров известен' ) );
