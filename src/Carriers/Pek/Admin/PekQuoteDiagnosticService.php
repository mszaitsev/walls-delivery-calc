...
final class PekQuoteDiagnosticService {
	...
	private function report_from_result( mixed $result, int $location_id, mixed $location, array $mapping, string $mode ): array {
		...
		return array(
			...
			'result' => array(
				'carrier_cost_total_rub' => round( $result->carrier_price_kopecks / 100, 2 ),
				'carrier_price_kopecks' => $result->carrier_price_kopecks,
				'bag_surcharge_kopecks' => $result->bag_surcharge_kopecks,
				'sealing_surcharge_kopecks' => $result->sealing_surcharge_kopecks,
				'light_cargo_surcharge_kopecks' => $result->light_cargo_surcharge_kopecks,
				'final_price_rub' => round( $result->price_kopecks / 100, 2 ),
